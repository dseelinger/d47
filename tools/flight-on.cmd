@echo off
rem flight-on, from cmd and PowerShell. A shim and deliberately nothing more.
rem
rem Where d47 is installed, what the switch is called and what happens when one is already running
rem live in flight-on.ps1 and only there, for the reason the bash shim beside this one gives: two
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0flight-on.ps1" %*
exit /b %ERRORLEVEL%
