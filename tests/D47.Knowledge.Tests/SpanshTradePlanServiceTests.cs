...
public class SpanshTradePlanServiceTests
{
    ...
    private static readonly ArdentCommodityService Commodities =
        new(NullLogger<ArdentCommodityService>.Instance);
    ...
    // Two stations twenty light years apart, both carrying their whole market.
    ...
    public async Task TheMarketsComeBackWithTheStationsAndAreRoutedOverHere()
    {
        ...
        // 384 tonnes at 9,400 out and 52,282 back.
    ...
    {
        // Measured 2026-08-19: a station search answers in 1.1 to 1.3 seconds whatever it returns, so the
        // bill is the number of requests.
        var recorder = new Recorder((HttpStatusCode.OK, TwoMarkets));
        using var service = Service(recorder);
    ...
    public async Task ThePricesTheCommanderReadThemselvesWinWhenTheyAreNewer()
    {
        ...
        // The report's coordinates are kept even when its prices are not: Market.json carries no position,
        // and a market that cannot be placed cannot be routed to.
    ...
    public async Task APriceOlderThanTheCommanderAskedForIsNotPlannedWith()
    {
        ...
        // The destination aged out, so there is nowhere to sell and no route.
        Assert.NotNull(route);
