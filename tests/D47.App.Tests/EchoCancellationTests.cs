...
/// <summary>Echo cancellation, driven with synthesised audio and no hardware.</summary>
public class EchoCancellationTests
{
    private const int CaptureRate = 16_000;
    private const int RenderRate = 48_000;

    /// <summary>A test double for the arbiter's tap.</summary>
    ...
    /// <summary>Counts instead of collecting, for the one test that runs the capture path flat out.</summary>
    ...
    /// <summary>
    /// Throws on the first frame and behaves after that, standing in for the module failing once at
    /// runtime.
    /// </summary>
    ...
    /// <summary>d47 talking, as broadband non-periodic sound rather than as tones.</summary>
    ...
    /// <summary>
    /// A room where d47 is playing through speakers and the microphone hears it 60 ms later at a third
    /// of the level — optionally with the Commander talking over the top.
    /// </summary>
    private sealed class Speakers(EchoCanceller canceller, Tap tap, Collector gate)
    {
        ...
        public List<float> Play(double seconds, bool commanderTalking)
        {
            ...
            for (var frame = 0; frame < (int)(seconds * 100); frame++)
            {
                ...
                {
                    // The capture index maps to the render index three to one, then back by the delay — which
                    // is what makes this an echo of exactly what was played.
    ...
    /// <summary>
    /// A canceller with the noise suppressor switched off, for the two tests that measure how much echo
    /// was removed.
    /// </summary>
    ...
    /// <summary>The claim the whole item rests on.</summary>
    [Fact]
    public void WhatD47PlaysIsSubtractedFromWhatItHears()
    {
        ...
        // Half a second for the filter to converge, which is a real property of every echo canceller ever
        // built rather than something to hide inside the measurement.
    ...
    /// <summary>"Natural interruption", which is the checklist's own phrase for it.</summary>
    [Fact]
    public void TheCommanderCanTalkOverIt()
    {
        ...
        // Some loss is inherent and is not the failure being guarded against: AEC3 applies a residual
        // suppressor while the far end is active, and a few decibels off the near end during double-talk is
        // what every echo canceller in existence does.
        ...
        // Paired with WhatD47PlaysIsSubtractedFromWhatItHears, which measures the other half on the same
        // room: the echo goes and the Commander stays.
        Assert.NotEmpty(echo);
    }

    /// <summary>
    /// Frames arrive from WASAPI at whatever size it feels like, and the module takes exactly 10 ms.
    /// </summary>
    [Fact]
    public void AudioIsNotLostAcrossOddlySizedBuffers()
    {
        ...
        // Whole frames only, with the remainder held for the next call — padding a short frame with silence
        // would tell the canceller the Commander stopped talking, mid-word, several times a second.
        var frame = CaptureRate / 100;

        Assert.Equal(written / frame * frame, gate.Samples.Count);
    }

 /// <summary>A capability being off, not a startup failure.</summary>
    [Fact]
    ...
    public void ItCanBeStartedAndStoppedRepeatedly()
    {
        ...
        // Applying any listening setting runs this path, so it happens on every change of every row — the
        // same lifecycle mistake the microphone and the transcriber both had.
    ...
    /// <summary>
    /// Stopping runs on whichever thread applied a settings row; the capture and render threads are
    /// inside the module continuously.
    /// </summary>
    [Fact]
    public void StoppingWhileAudioIsFlowingDoesNotTakeTheProcessDown()
    {
        // Enough start/stop cycles to have hit the window many times over, and few enough that the whole
        // thing is about a second.
        ...
        // One buffer and one format, reused.
        ...
        // Joined with a deadline rather than waited on, because a thread that never comes back is one of the
        // two failures being guarded against: Stop takes the lifecycle lock and then the processing lock, and
        // anything that ever takes them the other way round hangs here instead of hanging a Commander's
        // machine.
        ...
        // The race was actually driven, rather than the test having measured an arrangement that never
        // happened.
    ...
    /// <summary>
    /// A failure at runtime gives up cleanly: the canceller stops, says why, and the microphone keeps
    /// working.
    /// </summary>
    [Fact]
    ...
    public void ADeviceGoingAwayResetsTheFramingAndTellsTheGate()
    {
        ...
        // The held half belonged to a device that is no longer the one being heard, and joining it to the
        // front of the next device's first frame would be a click at best.
        canceller.Write(new float[80]);
