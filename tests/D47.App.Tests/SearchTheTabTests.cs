...
/// <summary>One search affordance, on the tab that is selected.</summary>
public class SearchTheTabTests
{
    ...
    {
        // Themed, because the highlight is a theme resource: unthemed, every brush lookup resolves to nothing
        // and a test reading the fills back would be comparing two absences.
    ...
    public void AQueryHighlightsEveryHitAndSaysHowManyThereAre()
    {
        ...
        // The lines around the hits stay where they were: reading back a conversation means reading what was
        // around it, so this highlights rather than filters.
    ...
    /// <summary>
    /// Which hit is the current one, told the way a reader tells: it is the one drawn in a different
    /// colour from the rest of them.
    /// </summary>
    ...
    /// <summary>
    /// Mini shows no strip, and a surface that was not handed a search box does not have one — which is
    /// what keeps a box the Commander cannot type into out of the headset.
    /// </summary>
    [AvaloniaFact]
    ...
    public void SurvivingRowsMarkWhatTheQueryFound()
    {
        ...
        // A TextBlock draws its Text and then its Inlines, so a caption that still holds both renders twice —
        // the plain string followed by the marked one.
        Assert.All(
            host.View.GetVisualDescendants().OfType<TextBlock>()
        ...
        // And the marks come off with the query, or a row keeps a highlight for a string nobody is searching
        // for.
        box.Text = string.Empty;
        Avalonia.Threading.Dispatcher.UIThread.RunJobs();
    ...
    public void AKeyOnlyMatchShowsTheKeyThatMatched()
    {
        ...
        // The whole key, though the query was only a prefix of it — the line is evidence about the row, not
    ...
    /// <summary>What a caption says, however it is put together.</summary>
