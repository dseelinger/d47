...
public sealed class EveryTabOpensWhereItWasLeftTests
{
    ...
    /// <summary>
 /// The tab itself, back where it was left — not just the reading each one is on, which is
    /// what the rest of this class already covers.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>Raw Journal is written down as the journal.</summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// A root nothing answers to costs the tab's first reading rather than raising — a renamed reading,
    /// a tab this surface never furnished, or a hand-edited file.
    /// </summary>
    [AvaloniaFact]
    ...
    private static (PanelView Panel, Window Window) Shown(ViewStateStore store)
    {
        ...
        // Last, exactly as the window wires it: a root can only be selected once the tab that owns it has
        // been furnished, and the tab itself after its root so it opens on the reading already restored
        // rather than the tab's first one.
