...
public class AdventuresTabTests
{
        ...
        params Adventure[] adventures)
    {
        ...
        // The palette, so a themed brush is a colour rather than unset — the trigger's highlight is one of
    ...
    /// <summary>The root card says where the story is, how far through it is, and what to do next.</summary>
    [AvaloniaFact]
 ...
 // What to do next, headed "Next" and phrased as an instruction.
    ...
    /// <summary>Pressing Go on a strip wide enough for three panes: the offer must not navigate with the root supplied as well as kept, or the strip hosts the root page in two panes.</summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// The reading level shows what was actually said, not the authored line — and the trigger beside
 /// it is in the highlight colour.
    /// </summary>
    [AvaloniaFact]
 ...
 /// <summary>The wait, drawn.</summary>
    [AvaloniaFact]
    ...
    /// <summary>
 /// Mini reads the tab the panel is on: the story, how far through, what was
    /// just done, what to do next, and the last thing the ship's AI said about it.
    /// </summary>
    [AvaloniaFact]
    public void TheMiniPanelShowsTheStoryAtAGlance()
    {
        ...
        // And the tab strip is still not there.
        Assert.DoesNotContain(drawn, text => text == "Adventures");
    }

    /// <summary>
    /// Mini with no story under way says so, rather than drawing an empty pane that reads as a panel
    /// that failed to load.
    /// </summary>
    [AvaloniaFact]
