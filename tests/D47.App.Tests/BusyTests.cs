...
/// <summary>Anything that might take a moment says it is working.</summary>
public class BusyTests
{
    ...
    {
        // Hidden to begin with, as every one of these is on the surface: a glyph that starts visible would
        // never raise the change this is watching for.
    ...
    /// <summary>
    /// And the slow case shows it, and keeps it up for the minimum — an operation that runs just past
    /// the delay must not put a spinner on screen for ten milliseconds, which is the same flash moved
    /// to the other end.
    /// </summary>
    [AvaloniaFact]
    public async Task TheSlowCaseShowsTheGlyphAndKeepsItUpForTheMinimum()
    {
        ...
        // Just past the delay, so the minimum is what decides how long this takes rather than the work.
    ...
    /// <summary>
    /// The control is shut while it runs, so a second click cannot queue a second copy of the same
    /// fetch — and it is reopened even when the work throws, or a row that failed once is a row that
    ...
    /// <summary>
    /// The Log file reading is the one that touches a disk, so it is the one that says it is working —
    /// on the affordance that was touched.
    /// </summary>
    [AvaloniaFact]
    public void TheModeControlCarriesTheGlyphOnBothSurfaces()
    {
        ...
        {
            // The headset renders this same view, so the glyph has to be part of the panel rather than
            // something the window hangs on it afterwards.
            var view = new PanelView { DataContext = model, Mode = mode };

            // Through the logical tree rather than the visual one, because this view has never been shown and
            // the claim is about a panel the headset rasterises offscreen.
    ...
    /// <summary>
    /// Opening the Log page reads the file off the thread that draws, which is the half of this that
    /// stops a slow read looking like a frozen window rather than a slow one.
    /// </summary>
    [AvaloniaFact]
