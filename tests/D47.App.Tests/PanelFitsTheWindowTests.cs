...
public class PanelFitsTheWindowTests
{
    ...
    /// <summary>
    /// And it wrapped rather than being clipped or truncated: a single line of this text at this size
    /// is far wider than the window, so fitting it means it is on several lines.
    /// </summary>
    [AvaloniaFact]
    public void TheTextIsOnSeveralLinesRatherThanOneThatFitsByBeingCutOff()
    {
        ...
        // Every line inside the window, which is the half that says it wrapped rather than simply being
        // clipped somewhere further down the tree.
        Assert.All(wrapped.TextLines, line => Assert.True(line.Width <= Width));
    }

    /// <summary>
    /// Zoomed in, the layout is narrower and drawn larger — a browser's text zoom rather than a picture
    /// scaled until it hangs off the side.
    /// </summary>
    [AvaloniaFact]
    ...
    private static PanelView LaidOut(string said, int zoom = 100)
    {
        ...
        // Twice: the first pass is what gives the scroll viewer a viewport, and the constraint that viewport
        // implies is only available to the second.
