...
public class LoadoutTabTests
{
    ...
    /// <summary>What the panel reads the game state through, so a test can move it after the page is
    /// open.</summary>
    ...
    private static CommanderGameState Flying(bool engineered = false)
    {
        ...
        {
            // A rolled module where the test asks for one, so the mark that says "this has been engineered"
            // has something to be drawn against.
    ...
    /// <summary>The ctrl-drag as it really arrives: pressed on one row, moved over another, and released on the row it was pressed on, because the pointer is captured there.</summary>
    ...
    /// <summary>A row by what it is called.</summary>
    ...
    /// <summary>
    /// The fleet is a root rather than a level, and it earns being landed on by answering where each
    /// ship is before anything is drilled.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// A hull the Commander does not own appears in the list and says it is not bought yet — acquiring
    /// it is the plan's first step rather than a precondition sitting outside it.
    /// </summary>
    [AvaloniaFact]
    ...
    /// the Commander wants rather than an edit to the truth.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// A ship the Commander is not flying reports no modules at all, and the page says so rather than
    /// showing a blank that implies disagreement.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// The ray points and the voice edits, so every page carries the phrase for what it is showing — a
    /// page that offers no phrase is one whose faster half is invisible.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// Promoting from the ship page proposes rather than writing, which is the trust boundary the whole
    /// phase turns on: the plan owns what, the checklist owns when.
    /// </summary>
    [AvaloniaFact]
    public void PromotingFromThePagePutsItOnTheChecklist()
    {
        ...
        // The button's own words are the contract.
    ...
    /// <summary> The module list is the outfitting screen's four blocks, in its order. </summary>
    [AvaloniaFact]
    ...
    public void AnEmptySlotIsStillASlot()
    {
        ...
        // And the empty ones say so, rather than carrying a blank that reads as d47 having nothing to say
        // about them.
 ...
 /// <summary>A module is named the way the outfitting screen names it.</summary>
    [AvaloniaFact]
    ...
    /// <summary>
 /// A ship page open while the journal moves under it follows along.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>
 /// The gear sits after the module name rather than in front of it.
    /// </summary>
    [AvaloniaFact]
    public void TheGearFollowsTheModuleNameAndTheDotDoesNot()
    {
        ...
        // The gear follows the *module's* name now, not the slot's — the row leads with the size and the
        // module, so the module is what the mark belongs to.
        ...
        // And it is not a glyph of its own anywhere on the row: the gear travels with the last word of the
        // name rather than sitting in a column.
    ...
    public void KeepingWhatIsFittedListsThatModulesRolls()
    {
        ...
        // The row names what is in the slot, which is the whole reason the question below has an answer to
        // fall back on.
    ...
    /// <summary>Anaconda, Utility Mount 5 to Utility Mount 6.</summary>
    [AvaloniaFact]
    ...
    public void KeepingWhatIsFittedStillAsksAboutTheExperimentalEffect()
    {
        ...
        // A pulse laser's own, from the shipped table.
        Assert.Contains("Thermal Shock", effects);
        Assert.Contains("Scramble Spectrum", effects);

        // And not one belonging to something else.
        Assert.DoesNotContain("Ammo Capacity", effects);

        surface.Window.Close();
    }

    /// <summary> Planning a slot offers the valid choices for it, and never asks for a spelling. </summary>
    [AvaloniaFact]
    public void PlanningASlotOffersWhatFitsIt()
    {
        ...
        // And nothing that goes somewhere else: a shield generator is an optional internal and a shield
        // booster is a utility mount.
        ...
        // Then which one of it, in the words a Commander uses rather than the code the outfitting screen
 // files it under.
        ...
        // **And it never claims the size or the mount is irrelevant**.
 ...
 // And then the experimental effects a pulse laser supports.
    ...
    public void TheGradeReadsLastAndTheStepperFollowsIt()
    {
        ...
        var said = Assert.IsAssignableFrom<TextBlock>(order[0]);
 ...
 /// <summary>The chooser narrows as the Commander types.</summary>
    [AvaloniaFact]
    ...
    /// <summary> A module the plan names can be kept while only the engineering changes. </summary>
    [AvaloniaFact]
    public void APlannedModuleCanBeKeptWhileTheRollChanges()
    {
        var surface = Open();

        // A medium hardpoint with a plan on it and nothing fitted — the state a drag-copy leaves, and the
        // state every slot of a ship being designed is in.
        var build = surface.Ships.BuildFor(12, "python", "Bad Idea");
    ...
    /// <summary> A module the ship already carries its limit of is not offered again. </summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// And the slot that holds it still offers it, which is the Commander's own condition: replacing
    /// the fuel scoop that is already there must still offer fuel scoops.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary> A planned experimental says what it does, as a planned blueprint already did. </summary>
    [AvaloniaFact]
    public void APlannedExperimentalSaysWhatItDoes()
    {
        ...
        // Named, because two sentences under one heading are two claims about the same slot and the Commander
    ...
    /// <summary>
    /// An experimental with no blueprint under it needs no name in front of it: the heading has already
    /// said what the sentence is about.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary> The experimental chooser says what each effect does. </summary>
    [AvaloniaFact]
    ...
    /// <summary> A searchable chooser is ready to be typed into. </summary>
    [AvaloniaFact]
    ...
    /// <summary>The row draws both names, not merely models them (the Commander's ruling, 2026-08-20).</summary>
    [AvaloniaFact]
    public void TheRowDrawsWhatIsFittedAndWhatIsPlanned()
    {
        ...
        // MainEngines holds a 5A Thrusters.
 ...
 /// <summary>A ship you are not flying says what it is and where it is.</summary>
    [AvaloniaFact]
    public void AnOwnedShipShowsItsDetails()
    {
        ...
        Assert.Contains("150,000,000 cr", shown);

        // And what the hull is, from the shipped table — true of every Anaconda ever built, which is why it
        // can be said for a ship in another dock. Figures are stat tiles, their labels in capitals.
        Assert.Contains("THE SHIP", shown);
        Assert.Contains("Anaconda, by Faulcon DeLacy", shown);
        Assert.Contains("large", shown);
        Assert.Contains("180 m/s", shown);
        Assert.Contains("BOOST", shown);
        Assert.Contains("240 m/s", shown);
    ...
    /// <summary>
    /// The system a ship is parked in can be taken away on the clipboard, to be pasted into Elite's
 /// Galaxy Map search.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// And the one you are flying adds the figures that depend on what is fitted, because those are the
 /// ones Elite actually reports.
    /// </summary>
    [AvaloniaFact]
    public void TheShipYouAreFlyingAddsWhatIsFitted()
    {
        ...
        Assert.Contains("34.2 ly", shown);
        Assert.Contains("128 t", shown);
        Assert.Contains("100,000,000 cr", shown);
        Assert.Contains("REBUY", shown);
        Assert.Contains("5,000,000 cr", shown);
        Assert.Contains("HULL INTEGRITY", shown);
        Assert.Contains("87%", shown);

        surface.Window.Close();
    }

    /// <summary> A row stays one line and trims, however much there is to say. </summary>
    [AvaloniaFact]
