...
/// <summary>The bar, the modes, the breadcrumb and the panes.</summary>
public class ThePanelIsAPlaceTests
{
    ...
    /// <summary>One tab for the three readings, and the tabs the host has not furnished are not drawn.</summary>
    [AvaloniaFact]
    public void TheBarCarriesOneTranscriptTabAndOnlyWhatWasFurnished()
    {
        ...
        // And there is no second tab strip: the readings are one drop-down inside the pane, not more tabs
 // beside it.
        Assert.Equal(3, PanelModes.Count(panel));
    }

    /// <summary>
    /// The mode control is visible at a root and hidden below one: a mode switch three levels into a
    /// ship is a question about which stack you are in, and the breadcrumb is already answering the one
    /// about where you are.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>A tab with one root shows no mode control at all, rather than a control with one entry.</summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// The breadcrumb is drawn below a root and not at one — at a root its only crumb would be the tab,
    /// which is already highlighted a row above.
    /// </summary>
    [AvaloniaFact]
    public void TheBreadcrumbAppearsOnlyBelowARoot()
    {
        ...
        // The whole trail, root first — including levels the Commander never walked, because voice jumps and
        // the breadcrumb has to be right the first time it is drawn.
        Assert.Equal(["Ships", "Corsair", "Weapon 3"], words);

        // Every crumb but the last is pressable; the last is where you are, and a control that does nothing
        // costs a press to learn that.
    ...
    /// <summary>
    /// A chooser replaces the panel until it is dismissed, which is what makes it a level rather than a
    /// pop-up — and while it is up, nothing navigates away.
    /// </summary>
    [AvaloniaFact]
    public void AChooserTakesThePanelAndHoldsIt()
    {
        ...
        // The content region gives way to it entirely: a page still visible behind a modal is a page a ray
        // can still reach.
        ...
        // The header carries what is being chosen for, which is the one thing a drop-down cannot do and the
        // whole reason taking the panel costs nothing.
    ...
    /// <summary>A layer chooser stays over the page instead.</summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// Back is the one method every route goes through, and it says whether there was anything to go
    /// back from — so a controller button at a root stays available to whatever else wants it.
    /// </summary>
    [AvaloniaFact]
 ...
 /// <summary>A tab press goes to that tab even with a question up.</summary>
    [AvaloniaFact]
    public void ATabPressAbandonsAQuestionRatherThanBeingRefused()
    {
        ...
        // By name rather than by content: a tab's content is a mark and a word since #266, so asking for the
        ...
        // And nothing was committed on the way out: abandoning a question and answering it are different
        // things.
