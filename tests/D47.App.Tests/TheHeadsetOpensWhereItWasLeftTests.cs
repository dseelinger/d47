...
/// <summary>The headset back on the tab and the reading it was left on, across launches — the counterpart to <see cref="EveryTabOpensWhereItWasLeftTests"/>, which covers the desktop window.</summary>
public sealed class TheHeadsetOpensWhereItWasLeftTests
{
    ...
    /// <summary>
    /// The window can be three levels into a ship's slots while the headset reads the conversation, so
    /// a store shared by both must not have one surface's tab overwrite the other's.
    /// </summary>
    [AvaloniaFact]
