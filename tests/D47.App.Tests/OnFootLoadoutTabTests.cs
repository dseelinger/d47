...
public class OnFootLoadoutTabTests
{
    ...
    /// <summary>
    /// A Commander on foot in a grade 3 Maverick carrying one weapon, rank 5 with Farseer, with a ship
    /// and some materials — enough for both ledgers to have something in them.
    /// </summary>
    ...
    /// <summary>
    /// Suits, then the item, then the slot — the same three levels of the same drill stack, with the
    /// grade first because a grade 1 item has no modification slots.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// Fitted and planned are two blocks and never one merged line, on foot as on a hull — and a mod
    /// slot says outright that its number is d47's own, because Elite reports modifications as a set
    /// with no positions in it.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// A surface handed no on-foot plans has the one root Phase 26 gave it, rather than two that answer
    /// nothing.
    /// </summary>
    [AvaloniaFact]
    public void ASurfaceWithNoOnFootHalfKeepsTheOneRoot()
    {
        ...
        // Suits and Gap are the on-foot half and are absent; Carrier is not gated on owning one, because "d47
