...
public class APlanIsNotDrawnAsThoughItWereFittedTests
{
    ...
    /// <summary>The marker means work left to do, not "a plan exists".</summary>
    [Fact]
    ...
    /// <summary>
    /// A plan that asks for a roll the module has not had is still outstanding, which is the case the
    /// marker exists for.
    /// </summary>
    [Fact]
 ...
 // ---- The blueprint, in the Commander's words ----------------------------

    /// <summary>One blueprint has two spellings: <c>Heavy Duty Hull Reinforcement</c> and Frontier's <c>HullReinforcement_HeavyDuty</c>.</summary>
    [Fact]
 ...
 // ---- A roll that disagrees with the plan --------------------------------

    /// <summary>The case a plan exists to catch.</summary>
    [Fact]
    public void ARollThatDisagreesWithThePlanIsReported()
    {
        ...
        // One row, two columns, and neither borrows from the other: the hull rolled System Focused and the
        // plan asks for Weapon Focused, and both are said.
        Assert.Equal("System Focused", row.Parts.Current.Blueprint);
 ...
 /// rather than repeating the words already in the first.
    ...
    /// <summary>
    /// And an unplanned roll has nothing to disagree with, so it is not reported as a disagreement —
    /// which would put a mark on every slot a Commander engineered without planning it first.
    /// </summary>
    [Fact]
 ...
 // ---- The experimental, in the Commander's words ------------------------

    /// <summary>The dot came back, by the third road.</summary>
    [Fact]
    ...
    /// <summary>
    /// And the other half, which is what makes the mark worth anything: an experimental the plan asks
    /// for and the roll has not got is still outstanding.
    /// </summary>
    [Fact]
    ...
    /// <summary>One ship whose power distributor carries a real roll, plus whatever plan the test wants.</summary>
        ...
        params SlotPlan[] plans)
    {
        ...
        var loadout =
            """{"timestamp":"2026-08-24T09:00:00Z","event":"Loadout","Ship":"type9","ShipID":53,"ShipName":"oxen","ShipIdent":"OX-1","HullValue":1,"ModulesValue":1,"Rebuy":1,"Modules":[{"Slot":"PowerDistributor","Item":"int_powerdistributor_size7_class5","On":true,"Priority":1,"Health":1.0,"Engineering":{"Engineer":"The Dweller","EngineerID":1,"BlueprintID":1,"BlueprintName":"BLUEPRINT","Level":99,"Quality":1.0,EXPERIMENTAL"Modifiers":[]}}]}"""
                ...
                // Elite writes the symbol here, never the name.
                .Replace(
                    "EXPERIMENTAL",
