...
/// <summary>Filtering the picker, and the two faults behind #146.</summary>
public class TypingMaleDoesNotListEveryWomanTests
{
    ...
    /// <summary>
    /// A picker on screen with its template applied, which the filter box and the facet both need: the
    ...
    /// <summary>
    /// The report, in one line: "'male' includes female voices too." Asserted with both spellings,
    /// since a fix that only handled the lower-case one would leave Edge broken.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>Word starts rather than whole words, asserted in both directions.</summary>
    [Theory]
    ...
    /// <summary>
    /// A capital after a lower-case letter starts a word too, which is what keeps the Edge catalogue
    /// searchable: most of it is named <c>en-US-AndrewMultilingualNeural</c>, and a rule without this
    /// would have silently stopped Multilingual finding anything.
    /// </summary>
    [Theory]
    ...
    /// <c>FEMALE</c> must not match male at its M.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Offered where the choices carry one and absent where they do not, the way the play glyph is
    /// absent on a microphone picker rather than present and inert.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>It opens on the option that hides nothing.</summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// Untagged voices are never silently dropped, which is the decision the issue asked to be taken
    ...
    /// <summary>
    /// Narrowing to a facet that excludes the highlighted row moves the highlight, rather than leaving
    /// a selection nobody can see that Enter would take anyway.
    /// </summary>
    [AvaloniaFact]
