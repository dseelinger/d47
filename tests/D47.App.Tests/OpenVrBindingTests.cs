...
/// <summary>The vendored binding and the one conversion that crosses into it.</summary>
public class OpenVrBindingTests
{
    /// <summary>The test that matters, and it rotates on all three axes deliberately.</summary>
    [Fact]
    ...
    public void ATransformSurvivesTheRoundTrip()
    {
        ...
        // Compared by where they send a point rather than component-wise: q and -q are the same rotation, and
        // a component comparison fails on a conversion that is correct.
        foreach (var point in Points())
        {
    ...
    /// <summary>
    /// An all-zero device slot converts to a perfectly valid-looking pose: <see
    /// cref="Quaternion.CreateFromRotationMatrix"/> picks the largest diagonal, finds the square root
    /// of one, and hands back a unit quaternion.
    /// </summary>
    [Fact]
    ...
    /// <summary>The interface versions the vendored file was generated against.</summary>
    [Fact]
    ...
    public void LookingForTheRuntimeIsSafeOnAMachineThatHasNone()
    {
        ...
        // A located DLL implies a located runtime; the reverse is not implied, because a runtime directory
        // can exist with the library missing.
    ...
    /// <summary>
    /// OpenVR's own convention: the matrix goes on the left and the point is a column, so a row of the
    /// matrix dots the point and the translation is the fourth column.
    /// </summary>
