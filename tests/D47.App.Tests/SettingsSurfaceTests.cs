...
/// <summary>
/// The settings surface, driven headlessly: the same window a Commander opens, built against the real
/// builtin registry, rendered by the real theme.
/// </summary>
public class SettingsSurfaceTests
{
    ...
    private static SettingsHost Open(SettingsService settings, ViewStateStore viewState, AppPaths paths)
    {
        ...
        // palette to follow, exactly as the app wires it.
    ...
    public void TheSettingsPageOpensWithEveryRegisteredSection()
    {
        ...
        // Rendering is the assertion that matters: measure, arrange and paint all ran over every generated
        // row without a control throwing.
    ...
    public void AChangeMadeInCoreIsReflectedWithoutRebuildingTheView()
    {
        ...
        // A change from any caller — here the keyword-router path — announces itself and the open panel
        // follows.
    ...
    public void EveryThemeRendersToACapture()
    {
        ...
        foreach (var theme in D47.Core.Interface.ThemeCatalog.All)
        {
            ...
            {
                // Depends on the Commander's own HUD matrix file; the fallback path is the plain Elite
                // palette already captured.
                continue;
            }
        ...
        // the egress panels — get looked at too.
    ...
    public void TheMainWindowRenders()
    {
        ...
        // No host: the window paints its chrome and says so in the transcript, which is all this capture is
        // for.
    ...
    /// <summary>The mark is wired, not merely committed.</summary>
    [AvaloniaFact]
...
public class DisclosureIsAHintTests
{
    ...
    /// <summary>
    /// The caption block the row's label sits in — which is what the hover is attached to, and what the
    /// row's own text has to be looked for inside.
    /// </summary>
