...
/// <summary>Choosing a speech model fetches it, on the row that was used to choose it.</summary>
public class ModelSelectionFollowsTheDiskTests
{
    /// <summary>
    /// The predicate the startup rule rests on: a named model that is not on disk is one still awaiting
    /// a download.
    /// </summary>
    [Fact]
    ...
    public void AFailedDownloadDoesNotBecomeASelection()
    {
        ...
        // Whatever was selected before the attempt — the shipped default on a fresh surface — rather than a
        // literal, because "where it was" is the property under test and the shipped default is free to move.
    ...
    public void TheRowSaysWhatWentWrongWhereItWasChosen()
    {
        ...
        // Searched from the surface: a row's message line is a sibling of its three-column grid rather than a
        // child of it, and what is being asserted is that it is said on the surface the choice was made on -
        // not which panel holds the text block.
        var said = host.View
    ...
    public void TheChoiceIsDisabledWhileItDownloadsAndEnabledAfter()
    {
        ...
        var (host, _) = Open((_, _) =>
        {
            // Read from inside the download, which is the only moment the answer means anything.
