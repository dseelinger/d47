...
public class AResistanceIsShownAsItsValueTests
{
    ...
    {
        // 15.849996 rounds to the +15.9% on the ship's own ring, which is the number a Commander is comparing
        // against.
        Assert.Contains("15.8% KineticResistance", Effects());
    }

    /// <summary>The case a proportion cannot express at all.</summary>
    [Fact]
    ...
    {
        // Mass 2.0 → 2.8 really is +40%, and that reading is the right one for it.
        Assert.Contains("+40% Mass", Effects());
    }

    /// <summary>The ordering is the other half of the defect.</summary>
    [Fact]
    public void TheHeadlineIsTheChangeThatActuallyMovedMost()
    {
        var effects = Effects();

        // +72% on the health addition is the largest real move here; the resistances are ~14.85 points and
        // mass is +40%.
