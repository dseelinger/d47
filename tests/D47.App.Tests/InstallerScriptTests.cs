...
public class InstallerScriptTests
{
    ...
    /// exactly — <c>{userprograms}</c> is the same folder — because that path is what the app's own
    /// "add a Start Menu entry?" offer tests before deciding to stay quiet.
    /// </summary>
    [Fact]
    public void TheInstallerWritesTheShortcutWhereTheAppLooksForIt()
    {
        // The script names the entry through a #define, so both halves are checked: that the macro still
        // holds the name the app looks for, and that the icon is written to the per-user Start Menu folder
        // using it.
    ...
    /// <summary>
    /// Whisper's loader probes <c>runtimes\win-x64</c> on disk beside the executable, so the natives
    /// have to be installed with their folder structure rather than flattened.
    /// </summary>
    [Fact]
    ...
    /// <summary>
 /// Per-user and unelevated is a stated property of the product, and the flag
    /// that grants it is one word in a file nothing else checks.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A fixed install directory is what keeps <c>data\</c> beside the executable across an upgrade.
    /// </summary>
    [Fact]
