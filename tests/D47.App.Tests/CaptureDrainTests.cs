...
/// <summary>The drain that runs on NAudio's capture thread for every buffer the microphone delivers.</summary>
public class CaptureDrainTests
{
    ...
    /// <summary>
    /// A BufferedWaveProvider with its default ReadFully, which is exactly what the microphone feeds
    /// the resampler.
    /// </summary>
    [Fact]
    ...
    /// <summary>With ReadFully set — the default, and what the microphone gets — a read from a provider that has never been given a single sample still returns a full buffer.</summary>
    [Fact]
    ...
    /// <summary>
    /// The budget converts between the device's rate and Whisper's. 48 kHz in, 16 kHz out, both mono
    /// float: a third as many bytes come out as went in.
    /// </summary>
    [Fact]
    ...
    /// <summary>The signal reaches the gate intact, rather than as a fragment followed by silence.</summary>
    [Theory]
    [InlineData(1)]
    [InlineData(2)]
    public void TheSignalArrivesWholeRatherThanPaddedWithSilence(int channels)
    {
        ...
        // Configured exactly as WasapiMicrophone configures it; ReadFully is the property under test, so this
        // mirrors the real one rather than sharing it.
    ...
    private static ListenGate Gate()
    {
        ...
        // Open, so writes accumulate into an utterance that can be counted rather than being absorbed by the
        // pre-roll ring.
        gate.KeyDown(DateTimeOffset.UnixEpoch);
        return gate;
    }
