...
/// <summary>
/// The headset panel surface: which mode it is in, which slot it reads, and — the one that matters —
/// that going mini in the headset does not take the desktop window with it.
/// </summary>
public class VrSurfaceTests
{
    ...
    public void ThePlacementRowsExistForBothSurfacesAndNoneOfThemIsProtected()
    {
        ...
        // Opacity is not in that list because it is not per-surface: one knob answers for both panels, which
        // is why "set the opacity" can no longer land on the one you cannot see.
 ...
 /// <summary>Captions gain a lock and nothing else.</summary>
    ...
    public void TheCaptionSurfaceGainsALockAndNothingElseFromThePlacementRows()
    {
        ...
        // Grouped with the other caption rows rather than with the panel's placement, because it is a caption
        // setting and not a placement one.
    ...
    public void TheHeadsetSettingsCardRendersToACapture()
    {
        ...
        // Inside the scroller, so this is the card's heading and not the nav item of the same name in the
        // sidebar — which sits at the top and would scroll nowhere.
        var card = scroller.GetVisualDescendants()
            .OfType<TextBlock>()
            .First(text => text.Text == "Headset");

        // Scrolled by offset rather than by BringIntoView, which does nothing before the surface has settled
        // and leaves the capture showing whatever was at the top.
    ...
    /// <summary>How far down the scrolled content a control sits.</summary>
