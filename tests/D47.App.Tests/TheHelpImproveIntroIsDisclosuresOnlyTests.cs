...
/// <summary>
/// The intro carries the disclosures and the <c>ⓘ</c> carries the reasoning
...
/// argument drifting back into the intro rebuilds the wall of text a press at a time.
/// </para>
/// </summary>
public sealed class TheHelpImproveIntroIsDisclosuresOnlyTests
{
    ...
    // The intro reads the destination and nothing else, so a send delegate would only add a button
    ...
    public void TheLedeStillCarriesEveryDisclosure()
    {
        ...
        var intro = Intro(window).Text!;

        Assert.Contains("Nothing is read, written or sent until you press", intro, StringComparison.Ordinal);
        Assert.Contains("no standing consent", intro, StringComparison.Ordinal);
        Assert.Contains("your name and IDs replaced", intro, StringComparison.Ordinal);
        Assert.Contains("other people's words", intro, StringComparison.Ordinal);
        Assert.Contains(Destination, intro, StringComparison.Ordinal);
        Assert.Contains("thirty days", intro, StringComparison.Ordinal);
        Assert.Contains("Privacy and egress", intro, StringComparison.Ordinal);
        Assert.Contains("data\\donations", intro, StringComparison.Ordinal);
        Assert.Contains("data\\donor-token.txt", intro, StringComparison.Ordinal);

        window.Close();
    }

    /// <summary>
    /// And none of the arguments for pressing. Each of these was a sentence in the intro before
    ...
    /// A disclosure moved here to shorten the intro would be invisible to the Commander who never
    ...
    /// <summary>
    /// With no address there is nothing to name a destination for, and the intro says so rather
