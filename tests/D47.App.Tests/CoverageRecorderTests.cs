...
public class CoverageRecorderTests
{
    ...
    /// <summary>
    /// What was exercised survives a restart, or the record would only ever cover one session — which
    /// is the whole difference between this and the session-scoped help ranking.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The outcome has to survive the whole path — registry, event, recorder, ledger — or the report
    /// shows a green line for something that threw every time it was tried.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A row set to a value it will not take was still driven by hand, and the validation saying no is
    /// the validation working — the same reading as a refused tool argument.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The model can never be evidence that a protected row has been driven — only the panel, a hotkey
    /// or the keyword router can be.
    /// </summary>
    [Fact]
