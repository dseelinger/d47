...
public class ACorpusIsSentByTheSameWriterThatSavesItTests : IDisposable
{
    ...
    /// <summary>What a fake corpus writer puts on the stream.</summary>
    ...
    /// <summary>The one writer both roads go through, standing in for <c>CorpusDonation.Write</c>.</summary>
    private static Func<Stream, IProgress<int>, CancellationToken, Task> Writer(string payload) =>
        async (stream, progress, cancel) =>
        {
            progress.Report(1);

            // No BOM, exactly as the real writer is constructed — a byte order mark in front of the first
            // event is a payload that fails at line one.
 ...
 // The history half of the merged window, which is one press in: one donation page for every
    ...
    /// <summary>
    /// The bytes on the wire, decompressed, are what the writer wrote — the writer being the same
    /// delegate the Save button hands a file stream.
    /// </summary>
    [Fact]
    ...
    /// <summary>The length is declared, and this is what settled the hashing question.</summary>
    [Fact]
    ...
    /// <summary>
    /// The same two claims against a payload that does not fit in one buffer, which is the only shape
    /// the real thing ever has.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// It goes under the corpus prefix, because the two retention rules are opposite and a prefix is
    /// what a lifecycle rule is written against.
    /// </summary>
    [Fact]
    ...
    /// <summary>The spool does not survive the send.</summary>
    [Fact]
    ...
    /// <summary>
    /// What is kept is the report, and the receipt says so rather than claiming the hash covers it.
    /// </summary>
    [Fact]
    ...
    /// <summary>Nothing is assembled with nowhere to send it.</summary>
    [Fact]
    ...
    /// <summary>
    /// The window offers the send only where one was composed — the same rule the excerpt window and
    /// the donate button itself already follow.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// And with nowhere to send, the window still says nothing reaches a network — which is then true,
    /// and is the sentence #181 had to stop saying unconditionally rather than delete.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// With an address, the intro says where a send goes and how long it is kept before anything is
    /// pressed.
    /// </summary>
    [AvaloniaFact]
    public void WithAnAddressItSaysWhereASendGoesBeforeAnythingIsPressed()
    {
        var window = Shown(
    ...
    /// <summary>
    /// What is sent is the report that was on screen, and pressing sends once — the same rules and the
    /// same words as the excerpt window's button.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>Changing the scope throws the send away with the report.</summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// A refusal says so and offers the press again, rather than looking like it worked — the one thing
    /// worse than a donation that did not arrive is one that silently did not.
    /// </summary>
    [AvaloniaFact]
