...
/// <summary>Where to buy everything a build still needs, drawn.</summary>
public class TheSourcingPageTests
{
    ...
    /// <summary>Onto the Sourcing root.</summary>
    ...
    /// <summary>
    /// The root is on the Checklist tab, beside the checklist itself — as a second reading rather than
    /// a second tab, which is the same arrangement Loadout's three roots and Routing's four already
    /// make.
    /// </summary>
    [AvaloniaFact]
    ...
    public void ASurfaceThatDoesNotFurnishItDoesNotGetIt()
    {
        ...
        // And the navigator declines to select a root nobody furnished, which is what stops a stale phrase
        // putting a surface on an empty pane with no way back.
    ...
    /// <summary>
    /// The button invokes the tool through the registry — the same path the model takes and the same
    /// path the keyword router takes — and the table is what the capability posted on its way out.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>The carrier is told here and nowhere else, and it comes off what gets searched for.</summary>
    [AvaloniaFact]
