...
public class DesktopWindowTests
{
    ...
    public void ZoomWrapsTheWindowContentInALayoutTransformRatherThanARenderTransform()
    {
        ...
        // Inside a horizontally scrolling viewport, which is what makes zoom browser-like: enlarging past the
        // window's width scrolls sideways rather than squeezing the layout into what is left.
        ...
        // A render transform would scale the finished picture and let the window clip it.
    ...
    public void TheFourGesturesStepTheLadderAndPersist()
    {
        ...
        // Persisted like the theme, not held in the window: the level has to survive a restart and be the
        // same one the settings surface shows.
        zoom.Set(150);
    ...
    /// <summary>
    /// The ladder is a closed vocabulary, so a level that is not on it is refused at the row like any
    /// other bad choice.
    /// </summary>
    [AvaloniaFact]
    ...
    public void ARememberedPlacementIsAppliedBeforeTheWindowIsShown()
    {
        ...
        // Not 820x640: the remembered size wins, and it was applied while the window was still unshown so
        // there is no visible resize for the Commander to see.
    ...
    /// <summary>
    /// Maximised has to survive too, and it is a state a Commander leaves a window in and then expects
    /// back.
    /// </summary>
    [AvaloniaFact]
    public void ClosingMaximisedRemembersMaximised()
    {
        ...
        // Whatever it opened at, which is a proportion of the screen rather than the numbers above: this test
        // is about what maximising does to the memory, not about the default.
        ...
        // And the normal-state size it had before is what it restores to, not the maximised rectangle —
        // otherwise maximising once leaves a window that cannot be un-maximised back to a size the Commander
        // chose.
        Assert.Equal(normal, (remembered.Width, remembered.Height));
    }

    /// <summary>And it comes back maximised, which is the half nothing asserted.</summary>
    [AvaloniaFact]
    ...
    /// <summary>The monitor a maximised window was on travels with the flag, so it comes back on that
    /// one.</summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// And it is put back there before it is maximised, because Windows maximises a window to whichever
    /// monitor it is already on.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>A monitor that has been unplugged since is not one to open on.</summary>
    [AvaloniaFact]
    ...
    /// <summary>Bigger text wants a bigger window to put it in, so the opening size follows the zoom.</summary>
    [AvaloniaFact]
    public void TheDefaultSizeFollowsTheZoomButARememberedOneDoesNot()
    {
        ...
        // And it is genuinely larger than the unzoomed opening size, or the assertion above would pass on a
        // clamp that had swallowed the zoom entirely.
    ...
    /// <summary>One capture per interesting level, for a human to look at.</summary>
    [AvaloniaFact]
