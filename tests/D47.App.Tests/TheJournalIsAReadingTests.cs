...
/// <summary>The journal as a reading in the Transcript (https://github.com/dseelinger/d47/issues/51).</summary>
public sealed class TheJournalIsAReadingTests
{
    ...
    /// <summary>
    /// A surface nobody furnished the raw reading on has only the sentences — which is now the only
 /// thing that gates it, since the headset is furnished too.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>A furnished surface has both, and the raw one is deliberately not in the picker.</summary>
    [AvaloniaFact]
    public void TheRawReadingIsARootButNotAnEntryInThePicker()
    {
        ...
        // The box rather than the switch: the label sits beside the knob and the two show and hide together,
        // so the box is what carries the visibility.
        Assert.True(
    ...
    /// <summary>The toggle crosses between the two, and says which one is showing.</summary>
    [AvaloniaFact]
    ...
    public void TheSurvivingReadingsKeepTheirKeysAndDetailsIsGone()
    {
        ...
        // And a Commander whose stored reading was Details lands on the conversation rather than on nothing
        // at all.
    ...
    /// <summary>
    /// The detail pane is the fields of the selected line — the half that cannot be wrong, which is why
    /// a list with no detail was rejected as "lossy with no way back to the fields".
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// And the detail pane can be put away, which is what keeps the reading usable in one narrow column
    /// — the Commander's own amendment to the design.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>The two readings are different readings.</summary>
    [AvaloniaFact]
    ...
    /// <summary>And it never goes through the markup parser.</summary>
    [AvaloniaFact]
