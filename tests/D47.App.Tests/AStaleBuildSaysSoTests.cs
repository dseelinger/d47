...
/// <summary>A single-file bundle left in <c>bin/Debug/…/win-x64/</c> shadows every later build: <c>dotnet build</c> rebuilds the DLLs and the bundle ignores them.</summary>
public class AStaleBuildSaysSoTests
{
    private static readonly DateTime Tonight = new(2026, 8, 23, 21, 42, 0, DateTimeKind.Utc);

    /// <summary>A bundle-sized exe with loose assemblies beside it.</summary>
    [Fact]
    ...
    /// <summary>
    /// The tell that was in plain view the whole morning, without the size: the exe dated 08-16 in a
    /// folder whose DLLs were minutes old.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A published single-file build is a bundle with no loose assembly beside it, which is the whole
    /// of what makes the pair above diagnostic.
    /// </summary>
    [Fact]
