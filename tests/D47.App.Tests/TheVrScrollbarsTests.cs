...
/// <summary> Scrollbars, aimed at with a controller. </summary>
public class TheVrScrollbarsTests
{
    ...
    /// <summary>
    /// And a ray that misses it by more than the bar's own width still finds it, which is the point:
    /// jitter means close enough has to be enough.
    /// </summary>
    [AvaloniaFact]
    ...
    /// <summary>Where along the bar the ray is is where in the document it goes.</summary>
    [AvaloniaFact]
    ...
    /// <summary>
    /// And it lights up when aimed at, because a control that gives no sign it has been found is one
    /// the Commander cannot tell they are pointing at.
    /// </summary>
    [AvaloniaFact]
    ...
    public void TheHeadsetSurfaceTakesHoldAndScrolls()
    {
        ...
        // Twenty pixels short of the bar, halfway down: aiming at it without hitting it, which is what a hand
        // at arm's length actually does.
    ...
    public void RestingAimDoesNotAskForARedraw()
    {
        ...
        // The ordinary case, and the one that made this flicker all the time: no ray on the panel at all,
        // asked and answered on every tick for as long as the session is up.
