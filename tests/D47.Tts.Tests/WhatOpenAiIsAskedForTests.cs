...
/// <summary>What d47 sends OpenAI, and what it does with what comes back.</summary>
public class WhatOpenAiIsAskedForTests
{
    ...
    {
        // Nothing has to be decoded on the way to the arbiter, which is the same trade the other paid
        // provider makes with pcm_24000.
    ...
    public async Task NoLanguageIsSentBecauseThereIsNothingToSend()
    {
        ...
        // Not an oversight and not a thing to add later: the field does not exist, and one sent anyway is
        // accepted with 200 and ignored, which is worse than a refusal because nothing can see it happen.
    ...
    public async Task TheThirteenAreOfferedWithNoKeyAtAll()
    {
        ...
        // And every one survives into the pool a re-voiced sender is drawn from. "multilingual" is a word
        // rather than a locale, which VoicePool is careful to tell apart — reading it as one once discarded
        // 472 of a 473-voice account.
        Assert.Equal(13, VoicePool.From(listed.Voices).Count);
    }

    /// <summary>
    /// No gender is claimed, so the sex-matching in <see cref="VoiceCast.ForSender"/> widens to the
    /// whole pool rather than narrowing on a guess.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The distinction the Check button is built on: one of these the Commander can fix from a row they
 /// can see, and the other is the network's problem.
    /// </summary>
    [Theory]
    ...
    public async Task ARefusalIsClassifiedIntoSomethingActionable(HttpStatusCode status, TtsFault fault)
    {
        ...
        // The service's own words rather than a status code translated, for the reason the ElevenLabs path
        // gives: a mapping worth writing would answer "it said 400".
    ...
    public async Task AVoiceItDoesNotHaveIsWorthForgettingRatherThanRetrying()
    {
        ...
        // Unlike ElevenLabs this cannot be a stale id from another account — the catalogue is fixed — so it
        // is a value typed or carried across, and forgetting it is still right.
