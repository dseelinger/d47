...
/// <summary>
/// A loopback HTTP server that replays one recorded response, so the streaming decoder can be driven
/// end to end without contacting Anthropic.
/// </summary>
internal sealed class RecordedEndpoint : IDisposable
{
    ...
    /// <summary>The responses to serve, in order.</summary>
    ...
    /// <summary>Replays a stream that stops part-way and does not finish until the test says so.</summary>
    /// <param name="release">
    /// Completed by the test once it has seen what the first part carried.
    /// </param>
    ...
    /// <summary>
    /// Replays a refusal and then a stream, which is the shape a demotion needs: the endpoint names a
    /// field it will not accept, and the retry without that field succeeds.
    /// </summary>
    ...
    /// <summary>One recorded SSE frame.</summary>
    ...
    private async Task ServeAsync(CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            // Taken before the response is served, so concurrent connections cannot both be handed the first
            // entry.
            ...
            _ = Task.Run(
                async () =>
                {
                    using (client)
                    {
                        ...
                        {
                        // The client hanging up mid-response is an ordinary end to a cancelled turn.
                        }
                    }
                },
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Headers first, then exactly as many body bytes as Content-Length promised.
        while (headerEnd < 0)
        {
        ...
        // Chunked when the body is withheld: Content-Length would have to state a total not yet decided.
