...
/// <summary>The two halves of the Commander's own account of themselves, and which calls carry which.</summary>
public class CommanderStoryTests
{
    ...
    /// <summary>A Commander who has written a story and no sheet still has a story.</summary>
    [Fact]
    ...
    /// <summary>Chosen by the index and nothing else.</summary>
    [Fact]
    ...
    /// <summary>
    /// Exactly one call in <see cref="CommanderStory.StoryEvery"/>, so "occasionally" is a number and
    /// the per-call cost is one that can be written down.
    /// </summary>
    [Fact]
    public void TheStoryGoesOneCallInEvery()
    {
        ...
        // And the first remark of a session is one of them — the one where the most is unknown about who is
        // flying.
        Assert.True(CommanderStory.TellsStory(0));
    }

    /// <summary>
    /// A line that was never one of a numbered set has no index to be chosen by, and the sheet is the
    /// default.
    /// </summary>
    [Fact]
