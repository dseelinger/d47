...
public class EffortRouterTests
{
    ...
    {
        // Same input, same effort, every time — which is what makes the heuristic testable rather than
        // something to observe in production.
    ...
    {
        // The checklist allows low through max but no "off".
...
public class KeywordRouterTests
{
    ...
    [Theory]
    // The one that got away: "where" matched, and the router answered a question about Iran with the
    // Commander's own star system, confidently.
    [InlineData("Where is Iran?")]
    [InlineData("where is Sol relative to the galactic core")]
    // "system" must not hijack anything containing the word.
    ...
    /// <summary>Single words that are specific enough to be safe on their own.</summary>
