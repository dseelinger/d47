...
/// <summary>The floor and the ceiling.</summary>
public class EffortRangeTests
{
    /// <summary>
    /// Decision 3 of the phase, asserted rather than assumed: a settings file written before the bounds
    ...
    /// <summary>
    /// <c>Math.Clamp</c> throws when the minimum exceeds the maximum, and a hand-edited settings file
    ...
    /// <summary>The rows and the clamp read one ladder.</summary>
    [Fact]
    ...
    /// <summary>A number is not a rung.</summary>
    [Theory]
    [InlineData(null)]
    [InlineData("")]
    [InlineData("   ")]
    ...
    /// <summary>
    /// Each row truncates against the other bound, so the picker cannot offer a floor above the ceiling
    /// in the first place.
    /// </summary>
    [Fact]
