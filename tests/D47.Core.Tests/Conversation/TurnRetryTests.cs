...
public class TurnRetryTests
{
    ...
    /// <summary>
    /// A configuration failure fails identically next time, so retrying it only spends the Commander's
    /// silence.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The asymmetry that shapes the whole design: streamed text has probably already been spoken, and
    /// there is no such thing as un-saying it.
    /// </summary>
    [Fact]
    ...
    public async Task RunningOutOfTriesIsSaidOutLoudRatherThanLeavingSilence()
    {
        ...
        // The text is what the speech pipeline speaks, so "said out loud" and "produced text" are the same
        // claim at this layer.
