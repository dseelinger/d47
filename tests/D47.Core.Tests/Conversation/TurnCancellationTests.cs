...
/// <summary>Cancelling is not silencing, and the difference is billed.</summary>
public class TurnCancellationTests
{
    ...
    /// <summary>
    /// Two turns talking over each other is not a state worth supporting, and letting the older one run
    /// is how a turn nobody is listening to keeps spending.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The identity check in End: a turn cancelled by a newer one must not release the newer one's slot
    /// as it unwinds, or "cancel" would find nothing to cancel a moment later.
    /// </summary>
    [Fact]
    ...
    public async Task ACancelledTurnStopsPullingFromTheProvider()
    {
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(async () =>
        {
            ...
            {
                // Cancel once it is genuinely under way, which is the real-world case: the Commander hears it
                // start and calls it off.
...
/// <summary>Streams forever.</summary>
