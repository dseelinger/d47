...
public class TurnLoopTests
{
        ...
        out SpendTracker spend)
    {
        ...
        // One attempt, so a provider call count in these tests means a turn rather than a turn times the
        // retry budget.
    ...
    public async Task ATransientOutageIsProbedAgainRatherThanLatchingOffForever()
    {
        ...
        // Turns, not seconds — no Core component reads the clock, so recovery is counted rather than timed.
        ...
        // The probe turn contacts the provider again.
        await RunAsync(loop, "tell me about hyperspace physics");
    ...
    /// <summary>
    /// Decision 3 of Phase 54, through the whole loop rather than at the clamp: a Commander who has set
    /// neither bound gets exactly what the router answered, on every shape of input.
    /// </summary>
    [Theory]
    ...
    /// <summary>
    /// The floor lifts a turn the router priced below it — and the request carries the lifted rung, not
    /// merely the report.
    /// </summary>
    [Fact]
    ...
    /// <summary>
 /// A model with no effort dial says so rather than reporting one it never applied.
    /// </summary>
    [Fact]
    ...
    public async Task HistoryAccumulatesOnlyAnsweredTurns()
    {
        ...
        // The model's own history is what it sees next turn; the request carries it plus the new user turn.
        await RunAsync(loop, "and what about witchspace");
        Assert.Equal(3, provider.LastRequest!.Prompt.History.Count);
    }

 /// <summary>What d47 says without being asked reaches the next turn.</summary>
    [Fact]
    public async Task WhatItSaidUnpromptedReachesTheNextTurn()
    {
        ...
        // Labelled as d47's own, because it is arriving inside a user message and the Commander did not say
        // it.
        ...
        // But it is still in the transcript, which is what makes the *next* question about it answerable
    ...
    /// <summary>
 /// The keyword router's own answers are recorded too. *"Stop calling
    /// things out"* → *"done"* → *"why did you do that?"* reproduces the reported transcript by a
    /// second road, and none of the four model-free routes wrote a word.
    /// </summary>
    [Fact]
 ...
 /// <summary>The transcript has a ceiling.</summary>
    [Fact]
