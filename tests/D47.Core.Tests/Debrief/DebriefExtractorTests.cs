...
/// <summary>The pass itself.</summary>
public class DebriefExtractorTests
{
    ...
    /// <summary>
    /// The address comes off the front before the cues are looked for, so leading with a name is the
    /// same instruction as leading with the verb — which is how people actually give one.
    /// </summary>
    [Theory]
    ...
    /// <summary>
    /// And what this installation answers to comes off too, so a renamed core's name is address rather
    /// than the first word of an instruction.
    /// </summary>
    [Fact]
    ...
    /// <summary>The poisoning defence, at the filter it lives in.</summary>
    [Fact]
    public void NothingFromOutsideTheCommanderIsEverExtractedFrom()
    {
        ...
        // And the same sentence from the Commander does draft one, which is what makes the assertion above
        // about the speaker rather than about the wording.
        Assert.Single(DebriefExtractor.Extract(
            [new DebriefLine(Now, DebriefSpeaker.Commander, hostile)], [], [], Now));
    }

    /// <summary>
    /// Everything the pass drafts is a proposal, and the tier that follows from that is an inference.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The clip anchors a proposal to the exact audio where the recorder was running, and is absent
    /// where it was not.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A direction already in the file is not drafted again, whatever state it is in — and the declined
    ...
    /// <summary>Implicit signals propose questions and never directions.</summary>
    [Fact]
    ...
    /// <summary>One occurrence is a Commander with something to say, not a habit.</summary>
    [Fact]
    ...
    /// <summary>
    /// A silenced warning gets a question and deliberately no suggestion: its answer is a threshold on
    /// a settings row, and the debrief writes one file that is not that row.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The question's wording carries no count, so answering it once is answering it: a question whose
    /// text moved with the tally would be re-proposed every session under a new spelling.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A review pane with forty things in it is one nobody works through, and the newest corrections
    /// are the ones still worth acting on.
    /// </summary>
    [Fact]
