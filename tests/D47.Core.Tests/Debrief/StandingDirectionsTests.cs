...
public class StandingDirectionsTests : IDisposable
{
    ...
    /// <summary>The whole gate, in one test.</summary>
    [Fact]
    ...
    /// <summary>
    /// The Commander gets the last word on their own sentence, and what they left in the editor is what
    /// enters the prompt.
    /// </summary>
    [Fact]
    ...
    /// <summary>A question with nothing typed cannot be adopted.</summary>
    [Fact]
    ...
    /// <summary>The cadence, made mechanical.</summary>
    [Fact]
    ...
    /// <summary>
    /// A per-core direction is a style overlay, and it rides in the persona block rather than in the
    /// general one — which is what keeps it out of the pack, where persona writing lives twice.
    /// </summary>
    [Fact]
    ...
    /// <summary>The block the pane shows is the block the prompt carries, character for character.</summary>
    [Fact]
    ...
    /// <summary>
    /// Position 6 is the last of the cached region and the guardrails are still position 2, which is
    /// the property that makes a direction about manner safe: it is read most recently, and it is four
    /// blocks below the thing it cannot loosen.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Bounded twice, for <see cref="MemoryRecall"/>'s reasons: a file that grows for a year cannot all
    /// reach the prompt, and the fix is not a bigger prompt.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The order is deterministic and total, so an unchanged file renders identical bytes — which is
    /// what stops the block invalidating a cached prefix it did not change.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Declining keeps the refusal, so the pass does not redraft it from the same sentence next session
    /// — and forgetting removes it outright, for the Commander who changed their mind.
    /// </summary>
    [Fact]
    ...
    /// <summary>A hand-written entry with no state is a proposal: an unlabelled fact about a person is their own word, and an unlabelled instruction going into a prompt is something nobody has agreed to yet.</summary>
    [Fact]
