...
public class MemoryCanBeErasedOnDemandTests
{
    ...
    /// <summary>
    /// And it empties the store — every Commander in the file, because a Commander pressing this means
    /// "forget me" rather than "forget the character I happen to be logged in as".
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// It survives a restart, which is the half that matters: an erase that emptied the copy in memory
    /// and left the file alone would look identical until the next launch.
    /// </summary>
    [Fact]
    ...
    /// <summary>The protection, stated as an assertion.</summary>
    [Fact]
    ...
    /// <summary>
    /// And it is not in the model's vocabulary at all, which is the difference between a request that
    /// is refused and one that is never made.
    /// </summary>
    [Fact]
    ...
    /// <summary>No spoken route, deliberately. "Forget everything about me" is a sentence a transcriber can produce out of a misheard one, and this is the only action here that cannot be undone.</summary>
    [Fact]
