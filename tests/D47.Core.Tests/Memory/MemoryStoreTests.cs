...
public class MemoryStoreTests : IDisposable
{
    ...
    public void AFactTypedOnThePanelSurvivesARestartAndIsStillTheCommandersWord()
    {
        ...
        // A second store over the same file is a restart.
        var reopened = Store();
        reopened.Poll();
    ...
    /// parameter: a model that could file its own entry as the Commander's word is a model a hostile
    /// in-game message could tell to.
    /// </summary>
    [Fact]
    ...
    public void NothingPromotesAnInferenceToTheCommandersWord()
    {
        ...
        // Said again, by the same route, a week later.
    ...
    public void TwoCommandersDoNotSeeEachOthersFacts()
    {
        ...
        // And the whole file is still readable as one thing, because that is what the panel shows and what
        // emptying covers.
    ...
    /// <summary>
    /// A hand-written line is the likeliest thing to have no stamp on it, and deleting the Commander's
    /// own words because they did not type a date is the worst reading of an ambiguous file.
    /// </summary>
    [Fact]
    ...
    public void EmptyingCoversEveryCommanderInTheFile()
    {
        ...
        // "Forget me" is not "forget the character I happen to be logged in as".
    ...
    {
        // Core owns no randomness and reads no clock, so a key has to be a function of what is already there
        // — the same rule authored checklist lines and ship build ids follow.
        var book = Book();
