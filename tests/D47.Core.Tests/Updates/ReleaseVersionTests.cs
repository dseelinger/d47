...
public class ReleaseVersionTests
{
    ...
    {
        // AssemblyInformationalVersion gets "+<git-sha>" appended by the SDK by default whenever the project
        // sits in a git repository - the running app's own version arrives in this shape, not as a bare tag.
        Assert.True(ReleaseVersion.TryParse(tag, out var version));
        Assert.Equal(new ReleaseVersion(major, minor, patch), version);
    }
