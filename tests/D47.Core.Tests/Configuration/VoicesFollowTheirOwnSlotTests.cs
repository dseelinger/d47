...
/// <summary>A stored voice moves with the slot that owns it, not with the ship.</summary>
public class VoicesFollowTheirOwnSlotTests
{
    ...
    public void ACarrierPinnedToEdgeKeepsItsVoicesWhenTheShipMovesAway()
    {
        ...
        // The ship's are Edge ids and go; the carrier is still speaking through Edge, so its two are still
        // the right ids for the provider that will say them.
    ...
    public void TheCarrierMovingOnItsOwnLeavesTheShipAlone()
    {
        ...
        // Untouched, including the flag: the pairing pass has still run against Edge's list, and clearing it
        // here would re-pick every core because the carrier changed provider.
    ...
    /// <summary>Both slots filing into one provider's entry, one at a time.</summary>
    [Fact]
    ...
    public void AndEachComesBackOnItsOwn()
    {
        ...
        // The ship is still on ElevenLabs, so its Edge voice stays filed and is not restored early —
        // restoring it here would put an Edge id in front of ElevenLabs.
    ...
    /// <summary>The stash holds what is owed and nothing else.</summary>
    [Fact]
    ...
    /// <summary>A file from before the carrier's own record existed.</summary>
    [Fact]
