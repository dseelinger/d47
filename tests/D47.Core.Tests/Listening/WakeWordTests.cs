...
/// <summary>
/// Wake-word gating, which decides about words rather than about audio — so every one of
/// these is a transcript in and a decision out, with no model and no microphone.
/// </summary>
public class WakeWordTests
{
    ...
    public void SomethingSaidToSomebodyElseIsDropped()
    {
        ...
        // The common case in this mode by a wide margin, and the reason the mode is bearable at all: a room
        // where people talk is not a room full of commands.
        Assert.Equal(
            WakeOutcome.Ignored,
    ...
    public void TheNameIsTakenOffWhatIsPassedOn()
    {
        ...
        // The turn should receive the request, not the vocative. "D47, how much fuel" reaching the router is
        // a phrase that matches nothing it declares.
        Assert.Equal(WakeOutcome.Addressed, decision.Outcome);
        Assert.Equal("how much fuel do I have?", decision.Text);
    }

    /// <summary>The reason this matches a flattened form rather than the string.</summary>
    [Theory]
    ...
    public void TheNameHasToBeNearTheFrontRatherThanAnywhereAtAll()
    {
        ...
        // Talking about d47 is not talking to it.
        Assert.Equal(
            WakeOutcome.Ignored,
    ...
    public void TheNameOnItsOwnWakesRatherThanAsks()
    {
        ...
        // Passing an empty request to the turn loop would produce an answer to nothing.
    ...
    public void WhatFollowsBeingCalledByNameNeedsNoSecondName()
    {
        ...
        // How a person addresses anybody: the name, an acknowledgement, then the thing.
    ...
    public void TheWindowIsOneFollowUpRatherThanAnOpenMicrophone()
    {
        ...
        // The follow-up shuts the window.
        Assert.Equal(
            WakeOutcome.Ignored,
    ...
    public void WithNoNameConfiguredEverythingIsAddressed()
    {
        ...
        // Which is what continuous listening without a wake word means.
    ...
    public void RenamingTheShipsAiRenamesTheWakeWord()
    {
        ...
        // The name follows the core, so the wake word has to as well — a Commander who switched core and
        // found the new one deaf to its own name would have no way to work out why.
        gate.Phrases = ["Athena"];
    ...
    {
        // A settings row cleared to whitespace must not turn into a phrase that matches the start of every
