...
/// <summary>Telling "the Commander said nothing" apart from "the device sent nothing".</summary>
public class SilentDeviceTests
{
    ...
    /// <summary>
    /// Room tone through a live microphone is very quiet and still nowhere near the floor: a threshold
    /// that called this silent would blame the hardware every time someone spoke softly, which is worse
    /// than the fault it is trying to name.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// One live sample in an otherwise dead buffer still means the device is delivering, so this is
    /// deliberately peak rather than an average — an average would call a genuine utterance with long
    /// pauses in it silent.
    /// </summary>
    [Fact]
