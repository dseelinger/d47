...
public class TheRecordingHasACapTests : IDisposable
{
    ...
    /// <summary>A tiny cap, so eviction is asserted rather than assumed.</summary>
    private const long SmallCap = 200_000;

    private RecordingLog Log(long cap = SmallCap) => new(_folder, NullLogger.Instance, cap);

    /// <summary>
    /// One clip, sized so that a handful of them crosses the cap without the test writing two hundred
    /// megabytes.
    /// </summary>
    ...
    /// <summary>
    /// Two rows landing in the same millisecond in opposite directions do not fight over one file name.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The half a recording cannot supply is written down beside it, in the corpus its kind belongs to.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A mispronunciation is the other corpus, and it carries the phonemes — which is the column that
    /// turns it from an anecdote into a diagnosis, and the reason the pair is a test case.
    /// </summary>
    [Fact]
    ...
    {
        // The real cap here rather than the small one, because what this asserts is that the stated number is
        // the one the Commander is shown.
