...
/// <summary>What the model can reach, and what only a person can.</summary>
public class LogbookCapabilityTests
{
    /// <summary>The assertion that keeps item 4 true.</summary>
    [Fact]
    ...
    /// <summary>
    /// Both acts have to be reachable without a model in the path, which the closed router grammar only
    /// allows for phrases carrying no extracted argument at all.
    /// </summary>
    [Fact]
    public void BothActsHaveArgumentFreePhrases()
    {
        ...
        // Quoting and spending are different sentences.
    ...
    /// <summary>
    /// A phrase cannot carry a date, so the values it can carry are exactly the spans the settings row
    /// offers — and the tool's enum has to agree, or a declared phrase writes a value the handler will
    /// not recognise.
    /// </summary>
    [Fact]
    ...
    /// <summary>Item 3: "the shipped default is the plain one".</summary>
    [Fact]
    ...
    /// <summary>
    /// The voice row is protected for the same reason the memory rows are: a log written in a voice the
    /// Commander did not choose is d47 having spoken at length as somebody else.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// With nothing composed, every tool says there is nothing set up rather than throwing — the same
    /// treatment every other optional service gets.
    /// </summary>
    [Fact]
