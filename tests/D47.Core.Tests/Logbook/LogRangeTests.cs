...
public class LogRangeTests : IDisposable
{
    ...
    /// <summary>
    /// A session that began at 23:50 and ran past midnight lives entirely inside a file named for the
    /// previous day.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Two date pickers can produce them the wrong way round, and a window with a negative length reads
    /// back as "nothing happened" rather than as an obvious mistake.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A misspelt setting must not silently buy thirty days of prose, so an id nobody recognises falls
    /// back to the cheapest window rather than the widest.
    /// </summary>
    [Fact]
