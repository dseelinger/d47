...
public class LogDigestBuilderTests : IDisposable
{
    ...
    public void NothingAnybodyTypedInGameReachesTheDigest()
    {
        ...
        // And the two are excluded by name rather than by having no handler that happens to fire, so a later
        // phase adding one is a deliberate act.
    ...
    public void ManyJumpsAreOneFactCitingAllOfThem()
    {
        ...
        // Capped as they arrive: the file records what the fact drew on, it does not reproduce the journal it
        // drew it from.
    ...
    public void FactsAreNumberedInTheOrderTheDigestRendersThem()
    {
        ...
        // Moments before totals, and the numbers count down the page — a Commander reading the Sources
        // section of a finished log should not have to hunt.
    ...
    /// <summary>
    /// A death matters more than a jump count however many jumps there were, which is what the per-kind
    /// cap is for: four hundred courier missions must not crowd out the one that mattered.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A log of the last two hours of a five-hour session is still that Commander's, and the event that
    /// named them is at the top of the file, before the window opened.
    /// </summary>
    [Fact]
