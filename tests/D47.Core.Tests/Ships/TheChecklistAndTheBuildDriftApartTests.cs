...
public class TheChecklistAndTheBuildDriftApartTests
{
    ...
    public void BoardingAShipWhoseBuildTheChecklistHasNotGotAsks()
    {
        ...
        // **It asks and never acts.** The question is a proposal, on the same accept-or-decline boundary the
        // model writes across, so nothing is on the list yet.
        Assert.Single(bench.Checklists.Proposals.Pending);
        Assert.Empty(bench.Checklists.Document.Items);
    }

    /// <summary>The live half of the Phase 44 defect, on this watch.</summary>
    [Fact]
    ...
    public void ItFiresOnASwapRatherThanOnEveryLoadout()
    {
        ...
        // A Loadout arrives on every re-outfit and every dock.
        Assert.Null(bench.Drift.Observe([Boarding(12)]));
    }
    ...
    public void SayingYesRevisesRatherThanRebuilds()
    {
        ...
        // **The evening's work.** The Commander puts the list in the order they mean to work it, which is the
        // state a rebuild costs and a revision keeps — nothing promoted from a plan ticks by hand, so
        // ordering is what there is to lose.
        ...
        // What was already there is where they put it, and the new lines are added after rather than the
        // whole list being written again in the plan's own order.
