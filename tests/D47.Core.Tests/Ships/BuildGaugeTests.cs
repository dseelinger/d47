...
/// <summary>The two gauges at the head of a ship's slot list.</summary>
public class BuildGaugeTests
{
    ...
    public void ThePowerGaugeWeighsTheDrawAgainstThePlant()
    {
        ...
        // The plant's own roll moved its output down: 15.6 MW stock, 13.26 after a grade 5 Low Emissions.
        Assert.Equal(13.26, power.Capacity!.Value, 3);

        // Nothing on this ship is a hardpoint, so the two totals are the same — and saying so is the test
        // that the split is about what is fitted rather than about arithmetic.
    ...
    {
        // **The distinction hardest to keep in your head by hand.** A shield booster sits in a TinyHardpoint
        // slot, so it looks like a hardpoint; its module type is `usb`, a utility, and it draws whether or
        // not the guns are out.
        var loadout = Flown(
            """
    ...
    public void PlanningARollMovesTheGaugeAndMarksItModelled()
    {
        ...
        // Modelled off the module's stock output rather than off the roll already on it: a plan replaces the
        // engineering rather than stacking on top of it. 15.6 MW at +12%.
    ...
    public void APlanNamingNoSizeIsCountedRatherThanIgnored()
    {
        ...
        // And the slot still weighs what is fitted, so the gauge is a complete total of a build that exists
        // rather than a partial total of one that does not.
    ...
    public void ElitesOwnPerModuleFigureWinsOverTheTable()
    {
        ...
        // ModulesInfo.json, for the ship being flown.
    ...
    public void APlannedSlotIgnoresTheMeasuredFileBecauseItDescribesWhatIsThereNow()
    {
        ...
        // The file says what is on the hull; a plan is by definition not that yet, so the modelled figure is
        // what the gauge shows for that slot.
    ...
    public void AFileDescribingAnotherShipIsNotUsed()
    {
        ...
        // A Commander who swaps ships without re-outfitting leaves the previous ship's figures on disk, and
        // using those would put one hull's draw under another hull's plant.
    ...
    {
        // A 2E plant under a 4A thruster, drive, distributor and a Plasma Accelerator: over budget once the
        // guns come out, which is the case the split exists to catch.
        var loadout = Flown(
            """
    ...
    public void ABuildWithNoPlantSaysThatRatherThanZero()
    {
        ...
        // Not fitting is a comparison, and there is nothing to compare against — so it does not claim the
        // build fails.
        Assert.True(power.Fits);
        Assert.Null(power.Overage);
    }
