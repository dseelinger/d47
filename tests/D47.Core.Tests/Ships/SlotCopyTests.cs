...
/// <summary>Dragging a plan from one slot to another.</summary>
public class SlotCopyTests
{
    ...
    {
        // Medium Hardpoint 3 carries a plan for a 2F Pulse Laser turreted, grade 5 Long Range, Flow Control.
    ...
    {
        // The decision the item reserved does not exist: `Blueprints.tsv` carries no size, class or rating,
        // so a blueprint belongs to a module *name* and downsizing preserves the name.
    ...
    {
        // 20 of 60 hardpoint names do not come in size 1.
    ...
    {
        // **Ten module names have an interior hole in their size run.** Six limpet controllers are odd sizes
        // only, so `min(slotSize, moduleMax)` would resolve a size 7 Collector Limpet Controller onto a size
        // 4 compartment as size 4, which does not exist.
    ...
    {
        // "A pulse laser, I do not mind which" is a real plan and is met by whatever the target takes, so
