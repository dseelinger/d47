...
/// <summary>What a package's own metadata claims about its licence, and where that claim came from.</summary>
/// <param name="Verdict">Permissive, Copyleft or Unknown.</param>
...
internal sealed class ProjectPackages
{
    ...
    /// <summary>
    /// Lower-cased ids of everything this project asks for by name — packages and project references.
    /// </summary>
    private IReadOnlyList<string> Roots { get; }

    /// <summary>Lower-cased id to the resolved package.</summary>
    ...
    /// <summary>Where a package's contents are on disk, or null if the folder is not there.</summary>
    ...
    public string PathTo(PackageId package)
    {
        ...
        // Reachable in the resolved graph but not from anything this project names.
    ...
    /// <summary>Reads one project's assets file.</summary>
    public static ProjectPackages Read(string name, string assetsPath)
    {
        ...
        foreach (var target in Property(root, "targets").EnumerateObject())
        {
            foreach (var entry in target.Value.EnumerateObject())
            {
                ...
                {
                    // A referenced project is a root as well as a node, so a package that arrives behind one
                    // still has a readable path rather than an unexplained appearance.
                    roots.Add(key);
                }
