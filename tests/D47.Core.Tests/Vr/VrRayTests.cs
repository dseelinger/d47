...
/// <summary>Where a controller ray meets the panel, and where the beam and cursor go as a result.</summary>
public class VrRayTests
{
    ...
    /// <summary>V reads from the top, matching raster order rather than OpenVR's Y-up.</summary>
    [Fact]
    ...
    /// <summary>
    /// U runs left to right across the face as the Commander sees it, which is the other half of the
    /// same mirror.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The curved model is the flat one's limit rather than a separate case, so the two have to agree
    ...
    /// <summary>The beam lies along the aim and turns its face to the viewer.</summary>
    [Fact]
    public void TheBeamLiesAlongTheAimAndFacesTheHead()
    {
        ...
        // The face can only be the part of "towards the head" that is square to the beam — the long axis is
        // already spoken for — so the property is that it is perpendicular to the beam and leans the head's
        // way, not that it points straight at them.
    ...
    /// <summary>The beam stops exactly where the cursor sits, because one distance feeds both.</summary>
    [Fact]
    ...
    /// <summary>A head directly along the beam leaves no perpendicular to roll towards.</summary>
    [Fact]
