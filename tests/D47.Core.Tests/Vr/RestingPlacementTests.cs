...
/// <summary>Where a world-locked panel goes the first time it is shown, if nobody has ever put it
/// anywhere.</summary>
public class RestingPlacementTests
{
    ...
    /// <summary>
    /// The requirement names the panel's top edge, and the top edge is the one thing the settings
    /// cannot set: width is configurable and height follows from the texture's aspect.
    /// </summary>
    [Theory]
    ...
    public void TheTopEdgeLandsWhereItWasAskedFor(float width, float quadHeight)
    {
        ...
        // And the width is what set the height, so a wider panel sits lower to keep its top in the same
        // place.
        Assert.True(width > 0);
    }

    /// <summary>
    /// In front of where the Commander is facing, at the distance asked for — measured on the floor
    /// plane, so looking down does not bring the panel closer.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A Commander looking at their feet when the panel first appears meant "in front of me", not
    /// "below where I was already looking".
    /// </summary>
    [Fact]
    ...
    /// <summary>A quad near the floor that is not tilted up is a line.</summary>
    [Fact]
    public void ItIsTiltedBackTowardsTheEyesAboveIt()
    {
        ...
        // The face an overlay actually shows, which looks along its own +Z: tipped back, it has climbed away
        // from horizontal towards the eyes.
    ...
    /// <summary>Straight up leaves no compass direction at all.</summary>
    [Fact]
    ...
    /// <summary>
    /// Knee height is derived from the floor and the eyes rather than assumed, and clamped so a dropped
    /// tracking frame cannot anchor the panel underground or at head height — a bad first placement is
    /// written to view state and outlives the frame that caused it.
    /// </summary>
    [Theory]
