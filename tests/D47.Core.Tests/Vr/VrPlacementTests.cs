...
/// <summary>The placement arithmetic, with no headset and no runtime.</summary>
public class VrPlacementTests
{
    /// <summary>Rotated on all three axes and standing somewhere that is not the origin.</summary>
    ...
    public void AHeadLockedSurfaceIsInFrontOfTheHeadRatherThanInFrontOfTheWorld()
    {
        ...
        // Forward is the head's own -Z, which is OpenVR's convention and not ours.
    ...
    {
        // A Commander lying on their side wants the panel below their eyes, not below the world.
    ...
    /// <summary>The tilt is worked out from the geometry, not taken from a setting.</summary>
    [Theory]
    ...
    /// <summary>
    /// The derived tilt actually points the quad's face at the eye, rather than merely being a number
    /// of the right size.
    /// </summary>
    [Theory]
    ...
    public void AHeadLockedSurfaceAimsItsFaceAtTheEye(float distance, float drop)
    {
        foreach (var head in new[] { VrPose.Origin, Awkward })
        {
            ...
            // The visible face of an overlay quad looks along its own +Z, which is the opposite of the -Z
            // every tracked device points along.
    ...
    /// <summary>The same question asked of the world-locked resting placement: with the sign inverted, a panel dropped to knee height tilts its face at the floor, through twice the angle it should have gone the other way.</summary>
    [Theory]
    ...
    /// <summary>
    /// The trim is added to the derived angle rather than replacing it, so a Commander who nudges it
    /// moves the panel by that much from facing them — not to that much from square.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Captions opt out, and are the only thing that does: they sit 0.45 m below the eye at 1.6 m, so
    /// deriving would tilt them 15.7° when they are meant to be square to the view.
    /// </summary>
    [Fact]
    ...
    /// <summary>A grab is rigid.</summary>
    [Fact]
    ...
    /// <summary>
    /// A quaternion out of a tracking runtime drifts off unit length over a session, and one that is
    /// one per cent long scales everything it touches — so the panel creeps away from the Commander the
    /// longer they play.
    /// </summary>
    [Fact]
    ...
    public void AWorldLockedSurfaceThatHasNeverBeenPutDownFallsBackToWhereTheHeadIs()
    {
        ...
        // Otherwise the first thing the Commander sees is a panel at the tracking origin, which is usually
        // behind them.
        var placed = never.Where(Awkward);

        // The distance is along the head's forward; the surface also sits below eye level, so the
        // straight-line distance is longer than the setting and should be.
        var forward = Vector3.Normalize(Vector3.Transform(-Vector3.UnitZ, Awkward.Facing));
        Assert.Equal(
