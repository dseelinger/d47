...
public sealed class HandledEventsGateTests
{
    ...
    private enum Comparand
    {
        ...
        /// <summary>Text decided at runtime — a search box, another event's kind.</summary>
        Runtime,
    ...
    /// <summary>The compile and the walk.</summary>
    private sealed class Survey
    {
        ...
        /// <summary>
        /// Every property, field or method whose value is a journal event's kind: the original, and
        /// every copy the walk finds it stored into.
        /// </summary>
        ...
        public static Survey Run()
        {
            ...
            // Everything the test process itself loaded, minus d47's own assemblies — those are the source
            // being compiled, and a type defined both in source and in a reference binds to the source with a
            // warning, which is noise rather than a problem.
            ...
            // Each pass walks the whole source for the symbols found since the last one.
        ...
        private void Walk(SyntaxTree tree, HashSet<ISymbol> batch, HashSet<string> names)
        {
            ...
            foreach (var name in tree.GetRoot().DescendantNodes().OfType<IdentifierNameSyntax>())
            {
                ...
                if (symbol is null)
                {
                    // A `.Kind` the compiler could not place, compared against text: it might be an event's,
                    // and a gate that guessed "probably not" would be grep again.
        ...
        {
            // A write — `with { Kind = … }`, an object initializer's left side — is where a copy is made, and
            // the right side is what gets followed. nameof() reads the property's name rather than the
            // event's.
        ...
        private void OnTheKind(MemberAccessExpressionSyntax access, SemanticModel model, ExpressionSyntax kind)
        {
            ...
            // A prefix or substring test against a constant is a dispatch on the kind's shape, which no list
            // of names can be checked against.
        ...
        /// <summary>The kind is a constructor argument.</summary>
        private void Stored(
        ...
        /// <summary>A place the kind is copied to.</summary>
        ...
        /// <summary>
        /// The key of one initializer element: <c>["Docked"] = …</c>, <c>{ "Docked", … }</c>, or the
        /// element itself.
        /// </summary>
