...
public class AdventureTellingTests : IDisposable
{
    ...
    /// <summary>A beat firing starts the wait, and the line arriving ends it.</summary>
    [Fact]
    ...
    /// <summary>A beat dropped rather than spoken — mid-interdiction — stops the wait too.</summary>
    [Fact]
    ...
    /// <summary>
    /// A story begun again is being told again, so the feed of the last run does not ride along.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Ten of them, short, and the pool wraps — the whole design is that they cost no model call and
    /// almost no synthesis, because they exist to arrive before anything else does.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The acknowledgement's key must not sit under the beat's prefix, or <c>FlavourBriefs</c> sends it
    /// through the model round trip it exists to arrive ahead of.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The heuristic the Commander chose: the story's name, a beat's title, or a place a beat waits at.
    /// </summary>
    [Theory]
    ...
    /// <summary>Whole words only, and nothing shorter than four letters.</summary>
    [Theory]
