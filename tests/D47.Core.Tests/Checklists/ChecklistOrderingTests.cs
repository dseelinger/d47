...
public class ChecklistOrderingTests : IDisposable
{
    ...
    /// <summary>A project the Commander has never ranked sorts below every one they have: ranking one project means "above everything I have not spoken for", or ranking means nothing on a list this size.</summary>
    [Fact]
    public void AProjectNeverRankedFallsBelowEveryRankedOne()
    {
        ...
        // A new project for the very ship being flown — the strongest here-and-now signal there is — still
        // lands below the two the Commander has an order for.
    ...
    /// <summary>
    /// The rank is the stored half and it has to survive a restart — "authored the way item text is
    /// authored" means the same file, the same store, the same round trip.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The spoken route: a phrase names a project by the word the list shows for it, and two possible
    /// answers refuse rather than guess — acting on the wrong list is worse than asking.
    /// </summary>
    [Fact]
