...
/// <summary>
/// The store follows <see cref="D47.Core.Actions.MacroStore"/> deliberately: a file under
...
public class ChecklistStoreTests
{
    ...
    public void ABadLineIsReportedAndTheRestOfTheFileStillLoads()
    {
        ...
        // One typo must not cost somebody their other forty items, and the one that was refused has to be
        // sayable rather than silently gone.
 ...
 // One way of saying it, and the shorter one.
    ...
    public void ADocumentThatArrivedFromOutsideIsFoldedWithoutAnnouncingIt()
    {
        ...
        // The roll was finished before this document arrived.
        Assert.Empty(checklists.Drain());
        Assert.Equal(ChecklistState.Done, checklists.List.For("F1").Items.Single().State);

        // And the silence is for the arrival, not for the session: the next thing the world does is still
        // news.
        Apply(
            """
            { "timestamp":"2026-08-16T11:00:00Z", "event":"Loadout", "Ship":"krait_mkii", "ShipID":12,
    ...
    public void TheFilterRowIsAProjectionRatherThanAList()
    {
        ...
        // Everything actually on the list, and nothing that is not.
        Assert.Contains("authored", filters);

 // The Commander's word, not the enum's.
        ...
        // **No Ship row, ever** (the Commander's ruling, 2026-08-20).
        checklists.AddNote(ChecklistScope.Ship(12), "swap the shield booster");

        Assert.DoesNotContain("ship", checklists.Filters());

        // And every row says which question it answers, rather than the four axes arriving as one flat list
        // of enum spellings.
