...
/// <summary>Planning a suit or a weapon on the Phase 17 substrate.</summary>
public class OnFootPlanTests
{
    ...
    public void TheGradeComesFirstBecauseAGradeOneItemHasNoSlots()
    {
        ...
        // Order is the deliverable here, not presentation: an engineer's base has no Pioneer Supplies, so a
        // modification attempted first is a wasted trip rather than a wasted turn.
    ...
    /// <summary>
    /// The whole reason the scope is keyed on <c>SuitID</c> rather than on the symbol: a Maverick taken
    /// from grade 3 to grade 5 is the same suit, and a plan that lost its list at the moment the
    /// upgrade landed would abandon itself on success.
    /// </summary>
    [Fact]
    ...
    /// <summary>Every step, not just the last one.</summary>
    [Fact]
    ...
    /// <summary>Held comes from the backpack and the locker, never from the ship's material store.</summary>
    [Fact]
    ...
    /// <summary>A symbol nothing joins is not evidence of absence.</summary>
    [Fact]
    ...
    /// <summary>
    /// A plan about a suit the Commander is not in says nothing rather than saying "not done" — the
    /// stored verdict then stands as of when it was taken.
    /// </summary>
    [Fact]
