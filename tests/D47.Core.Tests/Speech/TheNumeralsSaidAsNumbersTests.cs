...
/// <summary>Roman numerals said as numbers where the context says one is meant, and left alone everywhere
/// else.</summary>
public class TheNumeralsSaidAsNumbersTests
{
    ...
    /// <summary>Both spellings, and they must agree.</summary>
    [Theory]
    ...
    /// <summary>
    /// The two spellings produce the same sound, which is the assertion that matters — the text above
    /// is an implementation detail and the phonemes are what a Commander hears.
    /// </summary>
    [Fact]
    ...
    /// <summary>And the sound is a mark and a number rather than four letters.</summary>
    [Fact]
    public void TheLocalVoiceNoLongerSpellsAMarkNumber()
    {
        var said = Rules.ToPhonemes("Cobra MkIII");

        // "em kay" and "eye eye eye" are the spelled-out readings.
    ...
    /// <summary>It is a cardinal, not an ordinal: Mark Three, never Mark the Third.</summary>
    [Fact]
    ...
    /// <summary>Sudarsky classes, which are said on scans — the other place d47 talks constantly.</summary>
    [Theory]
    ...
    /// <summary>And "class" alone is not the context, deliberately.</summary>
    [Theory]
    ...
    /// <summary>The failure that would be found in a headset rather than in a test.</summary>
    [Theory]
    [InlineData("I")]
    [InlineData("MIX")]
    [InlineData("DID")]
    ...
    public void AWordMadeOfNumeralLettersIsNotConvertedInProse(string word)
    {
        ...
        // And with a context word in the same line but not in front of it, which is the shape a whole-line
        // rule would get wrong.
        Assert.Equal($"{word} Mk", SpokenNumerals.Expand($"{word} Mk"));
    }

    /// <summary>
    /// Only three of the eight above are even well-formed numerals, and the round trip is what says so:
    /// parsing <c>MILD</c> gives 1449, and 1449 renders as <c>MCDXLIX</c>, which is not what arrived.
    /// </summary>
    [Theory]
    [InlineData("I", 1)]
    [InlineData("II", 2)]
    ...
    /// <summary>
    /// Bare <c>I</c> after the written-out <c>Mark</c> is excluded, and only there. "Mark I saw him" is
    /// an English sentence and "Mark one saw him" is what converting it would produce.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The 74 table entries asserted as a set, so a new hull with a new spelling fails the suite rather
    /// than being discovered by ear.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// And the same for the gas giants, which are the other family and come from a different catalogue.
    /// </summary>
    [Fact]
