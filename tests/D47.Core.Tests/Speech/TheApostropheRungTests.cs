...
/// <summary>Words with an apostrophe in them.</summary>
public class TheApostropheRungTests
{
    /// <summary>
    /// A dictionary holding the handful of stems these cases need, with the shipped one's shape: no
    /// apostrophes anywhere in it, because the real one has none.
    /// </summary>
    ...
    /// <summary>The reported line.</summary>
    [Fact]
    ...
    /// <summary>
    /// The curly apostrophe a language model writes is the same word as the straight one a Commander
    /// types.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Which of the three sounds <c>'s</c> makes depends on what it lands on, and getting it wrong is
    /// audible: Commander's with a voiceless s is a hiss.
    /// </summary>
    [Theory]
    ...
    /// <summary>
    /// <c>n't</c> is a syllable of its own after a consonant and not after a vowel or an r, which is
    /// the difference between ˈɪzənt and ɑːɹnt.
    /// </summary>
    [Theory]
    ...
    /// <summary>
    /// The irregular ones are a table because no rule reaches them: don't is not do with an ending on
    /// it, and the dictionary's <c>wont</c> and <c>ill</c> are real words that are not the ones
    /// written.
    /// </summary>
    [Theory]
    ...
    /// <summary>
    /// <c>can't</c> is the one contraction that changes across the Atlantic, and a British voice saying
    /// the American one is the small wrongness heard every time — the same argument that bought zed its
    /// table entry.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// And the rung reaches invented names too, which is the point of deriving rather than listing: no
    /// dictionary is ever going to hold Buzhang's.
    /// </summary>
    [Fact]
