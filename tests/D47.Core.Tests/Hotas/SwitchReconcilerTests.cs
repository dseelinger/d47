...
/// <summary>A switch position meaning a state rather than a press.</summary>
public class SwitchReconcilerTests
{
    ...
    {
        // Startup, and after a reconnect. d47 learns where the switch is sitting rather than acting on
        // finding it there — otherwise every start would flip something.
        var reconciler = New();
    ...
    {
        // The switch says down and the game raises the gear by itself.
        var reconciler = New();
        var switches = new[] { GearSwitch() };
    ...
    {
        // A gear switch is not disagreeing with anything while the Commander is on foot.
        var reconciler = New();
    ...
    {
        // The device list fills in over seconds rather than at startup, so for the first moments of every run
        // every switch is missing its device.
        var reconciler = New();
    ...
    {
        // Unplug and replug is the same shape as startup: the switch is wherever it is, and finding it there
        // is not the Commander asking for anything.
        var reconciler = New();
        var switches = new[] { GearSwitch() };
    ...
    public void APausedSwitchStopsReconcilingUntilItIsResumed()
    {
        ...
        // Resuming presses nothing by itself — the switch is wherever it is, and the next flip is the next
        // question.
    ...
    {
        // Hardpoints retract by themselves on entering supercruise.
        var reconciler = New();
    ...
    public void AChangeBackLongAfterTheReconcileIsNotBlamedOnIt()
    {
        ...
        // Half a minute later the Commander raises the gear themselves.
    ...
    {
        // The centre-holds-nothing switch the spike could not rule out.
        var reconciler = New();
 ...
 // ----- Switches that reach d47 itself -----
    ...
    /// <summary>A tick with the panel in it.</summary>
    ...
    {
        // Null is unknown, never unchanged: the flip goes out and each surface declines for itself if it is
        // already there.
        var reconciler = New();
        var switches = new[] { TranscriptSwitch() };
    ...
    {
        // The vocabulary is whatever the surfaces registered, so a hand-edited key is checked here, where
        // they are known, and refused the way every other refusal is: by name.
        var reconciler = New();
