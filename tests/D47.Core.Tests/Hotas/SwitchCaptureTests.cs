...
/// <summary>Walking a switch.</summary>
public class SwitchCaptureTests
{
    ...
    /// <summary>A walk, driven the way the window drives it: a reading, then a wait, then a reading.</summary>
    private sealed class Walk
    {
        ...
        /// <summary>A reading taken now, with no time passing.</summary>
    ...
    {
        // Sixteen buttons were held at rest on the bench with nothing being touched, so "some button is down"
        // is not a signal — only a button that changes is (finding 4).
        var walk = new Walk()
    ...
    {
        // The exact walk the spike recorded: 4 -> 5 -> 6 -> 5 -> 4.
        var walk = new Walk()
            .Rest(Held(Stick, 4))
            .Rest(Held(Stick, 5))
            .Rest(Held(Stick, 6))
    ...
    {
        // Every maintained switch on the bench held exactly one button at all times including at centre — and
        // nothing in RawGameController requires that.
        var walk = new Walk()
            .Rest(Held(Stick, 4))
            .Rest(Held(Stick))
            .Rest(Held(Stick, 6));
    ...
    {
        // Hold duration cannot separate this from a switch flip — the measured ranges overlap outright
        // (finding 5).
        var walk = new Walk()
            .Rest(Held(Stick, 4))
    ...
    {
        // The other rushed walk, and it is a different problem with a different fix.
        var walk = new Walk()
            .Rest(Held(Stick, 4))
            .Move(Held(Stick, 5))
            .Move(Held(Stick, 6))
    ...
    {
        // Recorded rather than defended against, because it cannot be told apart: a push button held down and
        // never released *is* a control sitting in a position.
        var result = new Walk()
            .Rest(Held(Stick, 4))
            .Move(Held(Stick, 4, 20))
    ...
    {
        // The throttle presents four interfaces sharing one VID and PID (finding 2), so the device cannot be
        // picked by anything except what just moved.
        var walk = new Walk()
        ...
        // The grip moved first, so the grip is the device — and the throttle moving afterwards is somebody's
        // hand on a different control, not a position of this switch.
    ...
    {
        // Most of the devices this has to work on are ones nobody here will ever hold, so a declined walk has
        // to be diagnosable from a file the Commander sends.
        var walk = new Walk()
            .Rest(Held(Stick, 4))
            .Move(Held(Stick, 4, 20))
            .Move(Held(Stick, 4));
    ...
    {
        // The whole classification rests on the sample landing after the switch has had time to go home.
        var walk = new Walk()
            .Rest(Held(Stick, 4))
            .Move(Held(Stick, 5))
