...
/// <summary>
/// The same recorder, for the components that ask for <see cref="ILogger{TCategoryName}"/> rather than
/// the bare interface — <c>SettingsStore</c> among them.
/// </summary>
public sealed class RecordingLogger<T> : RecordingLogger, ILogger<T>;

/// <summary>
/// The composition the app performs at startup, in a throwaway folder: both stores, a settings service,
/// the builtin capability set, and the row table bound to it in that order.
/// </summary>
public sealed class TestSurface
{
    ...
    /// <summary>
    /// The same instance the registry was built with, so a test can put a proposal in front of a router
    /// that reads the state of one.
    /// </summary>
        ...
        bool timersAndAlarms = true)
    {
        ...
        // Loaded, not defaulted: a second surface over the same folder is a restart, which is how tests
        // assert that a change outlived the process that made it.
        ...
        var registry = CapabilityRegistry.Build(BuiltinCapabilities.All(
            ...
            // A real store over a real (empty) file, like the two above and for a reason of its own: the
            // documentation gate reads this registry, and a persona capability built without one registers
            // without its two protected ship-core tools — so the gate would report a fully documented
 // capability while the shipped app carried two tools it had never seen.
        ...
        // The same binding the composition root performs, for the same reason: a log level change has to
        // become live wherever it came from.
    ...
    /// <summary>The speech capability's surface with no sound card behind it.</summary>
 ...
 /// <summary>The fleet surface, every member supplied and none of them doing anything.</summary>
    ...
    /// <summary>
    /// The listening surface with no microphone behind it: no devices, not capturing, no transcriber,
    /// no binds.
    /// </summary>
...
/// <summary>A clock that does not pass.</summary>
