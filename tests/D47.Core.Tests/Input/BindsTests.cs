...
public class BindsTests : IDisposable
{
    ...
    public void TheActivePresetIsReadFromStartPresetRatherThanAssumedToBeCustom()
    {
        ...
        // Reading Custom.*.binds unconditionally parses a file the Commander is not using and then
        // confidently reports the wrong keys.
    ...
    {
        // Both, which is a real machine: Elite left StartPreset.start behind when it began writing
        // StartPreset.4.start, so an install that predates that change has the pair.
        ...
        // And the higher version is the one that answers, not whichever the file system listed first.
    ...
    public void AShippedPresetIsFoundInTheGameDirectory()
    {
        ...
        // Only searching Options/Bindings finds nothing for a Commander who never customised their controls —
    ...
    public void TheStartPresetVersionAndTheBindsVersionAreNotTheSameNumber()
    {
        ...
        // Custom.4.2.binds alongside StartPreset.4.start is a real machine's layout.
        Assert.True(binds.IsKnown);
    ...
    public void ModifiersAreOrderIndependent()
    {
        ...
        // Elite writes modifiers in binding order. "Ctrl+Alt+X" and "Alt+Ctrl+X" are the same key to the
        // Commander pressing it.
    ...
    public void AMalformedBindingsFileIsAStateNotACrash()
    {
        ...
        // An unreadable bindings file is a capability being unavailable, not a reason for d47 to fail to
        // start.
        Assert.Empty(binds.Bindings);
    }
    ...
    public void AKeyEliteIsAlreadyUsingIsReportedWithEveryActionUsingIt()
    {
        ...
        // The whole item: a double-bound push-to-talk key has no symptom other than not working, in one
    ...
    public void SharedGesturesAreListedWithTheActionsThatShareThem()
    {
        ...
        // Elite binds the same key across control modes on purpose, so this reports what is shared and leaves
        // the judgement to the caller rather than calling it a fault.
