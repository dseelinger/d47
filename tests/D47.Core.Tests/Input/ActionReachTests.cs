...
/// <summary>What d47 will admit to being able to do, given the Commander's own bindings and the mode they are
/// in.</summary>
public class ActionReachTests
{
    ...
    {
        // GalaxyMapOpen ships unbound in KeyboardMouseOnly, so this is the default experience rather than an
        // edge case.
        var reach = ActionReachability.Resolve(
            Action("galaxy_map"),
            Binds(("LandingGearToggle", "Keyboard", "Key_L")),
    ...
    {
        // The HOTAS case the checklist is about: the stick holds Primary and the keyboard holds Secondary.
    ...
    {
        // PrimaryFire is Mouse_1 on the preset this machine runs, and it is the honk's only route in.
        var reach = ActionReachability.Resolve(
            Action("primary_fire"),
    ...
    {
        // A Commander who has not started the game gets an empty profile rather than the whole catalogue
        // advertised against a mode d47 is guessing at.
    ...
    {
        // Standing on the pad next to your own ship: Elite keeps InMainShip set.
    ...
    {
        // Both spellings appear in the presets Elite ships.
