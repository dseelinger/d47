...
/// <summary>The ordering rules for turning a binding into presses.</summary>
public class InputSequenceTests
{
    ...
    {
        // Elite samples input per frame, so a press and release inside one frame is a press it never sees.
    ...
    {
        // The scancode for Delete and for the numpad's decimal point are the same number; this bit is the
        // whole difference.
    ...
    {
        // A sequence cut off after the key went down: the release has to let go of the key and both
        // modifiers, newest first.
