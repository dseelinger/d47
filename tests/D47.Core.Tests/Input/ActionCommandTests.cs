...
/// <summary>
/// Acting on the game end to end, with nothing sent: the declared phrase, the mode check, the state
/// check, and the refusals.
/// </summary>
public class ActionCommandTests
{
    ...
    {
        // Elite binds one toggle, so pressing it here would raise the gear — the opposite of what was asked,
    ...
    {
        // Elite reports FlightAssistOff, so the flag being set means the feature is off and "flight assist
        // off" is already satisfied.
        var fixture = Build(
    ...
    {
        // Whole-utterance matching. "Can you put the gear down while docked" is a question, and treating it
        // as a command flies the ship.
        var fixture = Build(Binds(("LandingGearToggle", "Key_L")), Flying());
    ...
    {
        // A phrase naming an action that is not in that tool's vocabulary would fail validation at the moment
        // the Commander says it, which is the worst time to find out.
    ...
    {
        // The whole reason availability lives in the game-state block: a schema that changed with the mode
        // would invalidate the entire cached prefix several times a minute.
