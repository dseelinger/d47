...
/// <summary>The first member of the autonomous-action category.</summary>
public class HonkOnArrivalTests
{
    ...
    /// <summary>Where a jump actually puts the Commander down: supercruise, in analysis mode.</summary>
    private static readonly StatusFlags Arrived =
        StatusFlags.InMainShip | StatusFlags.Supercruise | StatusFlags.AnalysisMode;

    /// <summary>Dropped out of supercruise before the arm was spent.</summary>
    ...
    /// <summary>Both halves of flying count.</summary>
    [Fact]
    ...
    {
        // A Commander who has already turned and burned does not want their guns cycling ninety seconds
        // later.
        var honk = Honk();
        honk.Examine(Context(Status(StatusFlags.InMainShip | StatusFlags.FsdJump), events: Jump()));
    ...
    {
        // Not an optimisation: an action that is off must not accumulate state either, so switching it on
        // mid-session starts it from now.
    ...
    {
        // The category's rule, asserted rather than trusted: a new member arriving switched on is consent for
        // one thing being spent on another.
