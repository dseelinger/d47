...
public class BindsAreReReadWhenEliteRewritesThemTests : IDisposable
{
    ...
    /// <summary>
    /// Written with a distinct write time, because the whole mechanism is a comparison of write stamps
    /// and a test that rewrites a file inside one filesystem tick is testing nothing.
    /// </summary>
    ...
    /// <summary>The startup read is unchanged.</summary>
    [Fact]
    ...
    /// <summary>A tick where nothing moved re-reads nothing.</summary>
    [Fact]
    ...
    /// <summary>Two files, not one.</summary>
    [Fact]
    ...
    /// <summary>And having noticed a preset switch, it goes on watching the file it landed on.</summary>
    [Fact]
    ...
    /// <summary>
    /// A Commander with no Elite install is a supported state, not a failure — and one that must not
    /// spend a parse on every tick forever.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Elite writing its bindings for the first time — a fresh install, or a Commander who has never
    /// opened the controls menu — is noticed without a restart.
    /// </summary>
    [Fact]
