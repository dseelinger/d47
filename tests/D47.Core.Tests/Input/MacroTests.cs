...
public class MacroTests : IDisposable
{
    ...
    {
        // Otherwise the Commander has two things answering to "gear down" and no way to tell which one ran.
        Write("""
    ...
    {
        // A macro that pressed three keys and then found the fourth unreachable would leave the ship
        // half-configured in a state the Commander did not ask for.
        Write("""
        { "macros": [ { "name": "prep", "steps": [
    ...
    {
        // The reason the router takes these from a function: descriptors are registered once and never
        // mutated, and the Commander authors these at runtime.
        var store = Store();
        store.Poll([]);
    ...
    {
        // Macro names change whenever the file is edited.
        var store = Store();
        store.Poll([]);
