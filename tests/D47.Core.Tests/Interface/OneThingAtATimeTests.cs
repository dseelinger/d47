...
/// <summary>Choosing a sibling replaces the one you were on.</summary>
public class OneThingAtATimeTests
{
    ...
    /// <summary>And it takes whatever was open underneath it with it.</summary>
    [Fact]
    ...
    /// <summary>A deeper level still nests, because a slot of a ship really is inside that ship.</summary>
    [Fact]
    ...
    /// <summary>And one slot replaces another, which is the same rule one level down.</summary>
    [Fact]
    ...
    /// <summary>A crumb that names no level nests as it always did.</summary>
    [Fact]
    ...
    /// <summary>Pressing the ship you are already on is still a no-op rather than a rebuild.</summary>
    [Fact]
    ...
    /// <summary>
    /// Back still walks out one level at a time, so replacing a sibling has not turned the trail into
    /// something with no way out of it.
    /// </summary>
    [Fact]
