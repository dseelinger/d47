...
{
    /// <summary>
    /// The case the checklist names: 820x640 device-independent pixels is 1230x960 real ones at 150%,
    /// which is taller than a 1366x768 laptop screen outright.
    /// </summary>
    [Fact]
    public void TheDefaultSizeIsClampedOnAScaledLaptopScreen()
    {
        ...
        // 820 is within a hair of the width limit too, which is the point: the default was sized for a screen
        // this one only resembles at 100%.
        Assert.True(width <= 1366 / 1.5 * WindowFit.Fraction);
    }

    /// <summary>
    /// 960 real pixels of a 1032-pixel work area is 93% — it fits, and it is still the thing being
    /// complained about.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The failure this prevents is silent and looks exactly like the app not starting: a window
    /// restored onto a monitor that has since been unplugged is a window nobody can reach with the
    /// mouse.
    /// </summary>
    [Fact]
    ...
    /// <summary>The opening size is a proportion of the screen rather than a length written down once.</summary>
    [Theory]
    ...
    /// <summary>And never so small that the panel stops making sense.</summary>
    [Fact]
    ...
    /// <summary>The minimum never wins over the screen.</summary>
    [Theory]
    [InlineData(480, 320)]
    [InlineData(800, 480)]
    ...
    /// <summary>
    /// A screen that will not say how big it is gets something rather than nothing, for the same reason
    /// <see cref="WindowFit.Clamp"/> leaves a size alone in that case.
    /// </summary>
    [Fact]
