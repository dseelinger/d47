...
/// <summary>The correction loop behind "say it, or type it".</summary>
public class TextEntryLoopTests
{
    ...
    public void AValueThatDoesNotResolveBringsTheKeyboardBackWithTheReason()
    {
        ...
        // And the complaint is what the keyboard opens under, so its reappearance reads as an answer rather
        // than as the microphone having failed silently.
        Assert.Equal(
            "There is no system by that name.",
            TextEntryLoop.Explain(fallback.Value, verdict?.Complaint));
    }

    /// <summary>Confident, valid, and therefore taken.</summary>
    [Fact]
    ...
    /// <summary>A partial is neither an answer nor a failure.</summary>
    [Fact]
    ...
    /// <summary>A caller with nothing to check accepts anything confident.</summary>
    [Fact]
    ...
    public void TheExplanationsAreSentencesRatherThanCodes()
    {
        ...
        // And the third falls back to a sentence rather than to nothing when a caller refused without saying
        // why.
    ...
    /// <summary>
    /// The readback is the only check left for the failure that cannot be detected, so it has to carry
    /// both what was heard and what it is for.
    /// </summary>
    [Fact]
