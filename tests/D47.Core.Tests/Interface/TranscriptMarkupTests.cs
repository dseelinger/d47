...
/// <summary>Reading the markdown a model writes, so the transcript stops drawing the markers.</summary>
public class TranscriptMarkupTests
{
    /// <summary>The line from the screenshot that started this: a bullet with a bold head.</summary>
    [Fact]
    ...
    /// <summary>
    /// Flags rather than one value, so nesting composes rather than the inner mark replacing the outer
    /// one.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The streaming case, and the reason an opener is not allowed to swallow the rest of the line: a
    /// reply arrives a delta at a time, so every bold stretch in it is unterminated for a moment.
    /// </summary>
    [Fact]
    ...
    /// <summary>Inside backticks an asterisk is an asterisk, and so is a fence's whole content.</summary>
    [Fact]
    ...
    /// <summary>Arithmetic, identifiers and the panel's own marks are not emphasis.</summary>
    [Theory]
    ...
    /// <summary>
    /// Links and tables are out of the subset on purpose: a URL a reader cannot see is worse than one
    /// they can, and a table drawn as runs in a wrapping block is not a table.
    /// </summary>
    [Fact]
    ...
    /// <summary>The panel's own note.</summary>
    [Fact]
    ...
    /// <summary>
    /// Every character that is not a marker survives, which is the property that matters most: this
    /// runs over every reply the Commander will ever read, and a pass that can lose a figure is worse
    /// than one that draws asterisks.
    /// </summary>
    [Fact]
