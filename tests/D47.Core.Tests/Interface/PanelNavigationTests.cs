...
/// <summary>
/// Where the Commander is, and every way of changing it.
/// </summary>
public class PanelNavigationTests
{
    ...
    /// <summary>
    /// The tab is the root rather than the first level, so the breadcrumb's first crumb is the tab and
    ...
    /// <summary>
    /// A tab with more than one root keeps a stack per root: leaving Ships halfway down a slot and
    /// coming back arrives where it was left, and going to Suits meanwhile disturbs nothing.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Pressing an already-selected tab returns to its root — which is what the surface has to add by
    /// hand, because a checked RadioButton pressed again announces nothing.
    /// </summary>
    [Fact]
    public void ReturningToTheRootIsAMoveAndSaysSo()
    {
        ...
        // And at the root it is a no-op the caller can tell from a move, so a host can leave the gesture to
        // whatever else wants it.
        Assert.False(nav.ToRoot());
    }

    /// <summary>
    /// Voice jumps levels, so a trail has to materialise mid-drill with a valid chain behind it rather
    /// than only being built by walking.
    /// </summary>
    [Fact]
    public void ATrailCanBeMaterialisedRatherThanWalked()
    {
        ...
        // Root first, and every crumb between, so the breadcrumb is right the first time it is drawn rather
        // than after the Commander has walked back up it.
        Assert.Equal(["Ships", "Corsair", "Weapon 3"], nav.Trail.Select(crumb => crumb.Word));
    ...
    /// <summary>And by word, because each crumb is a word that can be said as well as pressed.</summary>
    [Fact]
    ...
    /// <summary>
    /// A chooser takes the panel until it is dismissed, and while it does no route navigates away —
    /// which is the whole of "drawn as a level, behaving as a modal".
    /// </summary>
    [Fact]
    public void AChooserHoldsThePanel()
    {
        ...
        // A chooser opened from a chooser is legitimate — pick the engineer, then the grade — and is the one
        // thing that may push onto a modal.
    ...
    /// <summary>Pressing the tab you are on escapes a chooser.</summary>
    [Fact]
    ...
    /// <summary>
 /// The destination vocabulary is the roots, in bar order, and nothing else — derived
    /// from what was registered rather than kept as a second list.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Showing a root arrives on its tab and its mode, and showing the one already showing is refused —
    /// which is the are you already there a switch asks, answered exactly.
    /// </summary>
    [Fact]
 ...
 /// <summary>Help is declared on a crumb and inherited downwards.</summary>
    [Fact]
    ...
    /// <summary>
    /// Help itself declares none, so asking while it is open answers with the page underneath rather
    /// than with help about help.
    /// </summary>
    [Fact]
 ...
 /// <summary>Help can be asked for out loud.</summary>
    [Fact]
    ...
    /// <summary>
    /// Bare "help" is safe here and refused by the keyword router, and the difference is the matching
    /// rule rather than an inconsistency: this matches a whole utterance, so a sentence that merely
    /// contains the word cannot hijack the panel mid-flight.
    /// </summary>
    [Fact]
    ...
    /// <summary>A level whose page has no band opens the index instead of answering nothing.</summary>
    [Fact]
