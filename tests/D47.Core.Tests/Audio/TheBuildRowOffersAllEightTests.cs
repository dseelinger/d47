...
{
    /// <summary>
    /// The App's shape, and the reason the surface reads through delegates at all: a host that arrives
    /// after the rows do (see <see cref="TheLocalVoiceRowOffersItsButtonTests"/>, which is the fault
    ...
    /// <summary>
    /// The one already on disk is marked rather than hidden — a Commander comparing builds needs to
    /// know which choice costs a download and which is already paid for.
    /// </summary>
    [Fact]
    ...
    /// <summary>What is on disk outranks what the settings file says.</summary>
    [Fact]
    ...
    /// <summary>
    /// A surface that cannot answer at all keeps the row and cannot apply it, which is the designer's
    /// case — the row must exist either way, or a fault in it is invisible to the whole suite ("absent
    /// rows hide from every test").
    /// </summary>
    [Fact]
    ...
    /// <summary>The choice is the go-ahead and the fetch is what stands between it and the setting.</summary>
    [Fact]
    ...
    public void TheWriteRecordsTheBuildRatherThanWhateverWasTyped()
    {
        ...
        // A name this version does not publish resolves rather than being stored, so the file can never come
        // back naming something that will not load.
        Assert.Equal(
