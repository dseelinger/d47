...
public class AProviderThatCannotHoldALanguageStaysOutOfCommsTests
{
    ...
    {
        // Edge sends xml:lang in its SSML and ElevenLabs sends a language_code, which is why the rule below
        // costs them nothing and why false is the exception rather than the default.
 ...
 /// <summary>And the third one does too, which is half of why it was added.</summary>
    [Theory]
    [InlineData("known")]
    [InlineData("direct")]
    [InlineData("range")]
    ...
    /// <summary>
    /// And a settings file naming it for one of those slots is honoured rather than treated as unusable
    /// — the other side of the resolver's rule, and the assertion that would catch it being refused by
    /// accident along with OpenAI.
    /// </summary>
    [Fact]
    ...
    public void TheSlotsCarryingOtherPeoplesWordsDoNot(string slotId)
    {
        ...
        // The ones that can hold a language are all still there, so this narrows rather than empties: a slot
        // with nothing to choose from would be a worse answer than the fault.
    ...
    /// <summary>
    /// The half the picker cannot enforce. `settings.json` is a file a Commander reads and edits, and a
    /// rule that lives only in a dropdown is one a text editor walks past.
    /// </summary>
    [Fact]
    ...
    /// <summary>And the fallback cannot loop.</summary>
    [Fact]
    ...
    /// <summary>A file from before Phase 57, where every slot follows the ship's provider.</summary>
    [Fact]
    ...
    /// <summary>
    /// The clients built for such a file, which is what the arithmetic downstream depends on: two, not
    /// one, and the untrusted slots are on the free one.
    /// </summary>
    [Fact]
