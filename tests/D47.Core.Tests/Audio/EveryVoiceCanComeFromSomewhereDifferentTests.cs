...
/// <summary>Which slot a line belongs to, and which provider speaks for it.</summary>
public class EveryVoiceCanComeFromSomewhereDifferentTests
{
    ...
    {
        // The same division RadioVoice.IsOverTheAir already draws, given a second job rather than a new one
        // drawn beside it.
    ...
    /// <summary>The distinction the boolean cannot make.</summary>
    [Fact]
    ...
    {
        // Spoken by one and gated by the other is the disagreement that would not report itself, which is why
    ...
    /// <summary>Every channel Elite writes has a slot.</summary>
    [Fact]
    ...
    /// <summary>The one that carries the money.</summary>
    [Fact]
    ...
    /// <summary>The ids are settings keys, and the settings file is append-only.</summary>
    [Fact]
    ...
    public void TheShipsRowIsStillTheShipsProvider()
    {
        ...
        // "aboard" cannot be written into that map by any row, and if a hand-edited file holds one it is
        // ignored: speech.provider has meant the ship's voice since Phase 4.
    ...
    /// <summary>A file written before Phase 57 sounds exactly as it did: one provider, every voice.</summary>
    [Fact]
    ...
    /// <summary>
    /// The half a Commander would otherwise discover on a bill: it is not only the untrusted slots that
    /// move, because the carrier and the NPCs arrive over a radio too.
    /// </summary>
    [Fact]
    ...
    public void MigratingHappensOnceAndThenChangesNothing()
    {
        ...
        // The caller compares by reference to decide whether to write.
        Assert.Same(migrated, VoiceGroups.Migrated(migrated));
    }

    /// <summary>A Commander who asked for silence must not be answered by five slots starting to talk.</summary>
    [Fact]
    ...
    public void AWrittenMapIsNeverMigratedOverTheTopOfItself()
    {
        ...
        // Empty is a Commander who put every slot back on the ship's; a single entry is one who chose.
    ...
    /// <summary>
    /// The routing reads what the callout wrote, so the two are held against each other rather than
    /// against a fixture: a message on the wire lands in the slot its channel names.
    /// </summary>
    [Fact]
