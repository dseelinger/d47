...
public class AmbienceTests : IDisposable
{
    ...
    /// <summary>Each of the five, from the flags Elite actually writes.</summary>
    [Theory]
    ...
    /// <summary>Status.json not written yet is general rather than a guess.</summary>
    [Fact]
    ...
    public void AnEmptySituationFallsBackToGeneral()
    {
        ...
        // And the situation itself is still what it is.
        Assert.Equal(Situations.Docked, ambience.Situation);
    }

    /// <summary>General with no files is silence, not an error.</summary>
    [Fact]
    ...
    /// <summary>The whole folder is played before any track repeats, and the order is not the folder's.</summary>
    [Fact]
    ...
    /// <summary>The bed and the music play at once without either taking the queue head.</summary>
    [Fact]
