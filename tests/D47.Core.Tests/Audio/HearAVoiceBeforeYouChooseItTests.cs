...
/// <summary>What a voice says when it is auditioned.</summary>
public class HearAVoiceBeforeYouChooseItTests
{
    ...
    {
        // Warden's introduction runs to five sentences.
        Assert.True(
    ...
    public void EveryCoreHasALineAndNoneOfThemIsAFragment()
    {
        Assert.All(PersonaCatalog.Shipped, persona =>
        {
            ...
            // A sample that stops mid-clause tells you about d47's string handling rather than about the
            // voice.
            Assert.True(
    ...
    /// <summary>
    /// Six of the eleven cores open by addressing the Commander, so the literal first sentence of
    /// Warden's introduction is the single word "Commander." — a sample too short to tell one voice
    /// from another, which is the entire point of auditioning.
    /// </summary>
    [Fact]
    ...
    {
        // The carrier's captain and its tower are different people from the ship's AI and have no written
        // introduction, so they say what they are actually for — a Commander casting a tower voice is
        // listening for something different from a companion.
