...
/// <summary>Who is speaking, and who keeps their voice.</summary>
public class VoiceCastTests
{
    ...
    /// <summary>
    /// An NPC keeps their voice for as long as the Commander is in the system, which is the whole of
    /// what "remember which voice was used" asks for — and drops it on the jump out, which is the half
    /// the request explicitly does not need.
    /// </summary>
    [Fact]
    ...
    /// <summary>
 /// The crew are aboard, so their voices last the session.
    /// </summary>
    [Fact]
    public void TheCrewKeepTheirVoicesAcrossAJump()
    {
        ...
        // One assignment left, and it is the gunner's: the pirate was the only one in the table the jump
        // cleared.
        Assert.Equal((1, 0), cast.Assignments);
    }

    /// <summary>No stranger is handed a voice that already belongs to somebody in the ship.</summary>
    [Fact]
    public void NobodyOutsideTheShipIsGivenAVoiceFromInsideIt()
    {
        ...
        // Which leaves exactly one voice that is nobody's, and every sender gets it rather than one of the
        // two that are spoken for.
    ...
    {
        // Seeded from the name rather than drawn at random, so a recorded session replays identically.
    ...
    {
        // How fast the Commander likes to be spoken to is a property of the Commander, not of who happens to
        // be talking.
        var cast = Cast();
        cast.Rate = 1.15;
...
/// <summary>Two providers, one settings surface.</summary>
public class TtsProviderCatalogTests
{
    [Fact]
    public void EveryProviderDisclosesWhatItReceives()
    {
        // The set is exhaustive by construction, and a provider with no disclosure is a provider whose egress
        // row would render blank.
...
/// <summary>Speaking rate, remembered against the provider it was chosen for.</summary>
public class PerProviderRateTests
{
    /// <param name="provider">ElevenLabs is the exemplar throughout this fixture: it is the provider with a narrow range to be clamped to.</param>
    ...
    {
        // Normalising the units gets them agreeing about what 1.0 means.
    ...
    {
        // ElevenLabs refuses a speed outside its range outright, and a rejected request arrives as silence.
    ...
    public void WritingTheRowStoresItAgainstTheSelectedProviderOnly()
    {
        ...
        // Flash, because v3 Conversational has no rate to write and this is a test about where a written rate
 // is stored.
        ...
        // The general value is untouched, so clearing an override falls back to something sensible rather
        // than to the last provider's number.
...
public class TextToSpeechEgressTests
{
    ...
    public void TheEntrySpeaksForWhicheverProviderIsSelected()
    {
        ...
        // And it says the part a Commander would not otherwise know: turning on re-voiced messages sends text
        // other players wrote.
...
public class VoicesMatchTheNameTests
{
    ...
    /// <summary>
    /// A pool nothing is tagged in still works, because most of what this has to survive is a provider
    /// that says nothing about its voices.
    /// </summary>
    [Fact]
