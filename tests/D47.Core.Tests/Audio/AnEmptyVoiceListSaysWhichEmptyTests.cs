...
/// <summary>Four situations that could each arrive as one empty list.</summary>
public class AnEmptyVoiceListSaysWhichEmptyTests
{
    ...
    {
        // The service's own message is preferred over anything mapped from the status code, for the same
        // reason a refused synthesis prefers it: "Invalid API key" is actionable and "401" is not.
    ...
    public void BeingUnreachableIsNotTheSameAsBeingRefused()
    {
        ...
        // One means wait, the other means go and look at the key.
        Assert.NotEqual(unreachable, refused);
        Assert.Contains("could not be reached", unreachable, StringComparison.OrdinalIgnoreCase);
    }

    /// <summary>
    /// The one case where waiting, retrying and re-pasting the key all change nothing — so it must not
    /// read like any of the three that they do.
    /// </summary>
    [Fact]
