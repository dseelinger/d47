...
public class SentenceSplitterTests
{
    ...
    public void ASentenceIsEmittedAsSoonAsItClosesRatherThanAtEndOfTurn()
    {
        ...
        // The space after the stop is the deciding character, so the first sentence is out the moment it
        // arrives — before the rest of the reply has been generated at all.
        Assert.Equal(
    ...
    /// <summary>
    /// The failure this guards is silence: a model that never emits a terminator would leave the whole
    /// reply buffered until the turn ended, which is the thing the splitter exists to prevent.
    /// </summary>
    [Fact]
