...
public class ColonisationCandidateTests
{
    ...
    /// silently dropped, so a populated system that reaches this code has to be dropped by this code or
    /// it is reported as somewhere to colonise.
    /// </summary>
    [Fact]
    ...
    /// <summary>A system nobody lives in but somebody is already building in.</summary>
    [Fact]
    ...
    /// <summary>
    /// A star and no planets is what an unsurveyed system looks like in a crowd-fed index — a fifth of
    /// the neighbourhood on a real frontier reference.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Landability and rings are the two things the scan cannot see — the embedded body records carry
    /// neither — so asking for them has to reach the body search, by name and never by a zero-width
    /// distance range.
    /// </summary>
    [Fact]
    ...
    /// <summary>The scan already answered.</summary>
    [Fact]
    ...
    /// <summary>
    /// The naive rule produces "18 Rocky bodys" and "4 Gas giant with water-based lifes", which is the
    /// kind of wrongness a Commander hears rather than reads.
    /// </summary>
    [Theory]
