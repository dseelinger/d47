...
/// <summary>What a module can be engineered with, asserted against the shipped tables.</summary>
public class BlueprintOfferTests
{
    ...
    {
        // "Fuel tanks can't be engineered" — and d47's data always agreed.
    ...
    {
        // The third state, and the reason the other two can be trusted.
    ...
    {
        // These defeated every attempt to join through coriolis: a Cargo Scanner kept coming out as a Chaff
        // Launcher, because the two share the generic Lightweight and Reinforced rolls and nothing else
        // distinguished them.
        Assert.Equal(
        ...
        // One blueprint, and the two sources share no word for it: EDSY calls it "Expanded Radius" and
        // EDEngineer "Expanded Probe Scanning Radius".
    ...
    {
        // Bi-Weave and Prismatic are shield generators with their own names, so the name join missed them
        // exactly as it missed armour.
    ...
    {
        // coriolis files one recipe under several names, so the armour Lightweight roll answered to seven of
        // them — six belonging to limpet controllers and weapons.
    ...
    {
        // Remediation 15 item 10, and the string in the reported screenshot. `CannotConfirm` told the
        // Commander outright that nothing d47 shipped joined the two spellings; it does now.
