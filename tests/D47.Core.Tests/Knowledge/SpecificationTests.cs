...
/// <summary>The generated specification table, and what is said about a hull it has no figures for.</summary>
public class SpecificationTests
{
    ...
    {
        // The one fact that decides whether a station is even an option, checked against three hulls where a
        // wrong answer would strand somebody.
    ...
    {
        // The same key a fitted module arrives under, so "what have I got" and "what is that" are one lookup
    ...
    {
        // Armour is per-hull, so coriolis-data files it inside each ship's own JSON rather than under
        // modules/ — and reading only modules/ dropped all 241 of them.
        string[] written =
        [
    ...
    {
        // What IsBulkhead is read off, asserted against the shipped table rather than assumed.
    ...
    {
        // Forty-eight hulls have a "Lightweight Alloy".
        ...
        // And the hull is what makes them unique, which is what keeps a symbol lookup and a spoken lookup
        // answering about the same object.
        Assert.Equal(
            bulkheads.Length,
    ...
    {
        // The join between the two sections: a Sidewinder's 60 armour under Military Grade Composite is the
        // 210 the outfitting screen shows, and it is only derivable if the bulkhead row and the hull row
        // agree about what the number means.
    ...
    {
        // The whole reason the resistances are carried.
        ...
        // Signed, and the sign is the answer: Mirrored gives half again against thermal and takes three
        // quarters more kinetic, and Reactive is the other way round.
    ...
    {
        // Five of them, all the Lynx Highliner's.
    ...
    {
        // outfitting.csv indexes the corrosion-resistant rack at sizes 1 and 4 and
        // not at 5 or 6, so the larger two fell through to a name taken from their file — "Cargo Rack", the
        // same as the plain rack — and `disambiguate` then separated them with the only thing that told them
        // apart, giving "Cargo Rack (corrosionproof)".
    ...
    {
        // `disambiguate` is the generator's last resort: where two modules would otherwise share a name,
        // class, rating and mount, it separates them with whatever token their symbols do not share.
    ...
    {
        // Eleven drives differing by an order of magnitude in every figure worth quoting.
        var result = await Registry().InvokeAsync(
    ...
    public async Task AskedForABulkheadItGivesTheHullBoostAndTheResistancesWithTheirSigns()
    {
        ...
        // Zero is said out loud.
        Assert.Contains("caustic 0%", result.Content, StringComparison.Ordinal);

        // Armour has no size and no rating, so the size falls back to the name — and leading with both would
        // say the whole thing twice.
    ...
    {
        // Forty-eight hulls have a Lightweight Alloy and they weigh and cost different amounts, so an
        // unqualified name has no right answer.
        var result = await Registry().InvokeAsync(
            "get_module_specification",
