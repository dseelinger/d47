...
/// <summary>
/// The engineering capability — the first point in Phase 14 where the tables, the roll rules and the
/// Commander's own journal meet.
/// </summary>
public class EngineeringCapabilityTests
{
    ...
    /// <summary>Farseer at rank 5, so a grade 5 blueprint she offers is reachable.</summary>
    ...
    /// Commander has a rank with, the 399999 module that arrived already engineered, two modules of the
    /// same kind so that a name can be genuinely ambiguous, and one with no engineering at all.
    /// </summary>
    ...
    public async Task ABlueprintSaysWhatItCostsPerApplicationAtEveryGrade()
    {
        ...
        // Per application and per grade, because the recipe changes as the grade climbs and a Commander
        // gathering for grade 3 needs that row rather than the top one.
        ...
        // Ingredients arrive as symbols and go out as names.
    ...
    {
        // Elvira Martuuk grades this to 5 and the Commander is rank 2 with her; Farseer is not unlocked in
        // this fixture, so the best reachable engineer cannot roll it at all.
        ...
        // The gate is stated with how a rank is actually raised beside it.
    ...
    {
        // Capabilities are state, not guards: what a blueprint costs is true for everybody, and only the
        // paragraph about this Commander depends on a journal.
        var answer = await Ask(Registry(), "get_blueprint", ("blueprint", "Increased FSD Range"));
    ...
    public async Task AModuleKindListsWhatItCanTakeRatherThanEveryRecipe()
    {
        ...
        // Experimentals are per module kind rather than global, which is the thing coriolis gets wrong and
        // the reason the table carries them that way.
    ...
    /// <summary>The distinction <see cref="BlueprintKind"/> exists for.</summary>
    [Fact]
    ...
    {
        // Grade 3 at rank 4 fills a quarter at a time, so 0.4 leaves three rolls — counted towards a full 1.0
        // rather than towards the 0.85 band, because the band is where the game stops insisting rather than a
        // target to aim at.
    ...
    {
        // The same module with nothing known about the Commander's standing.
    ...
    public async Task TheRollIsReportedInRealUnitsWithTheDirectionTheRightWayRound()
    {
        ...
        // The two halves of LessIsGood, and the reason it is read as 0/1 rather than as a JSON boolean: read
        // as one, every improvement on mass reports backwards.
    ...
    {
        // 27 of 772 engineered modules measured carry engineer id 399999 and no name.
    ...
    {
        // "shield booster" names two fitted modules here, rolled to different grades.
        ...
        // And each with its own state, because which of the two is worth finishing is the actual question
        // behind asking.
