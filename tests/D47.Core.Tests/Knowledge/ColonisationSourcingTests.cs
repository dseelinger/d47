...
public class ColonisationSourcingTests
{
    ...
    /// <summary>The acceptance the plan calls blunt and not negotiable.</summary>
    [Fact]
    ...
    /// <summary>Why the join is on the name and not the symbol, asserted so nobody puts it back.</summary>
    [Fact]
    ...
    /// <summary>
    /// Phase 17 measured the fold going wrong in exactly this direction: the decoration stripped and
    /// the case left alone joined the depot to the hold perfectly and matched nothing else.
    /// </summary>
    [Fact]
    ...
    /// <summary>Nothing is dropped in silence.</summary>
    [Fact]
    ...
    /// <summary>The objective is trips, not credits.</summary>
    [Fact]
    ...
    /// <summary>
    /// Two stations covering the same set are separated by what the trip actually costs, rather than by
    /// whichever the sweep happened to return first.
    /// </summary>
    [Fact]
    ...
    /// <summary>The carrier is out of the arithmetic entirely.</summary>
    [Fact]
    ...
    /// <summary>A station that sells it and holds none of it is not a source.</summary>
    [Fact]
    ...
    /// <summary>
    /// A depot row that somehow carries no symbol falls back to folding its own name, so the join still
    /// happens rather than the commodity vanishing.
    /// </summary>
    [Fact]
