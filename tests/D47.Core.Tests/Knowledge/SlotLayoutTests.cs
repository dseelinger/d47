...
/// <summary>The slot layout, against the shipped table.</summary>
public class SlotLayoutTests
{
    ...
    {
        // The journal's name for it says hardpoint and the outfitting screen's does not, and the screen's is
 // the one the Commander is holding in their head.
    ...
    {
        // The exclusion is the table's shape rather than a filter anybody maintains: a paint job is not a
 // thing you outfit, so it is not a slot here.
        string[] absent =
        [
    ...
    // The four hulls whose numbering no rule derives.
    ...
    {
        // A Panther's two cargo holds, a Prospector's limpet bay and hangar, and the military compartment
        // every combat hull carries.
    ...
    {
        // The two core slots that refuse an undersized module.
    ...
    public void AnScoDriveWillNotSitInACompartmentLargerThanItself()
    {
        ...
        // The ordinary drive may be undersized; the supercruise-assist one may not, and it says so on the
        // module rather than in a rule about drives.
    ...
    {
        // A Sidewinder has compartments and cannot carry a fighter; the Prospector's hangar bay can.
