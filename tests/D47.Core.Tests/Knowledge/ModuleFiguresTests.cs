...
public class ModuleFiguresTests
{
    ...
    {
        // The formula is coriolis's own, lifted from MIT-licensed code rather than derived here, and this is
        // the check that the reading was right: EDOMH shows a 1G turreted Pulse Laser at 3.97HP/s from a rate
        // of fire of 3.33/s, and the table computes 3.97 from a damage of 1.19 and a fire interval of 0.3.
    ...
    {
        // Eighteen weapons carry no fire interval at all — every beam laser and every mining laser — because
    ...
    {
        // 156 of 178 distributions are real proportions and 22 are not: those 22 carry `X` for anti-xeno and
        // sum to 2, so `X` sits beside a real type rather than taking a share of it.
    ...
    {
        // The correction item 9 made to item 2b: the first list of figures was hardpoint-specific and applied
        // to no distributor at all.
        ...
        // And the standard one, so the Guardian's trade is legible: smaller capacitors, faster recharge,
        // which is exactly what Frontier's own description says it does.
    ...
    {
        // Ask #10, answered in one sentence from an id-keyed field, in Frontier's words — which is the shape
        // the game-data invariant prefers and NOTICE already practises.
    ...
    {
        // Item 8, and the ordering is free: the effects column carries a good-or-bad flag per line, so d47
        // never has to be taught which way is better.
        ...
        // Frontier's own attribute names, which match the outfitting screen — the same argument the slot
    ...
    {
        // The one thing item 8 said to check rather than assume. 34 of 160 module-and-blueprint pairs change
        // their attribute set across grades, and in every one the highest grade's set contains all the others
        // — so the general line is the top grade's, with real numbers, rather than a union of five.
