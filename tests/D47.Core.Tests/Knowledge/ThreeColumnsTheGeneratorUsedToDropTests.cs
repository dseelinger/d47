...
/// <summary>
/// The three columns Phase 38 asks the generator to stop dropping, asserted against the shipped table.
/// </summary>
public class ThreeColumnsTheGeneratorUsedToDropTests
{
    ...
    public void APrismaticShieldGeneratorIsBehindAPledgeAndAStockOneIsNot()
    {
        ...
        // The id is carried whole rather than parsed down to a flag: it names *which* Power, and a later
        // release that finds a source for those names should not have to regenerate the table to get at it.
    ...
    {
        // Named rather than counted alone, because the count is the part that can be right for the wrong
        // reason.
    ...
    public void EveryPowerPlantSaysWhatItMakes()
    {
        ...
        // A plant's own draw stays empty, which is the reason this column had to exist: the generator maps
        // draw, and for a plant the meaningful figure is output.
    ...
    {
        // Thrusters and shield generators carry an optimal mass too — 39 and 55 of them — so the older test
        // of "has an optimal mass" was true of 94 modules that are not drives, and the specification report
        // offered every one of them a max fuel per jump with nothing after it.
    ...
    {
        // The jump gauge's laden and full-tank needles are computed from these on a build d47 has to model
        // rather than read — and they also tell a 5E Cargo Rack from a 6E, which was the complaint the
        // figures bag was built to answer and which these two kinds fell through.
