...
/// <summary>
/// A tool handed a neighbour's input says so and answers, generalising the fix that shipped in
/// v0.73.0 for #54.
/// </summary>
public class TwoTablesOneQuestionTests
{
    /// <summary>A representative of each ledger, taken from the catalogue rather than written down here.</summary>
    ...
    /// <summary>
    /// Every direction says three things: what it actually is, which tool answers, and — the part that
    /// makes it an answer rather than a redirect — what that tool will tell them.
    /// </summary>
    [Theory]
    ...
    /// <summary>The rule the shipped one established: answer, do not redirect.</summary>
    [Fact]
    ...
    /// <summary>
    /// And an Odyssey good gets both halves — where it comes from and which buildings hold it — so the
    /// answer is as complete on the on-foot side as on the ship side.
    /// </summary>
    [Fact]
    public void AnOdysseyGoodSaysWhichBuildingsHoldIt()
    {
        // 163 of the 196 ship-locker entries carry buildings, and every one of those carries origins as well
        // — so the seam says both rather than choosing, and a test that expected one or the other would be
        // asserting a branch nothing reaches.
    ...
    /// <summary>
    /// The reason a Commander was told no is said in terms of what the tool they hit is for, rather
    /// than as a bare "wrong tool" — they did not choose the tool and should not have to care that one
    /// was chosen.
    /// </summary>
    [Theory]
    ...
    /// <summary>
    /// The seam reads <see cref="MaterialEntry.Ledger"/> and nothing else, so a material added to the
    /// generated table tomorrow is routed without anybody adding it anywhere.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// One seam is closed by construction rather than by a check, and this is where that is recorded:
    /// <c>find_material_trader</c> takes its <c>type</c> from a closed set with <c>AllowedValues</c> on
    /// the parameter, so a material name cannot land on it at all.
    /// </summary>
    [Fact]
    public void TheTraderToolCannotReceiveAMaterialName()
    {
        ...
        // Every accepted value is a kind of trader, not a thing to trade.
        Assert.All(
