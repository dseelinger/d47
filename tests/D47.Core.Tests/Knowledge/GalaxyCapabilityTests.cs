...
public class GalaxyCapabilityTests
{
    ...
    public async Task ASignalCountWithNoSignalToCountIsRefused()
    {
        ...
        // results rather than being ignored, which would read as "nowhere".
        var result = await registry.InvokeAsync(
            "find_body",
            Args(("signal_count", "3")),
            TestContext.Current.CancellationToken);
    ...
    public async Task AHotspotAnswerCarriesTheRingsAndHowOldTheReportIs()
    {
        ...
        // The other ring around the same planet has no Painite in it, and naming it invites the Commander to
        // fly to the wrong one.
