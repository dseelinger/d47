...
/// <summary>Decoding a procedural system name.</summary>
public class SystemNameTests
{
    ...
    // A multi-word sector *and* no boxel number at once, which is where a grammar that handled either alone
    // would come apart.
    ...
    /// <summary>
    /// The boxel number is written only once a sector's letters have run out, so its absence means zero
    /// rather than unknown — and a parser that demanded it would reject a real name and report "no mass
    /// code here", which is a wrong answer wearing the shape of a right one.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The internal consistency check that closes the ladder: eight rungs doubling from 10 land exactly
    /// on the sector, which is what mass code <c>h</c> is.
    /// </summary>
    [Fact]
    ...
    /// <summary>Things that look procedural and are not.</summary>
    [Theory]
