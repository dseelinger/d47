...
/// <summary>
/// Invented background chatter: a marker now and then, from a callout that follows the ambient
/// timing rules — and never a line of text, because chatter is model-written or it is nothing.
/// </summary>
public class NpcChatterCalloutTests
{
    ...
    public void TheFirstTickSeedsAndTheIntervalHoldsAfterIt()
    {
        ...
        // The first tick seeds the clock: silence for one whole interval after launch, exactly the ambient
        // rule.
        ...
        // A marker, never a line: empty text is what makes a missed road speak nothing rather than something.
    ...
    public void PrimingFoldsTheBacklog()
    {
        ...
        // Seeded by a real first tick, so what priming folds is an exchange that would otherwise be due.
        _ = callout.Examine(Context(T0)).ToArray();

        Assert.Empty(callout.Examine(Context(T0 + TimeSpan.FromHours(2), priming: true)));
    }

    /// <summary> The gap between exchanges varies inside [Interval, Longest] rather than ticking — and
    /// deterministically, off the pick counter, because a recorded session has to replay to the same spacing.
    /// </summary>
    [Fact]
    public void TheGapVariesInsideTheRangeAndReplaysTheSame()
    {
        ...
        // Nineteen minutes is inside no possible gap; forty is past every one.
        Assert.Empty(callout.Examine(Context(T0 + TimeSpan.FromMinutes(19))));
    ...
    /// <summary>
    /// The pairing ladder is deterministic off the pick counter — no Core component reads a clock or a
    /// seed.
    /// </summary>
    [Fact]
...
public class NpcChatterScriptTests
{
    ...
    {
        // One surviving line of a two-person exchange is a fragment, and silence beats a fragment — the same
 // judgement the ambient drop makes.
 ...
 // The Commander's own carrier: its two posts speak in the voices he cast for them, and nobody
    // invents a jump it is not making.
    ...
    public void TheCarrierIsAtHandOnItsDeckOrInItsSpaceAndNowhereElse()
    {
        ...
        // Nor is a pad inside somebody else's station in the same system — that tower is theirs, and casting
        // it as the carrier's is the reported fault pointed the other way.
        Assert.False(NpcChatterCarrier.Of(
            Mine,
    ...
    public void ItsTowerAndCaptainCarryTheCastRolesAndNobodyElseDoes()
    {
        ...
        // An invented pilot is an invented nobody, rank or no rank: "Captain Reyes" is a person with a title,
        // not the captain of this ship.
    ...
    public void AnInventedDepartureTakesTheWholeExchangeWithIt()
    {
        ...
        // Somebody else has to say which carrier they mean — and then it goes the same way, whether or not
        // the Commander is anywhere near it.
        Assert.Empty(NpcChatter.Parse(
    ...
    /// <summary>
 /// The contradiction guard's retry asks for one line rather than for a scene, so it never
    /// passes through <see cref="NpcChatter.Parse"/> — and the carrier veto above lives there.
    /// </summary>
    [Fact]
    public void AReplacementBeatGoesBackThroughTheCarrierVeto()
    {
        var here = NpcChatterCarrier.Of(Mine, At("Shinrarta Dezhra"));

        Assert.Null(NpcChatter.Rewritten("Heard Nomad's Rest is casting off at 0200.", null, here));
        Assert.Null(NpcChatter.Rewritten("Tower: We cast off at 0200.", VoiceRole.TowerControl, here));
        Assert.Null(NpcChatter.Rewritten("   ", null, here));

        // The name the model was asked not to write, written anyway, and the quotes with it.
        Assert.Equal(
            "Quiet run tonight, then.",
            NpcChatter.Rewritten("Vance: \"Quiet run tonight, then.\"", null, here));

        // One line was asked for; the first with anything in it is the answer.
        Assert.Equal(
            "Quiet run tonight, then.",
            NpcChatter.Rewritten("\nQuiet run tonight, then.\nAnd the pad is clear.", null, here));

        // Their own jump is still their own business, exactly as it is through Parse.
        Assert.Equal(
            "I jump for Sol as soon as the pad clears.",
            NpcChatter.Rewritten("I jump for Sol as soon as the pad clears.", null, here));
    }
    ...
    public void TheInstructionNamesTheTwoPostsAndRefusesTheJumpItIsNotMaking()
    {
        ...
        // The jump rule holds wherever the carrier is: the live game state names it parked three hundred
        // light years away just as clearly.
