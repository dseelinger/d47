...
/// <summary>Which callouts a model is allowed to rewrite.</summary>
public class OnlySomeLinesAreSaidInCharacterTests
{
    ...
    /// <summary>
    /// Personality off silences all of it, including the carrier — a captain improvising is personality
    /// by any reading.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// An ambient remark is the core's own, so it gets the persona block and the live game state: it is
    /// about where the Commander actually is.
    /// </summary>
    [Fact]
 ...
 /// <summary>Spoken to the Commander, never narrated.</summary>
    [Fact]
    ...
    /// <summary>The carrier's two roles are other people entirely.</summary>
    [Theory]
    ...
    /// <summary>
    /// The situation is read back off the key, because a batch may hold more than one ambient callout
    /// and the callout itself only remembers the last.
    /// </summary>
    [Fact]
    ...
    /// <summary>A core's first line goes through the model too, as a rewording.</summary>
    [Fact]
    public void ACoresFirstLineIsRewordedRatherThanRead()
    {
        ...
        // The authored line is handed over, because this is a rewording and not a composition: a first line
        // carries things a Commander needs, and a model asked to improvise a greeting would lose them.
    ...
    /// <summary>
    /// Personality off means said exactly as written — the same rule every announcement follows, and
    /// the reason a Commander who has turned it off hears the authored line unchanged.
    /// </summary>
    [Fact]
    ...
    /// <summary>
 /// Every line the ship's AI says to the Commander knows who the Commander is: the
    /// ambient remarks, the opening line and a core's first words all carry the sheet.
    /// </summary>
    [Fact]
    ...
    /// <summary>The two flags correlate and are not the same flag.</summary>
    [Fact]
    ...
    /// announcement's index, so it is decided by the same count that picked the stock line and never by
    /// a clock.
    /// </summary>
    [Fact]
