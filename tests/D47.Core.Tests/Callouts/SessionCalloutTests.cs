...
/// <summary>A word on getting into a game and on leaving one.</summary>
public class SessionCalloutTests
{
    ...
    /// <summary>And the clock does not restart on a suppressed one.</summary>
    [Fact]
    ...
    /// <summary>A second Commander is arriving for the first time, not re-logging.</summary>
    [Fact]
    ...
    /// <summary>
    /// Priming folds the backlog and says nothing. d47 starting reads every LoadGame of the day;
    /// without this, launching it after an evening's flying announces an arrival from four hours ago.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The two directions keep separate clocks, so a quick out-and-back still says goodbye and hello
    /// once each rather than one of them swallowing the other.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A session that never says goodbye is not reconstructed. 84 of 925 journals end with no Shutdown
    /// at all, and a timeout that guessed at a departure would eventually say it to a Commander who is
    /// still flying.
    /// </summary>
    [Fact]
