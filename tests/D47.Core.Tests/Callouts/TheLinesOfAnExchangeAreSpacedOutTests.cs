...
/// <summary>
/// Air between the lines of an invented exchange, reported as "it's like watching an episode of
/// the Gilmore Girls".
/// </summary>
public class TheLinesOfAnExchangeAreSpacedOutTests
{
    ...
    {
        // The gap belongs in front of a reply, not in front of the scene.
    ...
    /// already made about the gap between exchanges.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Off the line's position and nothing else — no clock, no seed — so a recorded session replays to
    /// the pacing it was heard at.
    /// </summary>
    [Fact]
    ...
    /// <summary>What the speaking loop asks before it takes a beat.</summary>
    [Fact]
    public void AWarningQueuedBehindAnExchangeIsSeenWhileItIsStillWaiting()
    {
        ...
        // Read rather than drained: the loop asks this again on every slice of a pause, and an answer that
        // consumed the warning would be an answer that lost it.
