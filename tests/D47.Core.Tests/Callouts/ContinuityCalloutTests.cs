...
/// <summary>The opening line of a session: a greeting on the Commander's own clock and a readiness, and nothing about the list, the gap or the engineer under their feet — those are answered when asked for.</summary>
public class ContinuityCalloutTests
{
    ...
    {
        // Nine in the morning five hours west of Greenwich is two in the afternoon there.
    ...
    /// <summary>
    /// What the line no longer carries (2026-08-21): "Top of your list: Grade 5 Efficient Weapon on 2F
    /// Pulse Laser on Hammer (Type-11 Prospector); then…" was the complaint, word for word.
    /// </summary>
    [Fact]
    ...
    {
        // Priming is the replay of everything before this session, and a greeting produced from the middle of
        // that replay would be about the wrong moment.
    ...
    public void TheLineWaitsForTheSettleWindowAndThenIsSaidExactlyOnce()
    {
        ...
        // The first live tick only starts the clock: the backlog has just been folded and Status.json may not
        // have been read yet.
    ...
    /// <summary>Once per session rather than once per run, where a login by somebody else starts a
    /// session.</summary>
    [Fact]
    ...
    /// <summary>
    /// The launch greeting is unchanged: it does not name the Commander even when the store knows who
    /// they are.
    /// </summary>
    [Fact]
    ...
    /// <summary>It is d47 talking, so it belongs on the conversation page as well as being heard.</summary>
    [Fact]
