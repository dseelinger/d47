...
/// <summary>Remarks said because nothing has happened.</summary>
public class AmbientCalloutTests
{
    ...
    // Longest pinned to the interval, so the timing tests below drive a fixed cadence; the range has its own
 // test.
    ...
    {
        // The checklist asks for ten per state covered.
    ...
    {
        // A remark while the Commander is still reading the panel is not ambient, it is an interruption.
        var callout = Callout();
 ...
 /// replacement can be chosen by the same count.
    /// </summary>
    [Fact]
    ...
    {
        // Status.json flips several times a minute during an approach.
        var callout = Callout();
    ...
    {
        // Two remarks about supercruise, an interval apart, is where ambient stops sounding like company.
        var callout = Callout();
        var cruising = In(StatusFlags.Supercruise | StatusFlags.InMainShip);
    ...
    {
        // The checklist puts "no ambient remarks" in that item's acceptance criteria, which makes this the
        // one callout the personality switch reaches.
        var callout = Callout();
    ...
    {
        // No Core component reads a clock or a random seed, and a recorded session has to replay to the line
        // it produced live.
        Assert.Equal(
    ...
    /// <summary>
 /// The gap between remarks varies inside [Interval, Longest] rather than ticking — and
    /// deterministically, off the pick counter, because a recorded session has to replay to the same
    /// spacing.
    /// </summary>
    [Fact]
    public void TheGapVariesInsideTheRangeAndReplaysTheSame()
    {
        ...
        // Sixteen minutes on from that one is past the floor and still inside this cycle's gap, which is the
        // whole of the change: the wait grew, and it grew by something other than a fixed amount.
        ...
        // The same drive again lands on the same cycle boundaries: the spacing comes off the pick counter,
        // never a clock or a seed.
        var replay = Spread();
    ...
    /// <summary>
    /// The two edge cases the chatter pair already has, and they are the same two on purpose: two rows
    /// of the same kind disagreeing about their own edges is worse than neither having the spread.
    /// </summary>
    [Theory]
    [InlineData(15)]
    [InlineData(5)]
    public void AMaximumAtOrBelowTheMinimumPinsTheCadence(int longestMinutes)
    {
        ...
        // Two cycles, both landing on the interval exactly.
        Assert.Single(callout.Examine(At(Start.AddMinutes(15), docked)).ToArray());
