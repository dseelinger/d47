...
public class CoverageIsOutOfTheWayTests
{
    ...
    /// <summary>
    /// And when it is asked for, it is one row on the diagnostics card — decided before registration,
    /// so descriptors are still registered once and never mutated either way.
    /// </summary>
    [Fact]
    public void AskingForItAddsExactlyOneRow()
    {
        ...
        // Info rows are read-only and computed at render time, so the row reports rather than configures —
        // there is nothing here for the model or the router to set.
        Assert.Null(added.Binding!.Write);
        Assert.Empty(added.Commands);
    }

    /// <summary>It never becomes something the model can call.</summary>
    [Fact]
