...
/// <summary>
/// The point of the ledger is the middle status. "I tested that" is only true of the thing as it was
/// when it was tested, and a definition that has changed since makes the old exercise worthless — which
/// is exactly the case a person's memory gets wrong.
/// </summary>
public class CoverageLedgerTests
{
    ...
    public void SomethingThatChangedSinceItWasRunIsStale()
    {
        ...
        // The date it was last exercised survives, because "I looked at this in August and it has moved
        // since" is more useful than losing the fact entirely.
        Assert.Equal(Monday, line.LastSeen);
    }
    ...
    /// <summary>The report opens with what still needs attention.</summary>
    [Fact]
    ...
    /// <summary>
    /// "It ran" and "it ran and worked" are different answers, and only the second one says whether to
    /// go back to it.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Last write wins in both directions: something fixed since it failed goes green, and something
    /// that has started failing goes red however long it worked before.
    /// </summary>
    [Theory]
    ...
    /// <summary>
    /// Failures are not a share of the exercised count — a failure can be stale too — so the summary
    /// keeps them in a sentence of their own rather than in the list of proportions.
    /// </summary>
    [Fact]
    ...
    /// leads — and it appears once, not in its failure section and again under its status.
    /// </summary>
    [Fact]
    ...
    /// <summary>A coverage.json written before outcomes existed has no outcome field.</summary>
    [Fact]
