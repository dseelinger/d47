...
/// <summary>The gap between every plan and what the Commander is carrying.</summary>
public class PlanGapTests
{
    ...
    /// <summary>
    /// A Commander flying a Python, rank 5 with Farseer, holding some iron and on foot in a grade 3
    /// Maverick with nothing in the locker.
    /// </summary>
    ...
    /// <summary>
    /// Ship materials and ship-locker micro-resources are separate ledgers with separate caps and no
    /// exchange between them, so they are reported apart and never summed.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Two slots wanting the same material are one line with both askers on it, and the need is the sum
    /// — which is the arithmetic nobody can do in their head.
    /// </summary>
    [Fact]
    ...
    public void CountingWhatIsNotOwnedIsAFilter()
    {
        ...
        // Either way it says how many were left out, so the filter is reported rather than silently applied.
    ...
    /// <summary>
    /// Nothing carried, nothing planned: an empty answer means "nothing is planned" rather than "you
    /// have everything", and the page needs to be able to tell those apart.
    /// </summary>
    [Fact]
    ...
    public void TradeIsOfferedBesideTheShortfallAndNeverInsteadOfIt()
    {
        ...
        foreach (var line in traded)
        {
            ...
            // And the offer covers it out of something the Commander actually holds spare - here the 300
            // iron, which nothing in the plan wants.
            Assert.True(line.Trade!.Get >= line.Short);
            Assert.NotEqual(line.Material.Symbol, line.Trade.From.Symbol);

            // And it says who would make the trade. "or trade 24 Iron for 8" states a rate with no owner, and a
            // turn carrying one ownerless offer beside one engineer's name produced an answer offering that
            // engineer's trade.
    ...
    /// <summary> A demand knows which blueprint it is for, asked for indirectly on 2026-08-20 when the
    /// Commander asked what a shortfall of Conductive Polymers was for and d47 answered that it could not tell
    /// which blueprint ate them. </summary>
    [Fact]
    ...
    /// <summary>
    /// And the fleet-wide line does not say it, which is the ruling rather than an oversight: a
    /// shortfall can name a dozen demands, and a blueprint on each would double a line that is read
    /// aloud as often as it is drawn.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A slot planned as a module and nothing else has no blueprint to name, and says the same thing it
    /// always said rather than an empty separator.
    /// </summary>
    [Fact]
