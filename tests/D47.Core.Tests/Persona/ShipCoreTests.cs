...
public class ShipCoreStoreTests
{
    ...
    public void ABindingSurvivesBeingWrittenAndReadBack()
    {
        ...
        // Written for a person reading the file, and read back so a hand edit of them is not silently
        // reverted the next time anything is bound.
    ...
    /// <summary>The Commander is half the key: Elite's ship ids are per Commander and start small, so two Commanders' ship 12s are two ships — each binds their own, and each reads back their own.</summary>
    [Fact]
    ...
    /// <summary>
    /// A file from before bindings carried a Commander: claimed whole by the first one seen, the way
    /// the checklist adopts unowned notes — a release must not silently unbind every core.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A hand edit is the route this file is meant to be usable by, so a bad line is reported and the
    /// rest still loads — one typo must not cost somebody the other five ships they bound.
    /// </summary>
    [Fact]
    public void ABadLineIsReportedAndTheRestOfTheFileStillLoads()
    {
        ...
        // A core d47 does not ship is refused rather than resolved to the default, which is the one place
        // this differs from the settings row: resolving would put a core the Commander never chose aboard a
        // ship they did, silently, every time they boarded it.
        Assert.Contains(store.Problems, problem => problem.Reason.Contains("not-a-core", StringComparison.Ordinal));
    }

    /// <summary>
    /// Polled by content rather than by a file-system stamp, so an edit made while d47 is running is
    /// live without a restart — and two writes inside one 15.6 ms clock tick are not one.
    /// </summary>
    [Fact]
...
public class ShipCoreWatchTests
{
    ...
    /// <summary>The ship d47 finds the Commander already in.</summary>
    [Fact]
    ...
    public void AShipWithNoBindingChangesNothing()
    {
        ...
        // And boarding it later is still nothing: a ship nobody bound leaves whoever is aboard aboard, which
        // is the whole of "nothing is bound until it is asked for".
        Board(state, 99);
    ...
    /// <summary>
    /// The shipyard shuffle, which is the case the settle window exists for: five ships in five minutes
    /// is one switch, for the ship they were actually still in.
    /// </summary>
    [Fact]
    ...
    /// <summary>Ending up back where they started is not a change.</summary>
    [Fact]
    ...
    /// <summary>
    /// Binding is an act and applying is a consequence: binding the ship they are sitting in must not
    /// then read as an arrival and announce the core that is already aboard.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The bug the key exists for (the Phase 44 defect item): another Commander's ship 12 is a
    /// different ship, so their binding must not put a core aboard this Commander's.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A file from before bindings carried a Commander still works on the release that added the key:
    /// the first Commander seen claims it, and their core arrives as it always did.
    /// </summary>
    [Fact]
    ...
    /// <summary>The live half of the Phase 44 defect.</summary>
    [Fact]
    public void AResetAdoptsTheNewCommandersShipAndItsOwnCoreSilently()
    {
        ...
        // Silent, as the ship d47 found them in: a login is not a ship change they just made, and the
        // greeting is the new core's first words.
