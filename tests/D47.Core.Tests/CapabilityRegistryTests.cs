...
public class CapabilityRegistryTests
{
    ...
    /// <summary>
    /// The announcement is what the coverage record is built from, and "it ran" and "it ran and worked"
    ...
    /// <summary>
    /// A handler is allowed to report failure without throwing, and that reads the same to the model —
    /// so it has to read the same here.
    /// </summary>
    [Fact]
    ...
    /// <summary>The distinction the whole outcome rests on.</summary>
    [Theory]
    ...
    /// <summary>An interrupted call is not a verdict on the tool.</summary>
    [Fact]
    ...
    /// <summary>Once per call, not once per return path.</summary>
    [Fact]
...
public class DiagnosticsCapabilityTests
{
    ...
    public void SettingsRowsAreProjectedFromTheSameClosedSetTheToolUses()
    {
        ...
        // One row per subsystem plus the default.
        Assert.Equal(
...
/// <summary>
/// Passes the test's cancellation token, which xUnit's analyzer asks for and which makes a hung
/// capability fail the run rather than stall it.
/// </summary>
