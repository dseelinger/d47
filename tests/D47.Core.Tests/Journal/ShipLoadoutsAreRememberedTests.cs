...
/// <summary>A ship's modules survive the Commander getting out of it.</summary>
public class ShipLoadoutsAreRememberedTests
{
    ...
    {
        // Core reads no clock, which is what lets the replay harness drive this at 100x and lets a backfill
        // over month-old files date what it finds correctly rather than as "now".
        var state = State(
    ...
    {
        // The one event that makes a remembered ship wrong rather than merely old.
        var state = State(
            Boarding("2026-08-20T01:00:00Z", "type9_military", 53, "int_engine_size7_class5"),
            Boarding("2026-08-20T02:00:00Z", "anaconda", 51, "int_engine_size7_class2"),
            """
    ...
    {
        // ShipLoadout.Apply returns `this` for every event it does not handle, so the registry can tell
        // "nothing changed" by reference and never compares a module list.
        var state = State(Boarding("2026-08-20T02:00:00Z", "anaconda", 51, "int_engine_size7_class2"));
