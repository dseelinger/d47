...
/// <summary>
/// The reporting surface over the depot fold: what
/// a site still wants, what is already aboard, and the two things d47 must refuse to say — that the
/// figures are live, and what is on the carrier.
/// </summary>
public class ColonisationCapabilityTests
{
    ...
    public async Task ASiteReportsWhereItIsHowFarAlongAndWhatIsLeft()
    {
        ...
        // One of the two rows is met, so it is not outstanding — and the tonnage is the remainder of the
        // other, never the total.
    ...
    /// <summary>The caveat that stops an exact number being read as a live one.</summary>
    [Fact]
    ...
    /// <summary>The measured refusal.</summary>
    [Fact]
    ...
    /// <summary>
    /// Several sites can be open at once — measured, three at a time — so this asks rather than picks.
    /// </summary>
    [Fact]
    ...
    /// says finished, never "the events stopped": a completed site keeps reporting, 2 to 60 more times
    /// in the corpus.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// With no hold read, the shortfall must not silently claim nothing is aboard — an unread hold and
