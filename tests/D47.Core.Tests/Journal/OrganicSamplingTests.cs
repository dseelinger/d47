...
public class OrganicSamplingTests
{
    ...
    /// <summary>A great circle, not a flat approximation.</summary>
    [Fact]
    ...
    /// <summary>
    /// Radius comes off <c>Status.json</c> per body, so a small moon and a large planet give different
    /// distances for the same angle — which is the point of carrying it on the fix.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// <c>ScanOrganic</c> carries no position on any of the 632 events measured, so a sample taken with
    /// no <c>Status.json</c> fix still counts and simply carries no distance.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// <c>ScanOrganic</c> writes <c>"Body": 37</c> — an id, not a name — so the key is numeric and two
    /// genera on one body share a record.
    /// </summary>
    [Fact]
    ...
    public void ARunSurvivesTheAppRestarting()
    {
        ...
        // The position too, so a Commander logging back in beside their last specimen is told how far they
        // have moved rather than being told nothing.
