...
/// <summary>Folding the community goal board.</summary>
public class CommunityGoalTests
{
    ...
    public void TheBoardIsReadIntoTheFieldsTheItemAsksFor()
    {
        ...
        // The tiers, which is what the phase asks for by name.
    ...
    {
        // Hiding a goal because a field was missing is the wrong direction to fail in: the Commander loses a
        // goal that may well be running, and nothing tells them why.
        var board = Fold(
            """
    ...
    {
        // The case the corpus could not settle.
        var board = Fold(
    ...
    {
        // The board says nothing about signing up, so a fresh read's defaults would un-join the goal every
        // time the Commander docked — and the answer would then read "not signed up" about a goal they are
        // actively flying.
        var board = Fold(
            Board(),
            """
    ...
    {
        // A Commander who joined a goal before d47 was watching is not a Commander who has not joined it.
        var board = Fold(
            """
    ...
    {
        // Which is also how "where did this module come from" gets answered: the Operations spike found a
        // pre-engineered module appearing in StoredModules at BuyPrice 0, thirty-two seconds after this
        // event.
        var board = Fold(
            Board(),
            """
    ...
    {
        // A tier is an ordering a Commander plans around.
