...
<p class="body">Several classes on one row — <em>2D, 1D</em> — means that module comes in more than one that fits here. Pick the name first; Directive 47 asks which variant afterwards, and only when the answer is not already obvious.</p>
</section>
<section>
...
<div class="cards">
<a class="card" href="capabilities/ships.html"><span class="ct">Ships →</span><span class="cd">The fleet, the slots a hull has, and what promoting a build does.</span></a>
<a class="card" href="capabilities/specifications.html"><span class="ct">Specifications →</span><span class="cd">Where these figures come from, and the three ways a hull can be unknown.</span></a>
<a class="card" href="capabilities/engineering.html"><span class="ct">Engineering →</span><span class="cd">The next question after this one: which blueprint goes on it.</span></a>
</div>
</div>
</div></div>
...
Above the search box, the page states the ship, the slot, its size, and what is in it now. It ends
with the sentence that matters most:

> It does not reach your checklist until you promote the build.

A build is a plan. Choosing here writes to that plan and nothing else — not your ship, not your
shopping list, not your materials. Promoting it is the separate, deliberate act that turns a plan
into work to do.
...
| Part | What it is |
|---|---|
| **The name** | The module, as the game calls it |
| **The classes** | Every size-and-class of it that fits this slot — `2D, 1D` means two do |
| **The figures** | What separates one from another: rate, type, and the raw number |

The list is ordered by what the slot is for, not alphabetically. Where several classes of one
module fit, they are one row rather than several — you are choosing *what*, and *which one* is a
second question Directive 47 asks only when the answer is not already forced.

### Keep what is fitted

The first row is always the one that changes nothing: **keep** what is there. It exists because
"leave this alone" is a real decision in a build, and a page that offers a hundred alternatives and
...
**¤ — needs a Powerplay pledge.** The module is one Power's, and **Directive 47 cannot tell you
which**. That is a limit of the data rather than an oversight, and saying so is better than naming
a Power that might be the wrong one.

**Engineered with no recipe.** Frontier engineers the module, and no source Directive 47 carries
knows what it costs. Named where it happens, so a blank is never mistaken for *not engineerable*.

### Searching

The box narrows the list as you type. It is a **filter, not a spelling test**: type `cargo` and you
get every cargo rack, type nothing and you get everything. It never refuses a value, because a list
only has to be got near, where a name has to be got right.

**Type it instead** is beside it for the opposite case — you know exactly what you want and would
rather say it than hunt for it.

### What it will not do

Directive 47 never buys, sells, stores or fits a module. It has no access to your ship's outfitting
and would not use it if it had. What it does is keep the plan, work out what the plan costs in
materials and credits, and put that on your checklist when you say so.
