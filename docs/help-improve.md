...
<!--
  ...
  **Why a history is shown as a report is on this page alone (#295).** It answers a question
  nobody standing at the window is asking, so the glyph dropped it. The dialog speaks a
  Commander's words too — bug rather than defect, the app rather than the parser — and this page
  follows it.
-->
...
**Forget sits beside Send, on both pages.** One press asks for everything this installation ever
sent to be deleted, and the window says what came back. The same press lives on the
[Privacy and egress](capabilities/privacy.html) page, and the two are the one call — an excerpt is
deleted after 30 days anyway, and a donated journal history is kept until you ask for it back.

### Why real journals

A bug in Directive 47 is nearly always a bug about a *situation*: a callout that fires when it
should not, a ship state nobody anticipated, an event Frontier added that the app had never seen.
...
what the game did and the log says what Directive 47 made of it, and a bug is nearly always in
the gap between them.

**A journal history** is many journals at once, as far back as Include says. It is for the
bugs nobody has reported: with enough real journals, a callout that misfires in some rare
...
The toggle is there whichever reading you pressed the button on, and it starts off. It used to
depend on where you were: pressed on the Journal page the window opened on the history, and pressed
on Log File the toggle was missing altogether. One page now, opening on the excerpt, with the
history one press away.
