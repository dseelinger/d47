...
<svg viewBox="0 0 880 232" role="img" aria-label="The journal is computed into a numbered list of facts, and only that list reaches the model">
 ...
 <text x="440" y="210" text-anchor="middle" font-size="15" fill="var(--text-muted)">It promotes a routine docking into a narrow escape, because that reads better and nothing stops it.</text>
</svg>
</section>
<section>
...
<svg viewBox="0 0 880 246" role="img" aria-label="Sentences carry bracketed fact numbers, and a sentence citing nothing is marked in the file">
 ...
 <text x="440" y="208" text-anchor="middle" font-size="17" font-weight="700" fill="var(--accent)">Counted in the header, marked where it stands, listed at the foot.</text>
 <text x="440" y="238" text-anchor="middle" font-size="15" fill="var(--text-muted)">The brackets stay in the finished file on purpose — a Sources section no sentence points into records nothing.</text>
</svg>
<p class="body">Nothing you or anybody else typed in game is ever part of it. <code>ReceiveText</code> and <code>SendText</code> have no handler at all, so a hostile message cannot travel into the largest prompt Directive 47 assembles — or out of it into something you posted.</p>
</section>
<section>
...
<svg viewBox="0 0 880 236" role="img" aria-label="A log can be written in your voice, in the ship AI's voice, or yours with the AI interjecting">
 ...
 <text x="440" y="202" text-anchor="middle" font-size="16" fill="var(--text)">The middle one is the only one Directive 47 can do — which is why you opt into it.</text>
 <text x="440" y="230" text-anchor="middle" font-size="15" fill="var(--text-muted)">With personality off it writes plainly, and the header says which voice actually wrote it.</text>
</svg>
</section>
<section>
