...
**One chord, two acts.** On the Transcript tab `Ctrl+L` scrolls the reading you are on above the
top of the view instead — In Ship, Log File, Journal or Raw journal, whichever is showing. **It
deletes nothing**: the file on disk is untouched, Directive 47's own record of the conversation is
untouched, and scrolling back up shows every line again. New lines arrive in the empty space below,
