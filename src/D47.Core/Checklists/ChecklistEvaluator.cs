...
/// <summary>
/// What the journal says about a derived item (Phase 17, "Per ship build planning/tracking" and "A
...
public static class ChecklistEvaluator
{
    ...
    /// <summary>
    /// The remembered loadout for a ship-scoped item, or null where the ship is not one d47 has been
    /// aboard.
    /// </summary>
    ...
    private static ChecklistVerdict Blueprint(ChecklistIntent intent, ShipModule module, CommanderGameState state)
    {
        ...
        // A wildcard grade is met by any engineering at all.
        if (wanted is null)
        {
        ...
        if (held < wanted)
        {
            // The gate is checked here rather than earlier, because a Commander part-way up a blueprint has
            // already proved they can reach the grades below.
        ...
        // Level is the fact — it names the grade the module has — and Quality is progress within it. 0.85 and
        // above is finished, not 1.0: of 994 grades measured, the 45 the game let a Commander move on from
        // all sat at 0.85 or above, and testing for 1.0 would tell them a module they can see is finished is
        // not (EngineeringRules.CompleteAt).
    ...
    /// <summary>The last check, and the one d47 cannot always make.</summary>
    ...
    private static ChecklistVerdict Unengineered(ChecklistIntent intent, ShipModule module, int? rank)
    {
        ...
        return new ChecklistVerdict(
            ChecklistState.Open,
            // "is not currently engineered", because this verdict is a reading taken at a moment and not a
    ...
    /// <param name="loadout">
    /// The ship this slot is on, so the slot can be named as a Commander says it rather than as the
    /// journal spells it (asked for 2026-08-24).
    /// </param>
    ...
    private static ChecklistVerdict? Facility(ChecklistItem item, ChecklistIntent intent, CommanderGameState state)
    {
        if (SiteFor(item, intent, state) is not { } site)
        {
            ...
            // (docs/spikes/colonisation-sources.md), so d47 has nothing to predict from and says what it
            // knows: that it has not seen this one.
            return new ChecklistVerdict(
                ChecklistState.Open,
    ...
    /// <summary>A grade on a suit or a hand weapon.</summary>
    ...
    /// <summary>A modification on a suit or a hand weapon.</summary>
        ...
        ChecklistItem item, ChecklistIntent intent, CommanderGameState state)
    {
        ...
        // Every slot spent and this one not among them.
        ...
        // A symbol nothing joins is not evidence of absence.
    ...
    // Internal rather than private since Phase 42: the ordering asks the same question — is this ship-scoped
    // line about the ship being flown — and two answers to it would drift.
    ...
    /// <summary>
    /// The module an intent's subject names — the slot first, then the fitted item, because a Commander
    /// says "thrusters" and a plan built from the ship says "MainEngines".
    /// </summary>
    ...
    /// <summary>
    /// The module as a Commander says it: the size and rating in front of the name, which is the order
    /// outfitting lists it in.
    /// </summary>
    ...
    /// <summary>
    /// The Commander's rank with whoever would roll this: the engineer the plan names, or the one who
    /// rolled the module already.
    /// </summary>
