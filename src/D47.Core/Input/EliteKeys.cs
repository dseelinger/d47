...
/// <summary>What kind of control a bindings entry names.</summary>
public enum EliteInputKind
{
    ...
    /// <summary>An axis.</summary>
    Axis,
}

/// <summary>One resolved control: what kind it is, and the code to send if d47 can send it.</summary>
/// <param name="Kind">What sort of control the symbol named.</param>
/// <param name="Code">
/// A virtual-key code for <see cref="EliteInputKind.Keyboard"/>, a one-based button number for <see
/// cref="EliteInputKind.Mouse"/>, and meaningless otherwise.
/// </param>
...
/// <summary>
/// Elite's key symbols to something d47 can press (Phase 10, "Know which actions the Commander can
/// actually reach").
/// </summary>
public static class EliteKeys
{
    /// <summary>The device name Elite writes for a slot with nothing in it.</summary>
    ...
    private static readonly Dictionary<string, uint> KeyboardCodes = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        // The numpad's Enter shares Enter's virtual-key code and is told apart only by the extended-key bit,
        // which the injector applies from the symbol rather than the code.
        ["Numpad_Enter"] = 0x0D,
    ...
    /// <summary>Symbols whose scancode needs the extended-key prefix.</summary>
    ...
    /// <summary>
    /// Resolves one symbol as the bindings file spells it — <c>Key_W</c>, <c>Mouse_1</c>, <c>Joy_3</c>,
    /// <c>Pos_Joy_RXAxis</c>.
    /// </summary>
    public static EliteKey Resolve(string? symbol)
    {
        ...
        // An analog control carries a direction prefix.
        ...
        {
            // Elite numbers mouse buttons from one.
            return uint.TryParse(name[6..], out var button) && button is >= 1 and <= 5
                ? new EliteKey(EliteInputKind.Mouse, button)
                : EliteKey.None;
        }

        // Everything with a device prefix d47 has no way to press: sticks, gamepads, hand controllers.
    ...
    /// <summary>
    /// How a Commander would say the device a symbol belongs to, for the spoken reason an unreachable
    /// action gives back.
    /// </summary>
