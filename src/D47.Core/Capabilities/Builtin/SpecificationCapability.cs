...
/// <summary>What a hull and a module can do (Phase 14, "Elite Dangerous Ships").</summary>
public static class SpecificationCapability
{
    ...
    public static CapabilityDescriptor Create(Func<CommanderGameState?> commander) => new()
    {
        ...
        Keywords =
        [
            // Phrases only, and every one of them about a ship as a class of thing rather than about this
            // Commander's — "my ship" belongs to the journal capability and routing it here would answer a
            // question about a hull when one was asked about a hold.
            "what pad does",
            "how fast is a",
            "top speed of",
        ],
    ...
    private static string DescribeShip(Func<CommanderGameState?> commander, ToolArguments arguments)
    {
        ...
        // The Loadout's symbol, not its display name: the symbol is what the table is keyed by, and going
        // through the display name would put a round trip through the matcher for a lookup that is already
        // exact.
    ...
    /// <summary>What to say about a hull with no figures, which is three different sentences.</summary>
    ...
    /// <summary>How many variants are worth reading out in full before the answer becomes a table.</summary>
    ...
    private static string Describe(ModuleSpecification module)
    {
        ...
        if (module.IsDrive)
        {
            // The figures a jump range is actually computed from, and the reason the table carries them at
            // all: they are the ones nobody can eyeball and every jump calculator needs.
            report.Append($". Optimal mass {module.OptimalMass} t, max fuel per jump {module.MaxFuelPerJump} t");
        }

        if (module.HullBoost is not null)
        {
            // All four resistances, including the zeroes.
    ...
    /// <summary>Slot sizes as counts by size — "2 × 4, 3 × 3" — rather than a list.</summary>
