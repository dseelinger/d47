...
/// <summary>
/// On foot: what the Commander is wearing, what upgrading or modifying it costs, who does it, and where
/// the materials come from (Phase 20).
/// </summary>
public static class OnFootCapability
{
    public const string Id = "on-foot";

    /// <summary><param name="plans"> The Commander's suit and weapon plans (Phase 27).</summary>
    /// <param name="plans">The Commander's suit and weapon plans (Phase 27).</param>
        ...
        Func<CommanderGameState?> commander, OnFootPlanService? plans = null) => new()
    {
        ...
        // Each names its tool (#161); five here take no required argument, two of which change a plan rather
        // than report one.
        Keywords =
        [
        ...
        [
            ...
            // The three plan tools are Protected, so none of them is advertised to the model and all three
            // are reachable by phrase and by press.
            new ToolDefinition
            {
                Protected = true,
            ...
            // Protected for safety as well as cost.
            new ToolDefinition
            {
                Protected = true,
    ...
    /// <summary>Which item a plan tool is about.</summary>
        ...
        string what, int? grade, IReadOnlyList<FittedModification> fitted, int? free)
    {
        ...
        {
            // Not a shortage and not a slow route: grade 1 cannot be modified at all, and the fix is Pioneer
            // Supplies rather than an engineer.
        ...
        // A symbol d47 cannot name is said as one, exactly as a fitted ship blueprint is, rather than being
        // dropped from the list as though nothing were there.
    ...
    /// <summary>
    /// The name the upgrade recipes are filed under, which is the equipment's without the word "Suit"
    /// on the end — EDDiscovery writes "Maverick Suit" and EDEngineer writes "Maverick".
    /// </summary>
    ...
    private static string Barter(MaterialEntry material, int wanted, CommanderGameState? active)
    {
        ...
        // Said because it changes what a Commander should offer: rounding is theirs to lose, so trading in
        // one lump beats trading in ones.
