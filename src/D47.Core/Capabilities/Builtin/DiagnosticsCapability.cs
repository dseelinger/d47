...
/// <summary>
/// The first capability, and the one that makes "Turn a subsystem up without restarting" reachable
/// rather than merely implemented (Phase 1).
/// </summary>
public static class DiagnosticsCapability
{
    ...
    /// <summary>
    /// A closed vocabulary, emitted into the tool schema as an enum and validated before the handler
    /// runs.
    /// </summary>
    public static readonly IReadOnlyList<string> LogLevelNames =
        Enum.GetNames<LogLevel>();

    /// <summary>The settings row a subsystem's level lives in.</summary>
    public static string LevelRowFor(string subsystem) => $"logging.subsystems.{subsystem.ToLowerInvariant()}";

    /// <summary>
    /// <param name="coverage"> What has been exercised by hand, when this process was asked to record
    /// that.
    /// </summary>
    /// <param name="coverage">
        ...
        Journal.HistoryBackfill? history = null)
    {
        return new CapabilityDescriptor
        {
            ...
            // Phrases rather than words, for the same reason as the journal capability: a bare "status"
            // hijacks any sentence containing it and answers a question nobody asked.
            Keywords =
            [
                "your status",
                "status report",
        ...
        Journal.HistoryBackfill? history)
    {
        ...
        // Named here because it is a file rather than a settings row (#150), so the diagnostics page is the
    ...
    /// <summary>Writes the settings row rather than the level switch directly.</summary>
    private static ToolResult SetVerbosity(ToolArguments arguments, SettingsService settings)
    {
        // The registry has already checked both values against the declared vocabularies, so anything failing
        // here is a genuine mismatch rather than a bad model guess.
    ...
    /// <summary>
    /// The hand-testing coverage row, named because the settings surface recognises it: it is the one
    /// row that offers a window rather than a value, and a magic string in the view would be a second
    /// place for the key to be wrong.
    /// </summary>
    ...
    private static IReadOnlyList<SettingRow> BuildSettingRows(Func<string>? coverage)
    {
        ...
        rows.AddRange(Subsystems.All.Select(subsystem => new SettingRow
        {
            ...
            Advanced = true,
            // Terse on purpose: nine rows each explaining that they are one subsystem's level is nine copies
            // of one sentence.
            ...
            Binding = new SettingBinding
            {
                ...
                // advertises — so clearing the row removes the entry rather than writing the default level
                // back as if it had been chosen.
        ...
        {
            // Registered only when this process was asked to record coverage, so the descriptor is
            // byte-identical to a normal run's otherwise — the registered-once-never-mutated invariant holds
            // either way, because which rows exist is decided before startup.
