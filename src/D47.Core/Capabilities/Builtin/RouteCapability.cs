...
/// <summary>Plotting a route (Phase 14, "Route Planning").</summary>
public static class RouteCapability
{
    public const string Id = "routes";

    /// <summary>
    /// <param name="routes"> The plotter, or null where none is composed — under the designer and in a
    /// test that is not about it.
    /// </summary>
    /// <param name="routes">
    /// The plotter, or null where none is composed — under the designer and in a test that is not about
    /// it.
    /// </param>
    /// <param name="commander">Where the Commander is and what they are flying.</param>
    /// <param name="trade">d47's own trade planner, or null where none is composed.</param>
    /// <param name="plans">
        ...
        Func<DateTimeOffset>? now = null) => new()
    {
        ...
        // No keywords, for the reason the galaxy search has none: every question here carries the thing being
        // asked about, and a router that cannot extract a destination cannot route "plot me a route to
        // Colonia" anywhere useful.
        Tools =
        [
            new ToolDefinition
            {
            ...
            new ToolDefinition
            {
                ...
                [
                    ...
                    // Spelled with its unit, matching the commodity search rather than differing from it
                    // (#178).
                    new ToolParameter
                    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Kept before it is described, so a surface has the whole route while the sentence keeps its five
            // waypoints (Phase 37).
    ...
    /// <summary>A plotted route as prose, cut to the next few waypoints.</summary>
    ...
    /// <summary>How much of a route is worth saying.</summary>
    ...
    private static string Describe(RichesRoute route, RichesQuery query)
    {
        ...
        foreach (var stop in route.Stops.Take(StopsSpoken))
        {
            ...
            // The bodies at each stop are the part that gets long: a single system can carry twenty.
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Null and empty are different answers and are worded differently.
            if (route is null)
            {
                return ToolResult.Ok(
    ...
    /// <summary>A trade route as prose.</summary>
    private static string Describe(TradeRoute route, TradeQuery query)
    {
        ...
        foreach (var stop in route.Stops)
        {
            ...
            // The line that is the whole phase.
            ...
            // Prices are crowd-reported unless the Commander read them off the board themselves, and a route
            // that is arithmetically perfect against a four-year-old market is worth nothing at all.
