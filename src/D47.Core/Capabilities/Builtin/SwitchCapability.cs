...
public sealed record SwitchSurface
{
    ...
    /// <summary>A surface that reaches no hardware.</summary>
...
/// <summary>HOTAS switches that mean a state rather than a press (Phase 21).</summary>
public static class SwitchCapability
{
    ...
    public static CapabilityDescriptor Create(SwitchSurface surface, Func<bool> keyboardEnabled) => new()
    {
        ...
        Tools =
        [
            // Argument-free, so the model-free keyword router reaches it with nothing in the path — which is
            // what makes it answerable in a headset.
            new ToolDefinition
            {
    ...
    /// <summary>The live half, for prompt position 7 — the disagreements and nothing else.</summary>
    ...
    /// <summary>What both surfaces show, in one sentence: the switches that disagree, or nothing at all.</summary>
    ...
    /// <summary>
    /// Protected: the model reads untrusted text, so a switch
    /// it could turn on to give a mapped toggle the keyboard is privilege escalation rather than a
    /// convenience.
    /// </summary>
    private static SettingRow EnabledRow(Func<bool> keyboardEnabled) => new()
    {
        ...
        // Absent rather than greyed out when there is nothing for it to do.
    ...
    /// <summary>A read-only summary that carries the way into the walk.</summary>
