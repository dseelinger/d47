...
/// <summary>Which of the three material lockers something belongs to.</summary>
...
{
    /// <summary>The player-facing name where the journal supplied one.</summary>
...
/// <summary>
/// What the Commander is carrying (Phase 7, "Know what materials you are carrying"), answerable with no
/// external lookup.
/// </summary>
public sealed record MaterialsInventory
{
    ...
    /// <summary>
    /// An init-only property rather than a readonly field so that <c>with</c> can replace it — which is
    /// the only way this record stays immutable while still folding deltas.
    /// </summary>
    ...
    /// <summary>Whether a full snapshot has been seen.</summary>
    ...
    public MaterialsInventory Apply(JournalEvent journalEvent)
    {
        switch (journalEvent.Kind)
        {
            ...
            // A trade is both directions at once, and the two materials are usually different grades of the
            // same line.
            case "MaterialTrade":
            {
                var traded = this;
            ...
            // Everything that consumes materials, under the property each event happens to use for its
            // ingredient list.
            case "EngineerCraft":
    ...
    private MaterialsInventory Add(string? name, MaterialCategory category, int delta, string? displayName)
    {
        ...
        // Clamped at both ends, per the note on this type.
        ...
        updated[name] = new MaterialHolding(
            name,
            // A delta event whose category is missing must not overwrite a known one with Unknown; the
            // snapshot is where categories are reliable.
