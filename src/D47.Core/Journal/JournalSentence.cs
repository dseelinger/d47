...
/// <summary>One journal event as a line a person can read (#51).</summary>
public static class JournalSentence
{
    /// <summary>The kinds that are noise rather than events, hidden by default (#51).</summary>
    ...
    public static string For(JournalEvent entry)
    {
        ...
        return entry.Kind switch
        {
            ...
            // ---- Ships and modules -------------------------------------------------------- Loadout carries
            // Ship as a BARE SYMBOL with no localised twin, so this asked for the Commander's own ship name
            // and said nothing otherwise.
            ...
            "ModuleBuy" => Named(raw, "BuyItem") is { } module
                ? $"Bought a {module}{ModuleCost(raw, "BuyPrice")}"
                : "Bought a module",
            ...
            "ModuleBuyAndStore" => Named(raw, "BuyItem") is { } boughtStored
                ? $"Bought and stored a {boughtStored}{ModuleCost(raw, "BuyPrice")}"
            ...
            "Resurrect" => raw.Long("Cost") is { } rebuy
                ...
                : "Rebought the ship",
            // The localised crime name carries a symbol tail — "...no fire zone_hulldamage" — so it is cut at
            // the underscore.
            ...
            // ---- Comms and crew ----------------------------------------------------------- The message
            // itself is the content, so the sentence is the sender and the words (#260).
            ...
            "GameModeChange" => Blank(raw.String("GameMode")) switch
            {
                "MainGame" => "Back in the main game",
                "Operation" => "Entered an operation",
                { } other => $"Game mode changed to {Spaced(other)}",
                null => "Game mode changed",
            },
            ...
            // Everything else. 221 kinds exist and this covers what is worth covering; the rest list as their
            // own name beside a detail pane that is exactly as complete as any other's, which is a summary
            // that is missing rather than one that is wrong.
            _ => Spaced(entry.Kind),
        };
    }

    /// <summary>A comms line, named where Elite said who it was from and bare where it did not (#260).</summary>
    private static string Attributed(string? sender, string message) =>
        sender is { Length: > 0 } named ? $"{named}: {message}" : message;

    /// <summary>Who a comms event was from or to, undecorated, or null where Elite named nobody.</summary>
    ...
    /// <summary>Everything before the first underscore.</summary>
    ...
    /// <summary>
    /// A module's price, falling back to Merc Coins when Frontier zeroes <c>BuyPrice</c> and carries
    /// the real cost in <c>BuyMercCoinsPrice</c> instead (#300).
    /// </summary>
    private static string ModuleCost(JsonElement raw, string property) =>
        raw.Long(property) is { } amount and > 0 ? $" for {Credits(amount)}" : MercCost(raw);

    private static string MercCost(JsonElement raw) =>
        raw.Long("BuyMercCoinsPrice") is { } merc and > 0
            ? $" for {merc.ToString("N0", CultureInfo.CurrentCulture)} Merc Coins"
            : string.Empty;

    private static string Grade(JsonElement raw) =>
        raw.Int("Level") is { } level ? $", grade {level}" : string.Empty;

    /// <summary><c>Armour_HeavyDuty</c> into <c>Armour Heavy Duty</c>.</summary>
    ...
    /// <summary>
    /// A credit figure as the sentence speaks it: banded at millions, billions and trillions with no
    /// "Cr" once it's banded — "for 362.6 million", not "for 362.6 million Cr" — and exact digits with
    /// "Cr" below the first band, where they are a price rather than a total (#336).
    /// </summary>
