...
/// <summary>
/// The debrief pass: a session's spoken corrections, turned into proposed standing directions (#162).
/// </summary>
public static class DebriefExtractor
{
    /// <summary>How many times a pattern has to repeat before it is worth a question.</summary>
    public const int SignalThreshold = 3;

    /// <summary>The most one pass may draft.</summary>
    public const int MaxProposals = 12;

    /// <summary>The longest utterance a direction may be drafted from.</summary>
    ...
    /// <summary>Cues that mean a standing instruction wherever they appear.</summary>
    ...
    /// <summary>Cues that only count when the Commander led with them.</summary>
    ...
    /// <summary>What a Commander puts in front of an instruction before the instruction starts.</summary>
    ...
    /// <param name="lines">The session, all three voices.</param>
    /// <param name="signals">Patterns nobody put into words.</param>
    /// <param name="known">
    /// Everything already in the file for this Commander — adopted, waiting and declined.
    /// </param>
    /// <param name="now">When the pass ran.</param>
    /// <param name="saidUnder">
    /// Which core was aboard, recorded so the pane can offer "just for this core".
    /// </param>
    ...
    public static string? Draft(string utterance, IReadOnlyCollection<string>? addressedAs = null)
    {
        ...
        // Stripping can take the cue with it, and "no more speeches about that" is the case that matters:
        // "no" is address in front of an instruction and is the first word of one here.
        if (!Cued(Words(stripped)) && Cued(Words(original)))
        {
            stripped = original;
        }

        // A question is a question. "Don't you think that is a long way?" carries a cue and is not an
        // instruction, and there is no reading of a trailing question mark that makes it one.
    ...
    private static string StripAddress(string text, IReadOnlyCollection<string>? addressedAs)
    {
        ...
        while (true)
        {
            ...
            // The whole utterance was address. "Hey d47" is not an instruction, and returning the empty
            // string here is what says so.
            if (rest.Length == 0)
            {
                return string.Empty;
            }
    ...
    /// <summary>The questions the implicit signals earn.</summary>
    private static IEnumerable<StandingDirection> Questions(IReadOnlyList<DebriefSignal> signals)
    {
        ...
        {
            // The count is deliberately not in the text.
    ...
    /// <summary>What two utterances have to share to be the same one: letters, digits and single spaces.</summary>
