...
/// <summary>
/// One line of the on-foot index: a suit or a weapon, whatever is planned for it, and whether it is on
/// the Commander right now (Phase 27, "The same page, on foot").
/// </summary>
...
/// <summary>
/// The Commander's suit and weapon builds, joined to what they are actually wearing (Phase 27, "The
/// same page, on foot").
/// </summary>
    ...
    Func<CommanderGameState?> state)
{
    ...
    /// <summary>
    /// The build a phrase means, made if it does not exist yet: the item named, the suit being worn
    /// when nothing is named, or a fresh intended build for something the Commander has not bought.
    /// </summary>
    public static OnFootBuild? Which(OnFootPlanService kit, string? named, bool weapon = false)
    {
        ...
        {
            // Nothing named means the thing there is only one of: the suit being worn, or the one weapon
            // being carried.
    ...
    /// <summary>Starts a build for a suit or weapon the Commander does not own (Phase 27).</summary>
    ...
    /// <summary>The first modification slot with nothing in it, or null when all four are spoken for.</summary>
    ...
    public string Promote(string buildId)
    {
        ...
        if (build.Scope is not { } scope)
        {
            // No SuitID, so there is no list for its items to be in.
        ...
        // Every item of this plan shares one subject — the equipment — so the whole build is what is
        // replaced, and a slot the Commander cleared drops off the list rather than standing because nothing
        // mentioned it.
        return checklists.ProposePlan(scope, ChecklistSource.OnFootPlan, items, [build.Equipment]);
    }

    /// <summary>
    /// Watches for the Commander buying something they had planned for, and adopts the plan onto it
    /// (Phase 27).
    /// </summary>
    public IReadOnlyList<string> Observe(IEnumerable<JournalEvent> events)
    {
        ...
        foreach (var journalEvent in events)
        {
            ...
            // The symbol, never Name_Localised: Frontier's own localisation reports every suit above grade 1
            // as Class1, which would adopt the plan onto the wrong grade's name.
