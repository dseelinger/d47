...
/// <summary>Encryption at rest for secrets.</summary>
public interface ISecretProtector
{
    byte[] Protect(byte[] plaintext);

    /// <summary>
    /// False rather than throwing: a blob that will not decrypt is a secret that is unavailable, which
    /// reads downstream as a capability being off.
    /// </summary>
    bool TryUnprotect(byte[] ciphertext, [NotNullWhen(true)] out byte[]? plaintext);
}

/// <summary>
/// DPAPI with <see cref="DataProtectionScope.CurrentUser"/> — no key management, and the reason the
/// install is per-user.
/// </summary>
