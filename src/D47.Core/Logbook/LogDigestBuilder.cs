...
public sealed class LogDigestBuilder(ILogger logger)
{
    ...
    /// <summary>The most individual facts one category contributes.</summary>
    public const int MaxPerKind = 25;

    /// <summary>Events that never reach a digest, whatever else changes.</summary>
    public static IReadOnlyList<string> ExcludedEvents { get; } = ["ReceiveText", "SendText"];

    /// <summary>Which categories survive the global cap, most notable first.</summary>
    ...
    public LogDigest Build(IReadOnlyList<string> files, LogRange range)
    {
        ...
        foreach (var file in LogRanges.FilesFor(files, range))
        {
            ...
            foreach (var line in lines)
            {
                ...
                // Identity is established from wherever it is stated, including from before the window
                // opened: a log of the last two hours of a five-hour session is still that Commander's, and
                // the event that named them is at the top of the file.
    ...
    private static LogDigest Seal(Fold fold, LogRange range, int journals, long events)
    {
        ...
        // Partitioned rather than appended wholesale.
        ...
        // The global cap trims moments rather than totals, and from the least notable categories first.
        ...
        // Numbered in the order the digest renders them, so a Commander reading the Sources section counts
        // down the page rather than hunting.
    ...
    /// <summary>Every roll of one blueprint on one module at one engineer, as one fact.</summary>
    ...
    /// <summary>The accumulators.</summary>
    private sealed class Fold
    {
        ...
        /// <summary>Who is flying, from wherever it was stated.</summary>
        ...
        public void Apply(JournalEvent journalEvent)
        {
            ...
            switch (journalEvent.Kind)
            {
                ...
                // Counted apart from launches. "1 launch or loss" reads as a launch, so a Commander who put
                // an SRV down a crater was being told they had used one — a small untruth, and the
                // read-through that found it is the reason the item asks for one.
                case "SRVDestroyed":
        ...
        private void Where(JournalEvent journalEvent)
        {
            ...
            // The event that put the Commander here, kept so the closing line can cite it.
            _lastPlace = new LogSource(journalEvent.Kind, journalEvent.Timestamp);
        }

        /// <summary>One engineering roll, folded into the module and blueprint it was for.</summary>
        ...
        /// <summary>The first thing that happened, stated once.</summary>
        ...
        {
            // Spoken, not Named: Elite omits _Localised on plenty of ship symbols and writes them lower case,
            // so one log would call it an "anaconda" in one line and an "Anaconda" in the next.
        ...
        /// <summary>A few of a set, and how many more.</summary>
