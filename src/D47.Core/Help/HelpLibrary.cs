...
/// <summary>Reads the ELI5 band out of a shipped documentation page (asked for 2026-08-22).</summary>
public static class HelpLibrary
{
    /// <summary>Where the csproj puts the pages.</summary>
    private const string ResourcePrefix = "D47.Core.Help.";

    private const string BandOpen = "<div class=\"d47-eli5\">";

    /// <summary>The how-to band's opener (#229).</summary>
    ...
    /// <summary>The band for a capability, or null when that page has none yet.</summary>
    ...
    /// <summary>
    /// The band out of one page's markdown, or null when it has none — which most pages still do, and
    /// which is a page waiting to be written rather than a fault.
    /// </summary>
    public static HelpArticle? Parse(string? markdown, string capabilityId) =>
        Parse(markdown, capabilityId, BandOpen);

    /// <summary>The how-to band, read by the same parser (#229).</summary>
    ...
    private static HelpArticle? Parse(string? markdown, string capabilityId, string opener)
    {
        ...
        return new HelpArticle
        {
            ...
            // From the band when they are still in it, and from the foot of the page when they are not
            // (#229).
    ...
    /// <summary>The cards from the foot of a page, when they are not inside the band.</summary>
    ...
    /// <summary>The "where to go next" cards at the foot of a band, if it has any.</summary>
        ...
        frame.Descendants("a")
            ...
            .Select(card =>
            {
                ...
                return new HelpLink
                {
                    ...
                    // A settings card names its section with the same id as the page it points at, which is
                    // what lets one href serve the browser and the panel both.
    ...
    private static string? Sibling(string href, string owner)
    {
        ...
        {
            // Climbing out of the capability folder reaches the general pages.
            if (fromGeneralPage)
            {
                return null;
            }
        ...
        {
            // The only way down, and only from up there — a capability page naming capabilities/x.html would
            // be pointing into a folder it is already in.
            if (!fromGeneralPage)
            {
                return null;
            }
    ...
    /// <summary>The band's span, by counting <c>div</c>s rather than by matching a closing tag.</summary>
    ...
    // A tag rather than a prefix: "<divider" is not a div, and neither is text that happens to contain the
    // letters.
    ...
    /// <summary>
    /// Walks the drawing in document order, which is paint order — a later shape covers an earlier one,
    ...
    /// <summary>A colour role, from a <c>var(--name)</c> token.</summary>
