...
/// <param name="Sha256">
/// The hash it is expected to have, pinned here rather than taken from whatever the host says on the
/// day — the rule <see cref="Listening.WhisperModels"/> already follows, and for the same reason: the
/// hash and the bytes came from the same server, so anything able to serve different bytes could serve
/// the hash for them.
/// </param>
...
/// <summary>What has to be on this machine before d47 can speak without the internet (#101, Phase 59).</summary>
public static class KokoroAssets
{
    ...
    /// file stores and what a support question is answered with.
    /// </param>
    /// <param name="RealtimeMultiple">
    /// How many seconds of speech it renders per second, measured through
    /// <c>Microsoft.ML.OnnxRuntime</c> on the Commander's machine.
    /// </param>
    public sealed record KokoroBuild(string Id, KokoroAsset Asset, double RealtimeMultiple)
    {
        /// <summary>
        /// How long a Commander waits before this build starts speaking, for a line of the length the
        /// benchmark used (#216).
        /// </summary>
        public double SecondsBeforeSpeaking => ReferenceLineSeconds / RealtimeMultiple;

        /// <summary>What the picker shows: the size and the wait, together and never apart.</summary>
        public string Label =>
            $"{Id} — {Asset.Megabytes:0} MB, about {SecondsBeforeSpeaking:0.0} s before it speaks";
    }

    /// <summary>
    /// The line every build was timed against: 7.2 seconds of speech, a realistic d47 utterance (#139's
    /// <c>spike/KokoroProbe bench</c>).
    /// </summary>
    public const double ReferenceLineSeconds = 7.2;

    /// <summary>The build every Commander gets unless they choose otherwise.</summary>
    public const string DefaultBuildId = "fp32";

    /// <summary>All eight published builds, each with a measured speed beside its size (#139).</summary>
    ...
    /// <summary>
    /// The named build, or the default where the name is unknown — a settings file naming a build this
    /// version no longer publishes resolves rather than failing, the way every other stored id in this
    /// repository does.
    /// </summary>
    ...
    /// <summary>The model itself, in the fp32 build the Commander chose.</summary>
    ...
    /// <summary>
    /// The 274,927-entry pronunciation dictionary, which is the top rung of the ladder and the reason
    /// ordinary English comes out right rather than by rule.
    /// </summary>
    ...
    /// <summary>The English voices, which is all of them d47 offers.</summary>
    ...
    /// <summary>What a voice is called.</summary>
