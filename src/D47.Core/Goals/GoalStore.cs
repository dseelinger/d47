...
public sealed class GoalStore(string path, ILogger<GoalStore> logger)
{
    ...
    /// <summary>Bounds how many arcs a person can invent.</summary>
    ...
    /// <summary>Arc keys this Commander does not want to see.</summary>
    ...
    /// <summary>
    /// Files one walk's conclusions, replacing whatever that Commander had before — because mining is a
    /// recomputation, and merging would carry forward a count the current journals no longer support.
    /// </summary>
    ...
    /// <summary>Writes a goal the Commander invented.</summary>
    ...
    /// <summary>Drops an authored arc outright.</summary>
    ...
    /// <summary>Throws away everything mined, for every Commander, and reports how many marks went.</summary>
    public int Empty()
    {
        Poll();
    ...
    private void Reload(string text)
    {
        ...
        try
        {
            foreach (var commander in JsonSerializer.Deserialize<Document>(text, Json)?.Commanders ?? [])
            {
                ...
                if (commander.Goals is { Count: > 0 } goals)
                {
                    ...
                    foreach (var record in goals)
                    {
                        ...
                        kept.Add(new GoalArc
                        {
                            ...
                            // A hand-written goal with no definition of done still reads back.
                ...
                // A record carrying only a person's own goals is a real state — nothing has been mined yet —
                // and it gets no mine rather than an empty one, so "have you ever looked" stays answerable.
    ...
    private sealed class CommanderRecord
    {
        ...
        /// <summary>The Commander's own arcs.</summary>
