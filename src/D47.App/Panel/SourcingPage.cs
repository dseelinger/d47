...
/// <summary>Everything one build still needs, and where to buy it (Phase 50).</summary>
public sealed class SourcingPage : UserControl
{
    ...
    /// <summary>The two boxes, built fresh on every draw rather than kept.</summary>
    ...
    /// <summary>
    /// Says something, and remembers it — the page rebuilds itself on every change, so a block holding
    /// the only copy would lose the sentence the moment a carrier figure was entered.
    /// </summary>
    ...
    /// <summary>What the Commander says is aboard the carrier, and the one place they can say it.</summary>
    private Control CarrierCard(ConstructionSite site)
    {
        ...
        foreach (var stock in aboard)
        {
            ...
            // Dated on every row, because this is the one number in a sourcing plan D47 has no way of
            // checking: a week-old "300 tritium" is a week-old memory of a carrier that has been flown since.
        ...
        // Fixed pixel columns, so the same wrap the results tables got (#333): narrow enough that these rows
        // fit today, but a scroll viewport is what keeps them inside the card if they ever stop fitting.
    ...
    private Control SearchCard(ConstructionSite site)
    {
        ...
        find.Click += async (_, _) =>
        {
            ...
            try
            {
                ...
                // **The sentence does not stay.** That is the Market page's arrangement and it is right
                // there, where the spoken answer is three lines; a build's is twenty and says exactly what
                // the table below is about to say again.
                Say(string.Empty);
                Refresh();
            }
    ...
    private void DrawPlan()
    {
        ...
        // **Nothing is dropped in silence.** Every outstanding row either resolves to a station above or is
        // named here, and found-but-short is separate from never-found: "widen the search" is the right
    ...
    private static Control Stop(SourcingStop stop, bool distances)
    {
        ...
        {
            lots.Children.Add(new StackPanel
            {
                Orientation = Orientation.Horizontal,
                Spacing = 10,
                Children =
        ...
        // Same wrap as every other fixed-column block on the page (#333).
        body.Children.Add(new ScrollViewer
        {
            HorizontalScrollBarVisibility = ScrollBarVisibility.Auto,
            VerticalScrollBarVisibility = ScrollBarVisibility.Disabled,
