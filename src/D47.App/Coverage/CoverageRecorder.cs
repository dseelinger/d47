...
/// <summary>
/// Notes which tools and settings rows have actually been driven in the running app, so "have I
/// exercised this by hand, and has it changed since I did" has an answer.
/// </summary>
public sealed class CoverageRecorder
{
    ...
    /// <summary>
    /// A recorder without consulting the environment, so a test can exercise the wiring without setting
    /// a process-wide variable that every other test in the run would also see.
    /// </summary>
    ...
    public void Follow(CapabilityRegistry registry, SettingsService settings)
    {
        ...
        // Applied rather than Changed: a row set to the value it already had, or refused a bad value, was
        // still driven by hand, and Changed does not fire for either.
    ...
    /// <summary>Writes the ledger and the readable report.</summary>
    public void Save()
    {
        try
        {
    ...
    /// <summary>How an apply reads as coverage, or null when it is not evidence of anything.</summary>
    private static CoverageOutcome? OutcomeOf(SettingApplyStatus status) => status switch
    {
        // The change was valid and could not be saved.
        SettingApplyStatus.Failed => CoverageOutcome.Failed,

        // Refused is the protected rule holding, which means the model can never be evidence that a protected
        // row has been driven — only the panel, a hotkey or the keyword router can be.
        SettingApplyStatus.Refused or SettingApplyStatus.UnknownKey => null,

        // Applied, Unchanged and Rejected all mean the row was driven and answered.
    ...
    private static CoverageLedger Load(string file, ILogger logger)
    {
        ...
        {
            // Starting over is the right failure: the record is an aid, and a corrupt one that stopped the
            // app would be worse than no record at all.
