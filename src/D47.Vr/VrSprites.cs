...
/// <summary>The two pictures that are not the panel: the aim beam and the cursor on the end of it.</summary>
public static class VrSprites
{
    /// <summary>
    /// The accent both share, so the beam visibly belongs to the dot on its end. d47's amber, which is
    /// the one colour the panel does not use for text.
    /// </summary>
    private static readonly (byte R, byte G, byte B) Accent = (255, 176, 64);

    /// <summary>
    /// The beam: solid at the hand and fading along its length, softened across it so the edges are not
    /// two hard lines a pixel wide.
    /// </summary>
    public static byte[] Beam()
    {
        ...
        {
            // Row zero is the far end, because the quad's +Y runs along the aim and V counts down from the
            // top.
    ...
    public static byte[] Cursor()
    {
        ...
        for (var y = 0; y < Size; y++)
        {
            for (var x = 0; x < Size; x++)
            {
                ...
                // Feathered on both edges by a pixel, which is what stops a small ring looking like a
                // staircase at the distance it is actually read from.
