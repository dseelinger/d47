...
> **Amended 2026-08-29, for [#175](https://github.com/dseelinger/d47/issues/175).** d47 now has a destination of its own — a Cloudflare Worker in front of an R2 bucket, `worker/`, which donated excerpts and journal histories are posted to. Two things follow for this section and both are built.
>
> **It is enumerated like every other destination.** `EgressDisclosure.Donation` is a row on the computed Privacy page, and it reads silent until an address is configured, which is out of the box. No-telemetry is untouched: the constraint in §1 rules out *ambient* collection — crash reporters, analytics SDKs, hosted logging — and a per-donation upload behind a full preview and an explicit press, every time, is the same shape as every other row here. What #175 reversed is a design commitment d47 made in a code comment, not an invariant.
>
> **And the boundary runs the other way as well, which is new.** Every row in the table above is untrusted input arriving *at* d47. The Worker is untrusted input arriving at *us*, from a stranger's machine over the open internet, and it is written to that standard: nothing that arrives chooses an object key, a prefix or a retention rule; the donor token is checked against a fixed hex alphabet before it becomes part of a path; the payload is refused on `Content-Length` before a byte of it is read; and a store failure says nothing about the store. The Worker is the only writer, so there is no bucket credential in the shipped binary and no path from a donor to storage that does not come through code that can refuse.
