...
| `--only language,speed,latency` | Which sections to run. Each one spends, so a re-measurement of one need not pay for three |
...
**It spends real characters on the Commander's account.** A full run is about 6,000 characters, so
roughly $0.30 at the $0.05 per thousand both models list at.
